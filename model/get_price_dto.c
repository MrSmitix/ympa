...
get_price_dto_t *get_price_dto_create(
    ...
    get_price_dto_t *get_price_dto_local_var = malloc(sizeof(get_price_dto_t));
    if (!get_price_dto_local_var) {
        return NULL;
    }
    get_price_dto_local_var->value = value;
    get_price_dto_local_var->currency_id = currency_id;
    get_price_dto_local_var->updated_at = updated_at;

    return get_price_dto_local_var;
}


void get_price_dto_free(get_price_dto_t *get_price_dto) {
    if(NULL == get_price_dto){
        return ;
    }
    listEntry_t *listEntry;
    if (get_price_dto->currency_id) {
        currency_type_free(get_price_dto->currency_id);
        get_price_dto->currency_id = NULL;
    }
    if (get_price_dto->updated_at) {
        free(get_price_dto->updated_at);
        get_price_dto->updated_at = NULL;
    }
    free(get_price_dto);
}
...
cJSON *get_price_dto_convertToJSON(get_price_dto_t *get_price_dto) {
    ...
    // get_price_dto->value
    if (!get_price_dto->value) {
        goto fail;
    }
    if(cJSON_AddNumberToObject(item, "value", get_price_dto->value) == NULL) {
    goto fail; //Numeric
    }


    // get_price_dto->currency_id
    if (_api__get_price_dto__NULL == get_price_dto->currency_id) {
        goto fail;
    }
    cJSON *currency_id_local_JSON = currency_type_convertToJSON(get_price_dto->currency_id);
    ...
    // get_price_dto->updated_at
    if (!get_price_dto->updated_at) {
        goto fail;
    }
    if(cJSON_AddStringToObject(item, "updatedAt", get_price_dto->updated_at) == NULL) {
    goto fail; //Date-Time
    }
...
get_price_dto_t *get_price_dto_parseFromJSON(cJSON *get_price_dtoJSON){
    ...
    get_price_dto_t *get_price_dto_local_var = NULL;

    // define the local variable for get_price_dto->currency_id
    currency_type_t *currency_id_local_nonprim = NULL;

    // get_price_dto->value
    cJSON *value = cJSON_GetObjectItemCaseSensitive(get_price_dtoJSON, "value");
    if (!value) {
        goto end;
    }
    ...
    // get_price_dto->currency_id
    cJSON *currency_id = cJSON_GetObjectItemCaseSensitive(get_price_dtoJSON, "currencyId");
    if (!currency_id) {
        goto end;
    }
    ...
    // get_price_dto->updated_at
    cJSON *updated_at = cJSON_GetObjectItemCaseSensitive(get_price_dtoJSON, "updatedAt");
    if (!updated_at) {
        goto end;
    }
    ...
    get_price_dto_local_var = get_price_dto_create (
        value->valuedouble,
