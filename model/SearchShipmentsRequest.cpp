/**
 * Партнерский API Маркета
 * API Яндекс Маркета помогает продавцам автоматизировать и упростить работу с маркетплейсом.  В числе возможностей интеграции:  * управление каталогом товаров и витриной,  * обработка заказов,  * изменение настроек магазина,  * получение отчетов. 
 *
 * The version of the OpenAPI document: LATEST
 * 
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.8.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */



#include "SearchShipmentsRequest.h"

#include <string>
#include <vector>
#include <map>
#include <sstream>
#include <stdexcept>
#include <regex>
#include <boost/lexical_cast.hpp>
#include <boost/property_tree/ptree.hpp>
#include <boost/property_tree/json_parser.hpp>
#include "helpers.h"

using boost::property_tree::ptree;
using boost::property_tree::read_json;
using boost::property_tree::write_json;

namespace org {
namespace openapitools {
namespace server {
namespace model {

SearchShipmentsRequest::SearchShipmentsRequest(boost::property_tree::ptree const& pt)
{
        fromPropertyTree(pt);
}


std::string SearchShipmentsRequest::toJsonString(bool prettyJson /* = false */) const
{
	std::stringstream ss;
	write_json(ss, this->toPropertyTree(), prettyJson);
    // workaround inspired by: https://stackoverflow.com/a/56395440
    std::regex reg("\\\"([0-9]+\\.{0,1}[0-9]*)\\\"");
    std::string result = std::regex_replace(ss.str(), reg, "$1");
    return result;
}

void SearchShipmentsRequest::fromJsonString(std::string const& jsonString)
{
	std::stringstream ss(jsonString);
	ptree pt;
	read_json(ss,pt);
	this->fromPropertyTree(pt);
}

ptree SearchShipmentsRequest::toPropertyTree() const
{
	ptree pt;
	ptree tmp_node;
	pt.put("dateFrom", m_DateFrom);
	pt.put("dateTo", m_DateTo);
	// generate tree for Statuses
    tmp_node.clear();
	if (!m_Statuses.empty()) {
        tmp_node = toPt(m_Statuses);
		pt.add_child("statuses", tmp_node);
		tmp_node.clear();
	}
	// generate tree for OrderIds
    tmp_node.clear();
	if (!m_OrderIds.empty()) {
        tmp_node = toPt(m_OrderIds);
		pt.add_child("orderIds", tmp_node);
		tmp_node.clear();
	}
	pt.put("cancelledOrders", m_CancelledOrders);
	return pt;
}

void SearchShipmentsRequest::fromPropertyTree(ptree const &pt)
{
	ptree tmp_node;
	m_DateFrom = pt.get("dateFrom", "");
	m_DateTo = pt.get("dateTo", "");
	// push all items of Statuses into member
	if (pt.get_child_optional("statuses")) {
        m_Statuses = fromPt<std::set<ShipmentStatusType>>(pt.get_child("statuses"));
	}
	// push all items of OrderIds into member
	if (pt.get_child_optional("orderIds")) {
        m_OrderIds = fromPt<std::set<int64_t>>(pt.get_child("orderIds"));
	}
	m_CancelledOrders = pt.get("cancelledOrders", true);
}

std::string SearchShipmentsRequest::getDateFrom() const
{
    return m_DateFrom;
}

void SearchShipmentsRequest::setDateFrom(std::string value)
{
    m_DateFrom = value;
}


std::string SearchShipmentsRequest::getDateTo() const
{
    return m_DateTo;
}

void SearchShipmentsRequest::setDateTo(std::string value)
{
    m_DateTo = value;
}


std::set<ShipmentStatusType> SearchShipmentsRequest::getStatuses() const
{
    return m_Statuses;
}

void SearchShipmentsRequest::setStatuses(std::set<ShipmentStatusType> value)
{
    m_Statuses = value;
}


std::set<int64_t> SearchShipmentsRequest::getOrderIds() const
{
    return m_OrderIds;
}

void SearchShipmentsRequest::setOrderIds(std::set<int64_t> value)
{
    m_OrderIds = value;
}


bool SearchShipmentsRequest::isCancelledOrders() const
{
    return m_CancelledOrders;
}

void SearchShipmentsRequest::setCancelledOrders(bool value)
{
    m_CancelledOrders = value;
}



std::vector<SearchShipmentsRequest> createSearchShipmentsRequestVectorFromJsonString(const std::string& json)
{
    std::stringstream sstream(json);
    boost::property_tree::ptree pt;
    boost::property_tree::json_parser::read_json(sstream,pt);

    auto vec = std::vector<SearchShipmentsRequest>();
    for (const auto& child: pt) {
        vec.emplace_back(SearchShipmentsRequest(child.second));
    }

    return vec;
}

}
}
}
}

