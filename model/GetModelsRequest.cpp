...
std::string GetModelsRequest::toJsonString(bool prettyJson /* = false */) const
{
	std::stringstream ss;
...
ptree GetModelsRequest::toPropertyTree() const
{
	ptree pt;
	ptree tmp_node;
...
void GetModelsRequest::fromPropertyTree(ptree const &pt)
{
	ptree tmp_node;
	// push all items of Models into member
	if (pt.get_child_optional("models")) {
        m_Models = fromPt<std::vector<int64_t>>(pt.get_child("models"));
	}
}
