...
std::string UpdateOrderStatusesRequest::toJsonString(bool prettyJson /* = false */) const
{
	std::stringstream ss;
...
ptree UpdateOrderStatusesRequest::toPropertyTree() const
{
	ptree pt;
	ptree tmp_node;
...
void UpdateOrderStatusesRequest::fromPropertyTree(ptree const &pt)
{
	ptree tmp_node;
	// push all items of Orders into member
	if (pt.get_child_optional("orders")) {
        m_Orders = fromPt<std::vector<OrderStateDTO>>(pt.get_child("orders"));
	}
}

std::vector<OrderStateDTO> UpdateOrderStatusesRequest::getOrders() const
{
    return m_Orders;
}

void UpdateOrderStatusesRequest::setOrders(std::vector<OrderStateDTO> value)
{
    m_Orders = value;
}
