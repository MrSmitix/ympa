...
std::string PriceCompetitivenessType::toJsonString(bool prettyJson /* = false */) const
{
	std::stringstream ss;
...
ptree PriceCompetitivenessType::toPropertyTree() const
{
	ptree pt;
	ptree tmp_node;
	return pt;
...
void PriceCompetitivenessType::fromPropertyTree(ptree const &pt)
{
	ptree tmp_node;
}

std::string PriceCompetitivenessType::toString() const {
    return boost::lexical_cast<std::string>(getEnumValue());
}

void PriceCompetitivenessType::fromString(const std::string& str) {
    setEnumValue(boost::lexical_cast<std::string>(str));
}

std::string PriceCompetitivenessType::getEnumValue() const {
    return m_PriceCompetitivenessTypeEnumValue;
}

void PriceCompetitivenessType::setEnumValue(const std::string& val) {
    static const std::array<std::string, 3> allowedValues = {
        "OPTIMAL", "AVERAGE", "LOW"
    };
    if (std::find(allowedValues.begin(), allowedValues.end(), val) != allowedValues.end()) {
        m_PriceCompetitivenessTypeEnumValue = val;
    } else {
