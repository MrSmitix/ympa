...
get_models_response_t *get_models_response_create(
    ...
    get_models_response_t *get_models_response_local_var = malloc(sizeof(get_models_response_t));
    if (!get_models_response_local_var) {
        return NULL;
    }
    get_models_response_local_var->models = models;
    get_models_response_local_var->currency = currency;
    get_models_response_local_var->region_id = region_id;

    return get_models_response_local_var;
}


void get_models_response_free(get_models_response_t *get_models_response) {
    if(NULL == get_models_response){
        return ;
    }
    listEntry_t *listEntry;
    if (get_models_response->models) {
        list_ForEach(listEntry, get_models_response->models) {
            model_dto_free(listEntry->data);
        }
        list_freeList(get_models_response->models);
        get_models_response->models = NULL;
    }
    if (get_models_response->currency) {
        currency_type_free(get_models_response->currency);
        get_models_response->currency = NULL;
    }
    free(get_models_response);
}

cJSON *get_models_response_convertToJSON(get_models_response_t *get_models_response) {
    cJSON *item = cJSON_CreateObject();

    // get_models_response->models
    if (!get_models_response->models) {
        goto fail;
    }
    ...
    if (get_models_response->models) {
    list_ForEach(modelsListEntry, get_models_response->models) {
    ...
    // get_models_response->currency
    if(get_models_response->currency != _api__get_models_response__NULL) {
    cJSON *currency_local_JSON = currency_type_convertToJSON(get_models_response->currency);
    ...
    // get_models_response->region_id
    if(get_models_response->region_id) {
    if(cJSON_AddNumberToObject(item, "regionId", get_models_response->region_id) == NULL) {
    goto fail; //Numeric
    }
    }
...
get_models_response_t *get_models_response_parseFromJSON(cJSON *get_models_responseJSON){
    ...
    get_models_response_t *get_models_response_local_var = NULL;

    // define the local list for get_models_response->models
    list_t *modelsList = NULL;

    // define the local variable for get_models_response->currency
    currency_type_t *currency_local_nonprim = NULL;

    // get_models_response->models
    cJSON *models = cJSON_GetObjectItemCaseSensitive(get_models_responseJSON, "models");
    if (!models) {
        goto end;
    }
    ...
    // get_models_response->currency
    cJSON *currency = cJSON_GetObjectItemCaseSensitive(get_models_responseJSON, "currency");
    if (currency) { 
    currency_local_nonprim = currency_type_parseFromJSON(currency); //custom
    }

    // get_models_response->region_id
    cJSON *region_id = cJSON_GetObjectItemCaseSensitive(get_models_responseJSON, "regionId");
    if (region_id) { 
    ...
    get_models_response_local_var = get_models_response_create (
        modelsList,
