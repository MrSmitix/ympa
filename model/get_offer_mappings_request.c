...
#include <string.h>
#include <stdio.h>
#include "get_offer_mappings_request.h"
...
get_offer_mappings_request_t *get_offer_mappings_request_create(
    ...
    get_offer_mappings_request_t *get_offer_mappings_request_local_var = malloc(sizeof(get_offer_mappings_request_t));
    if (!get_offer_mappings_request_local_var) {
        return NULL;
    }
    get_offer_mappings_request_local_var->offer_ids = offer_ids;
    get_offer_mappings_request_local_var->card_statuses = card_statuses;
    get_offer_mappings_request_local_var->category_ids = category_ids;
    get_offer_mappings_request_local_var->vendor_names = vendor_names;
    get_offer_mappings_request_local_var->tags = tags;
    get_offer_mappings_request_local_var->archived = archived;

    return get_offer_mappings_request_local_var;
}


void get_offer_mappings_request_free(get_offer_mappings_request_t *get_offer_mappings_request) {
    if(NULL == get_offer_mappings_request){
        return ;
    }
    listEntry_t *listEntry;
    if (get_offer_mappings_request->offer_ids) {
        list_ForEach(listEntry, get_offer_mappings_request->offer_ids) {
            free(listEntry->data);
        }
        list_freeList(get_offer_mappings_request->offer_ids);
        get_offer_mappings_request->offer_ids = NULL;
    }
    if (get_offer_mappings_request->card_statuses) {
        list_ForEach(listEntry, get_offer_mappings_request->card_statuses) {
            offer_card_status_type_free(listEntry->data);
        }
        list_freeList(get_offer_mappings_request->card_statuses);
        get_offer_mappings_request->card_statuses = NULL;
    }
    if (get_offer_mappings_request->category_ids) {
        list_ForEach(listEntry, get_offer_mappings_request->category_ids) {
            free(listEntry->data);
        }
        list_freeList(get_offer_mappings_request->category_ids);
        get_offer_mappings_request->category_ids = NULL;
    }
    if (get_offer_mappings_request->vendor_names) {
        list_ForEach(listEntry, get_offer_mappings_request->vendor_names) {
            free(listEntry->data);
        }
        list_freeList(get_offer_mappings_request->vendor_names);
        get_offer_mappings_request->vendor_names = NULL;
    }
    if (get_offer_mappings_request->tags) {
        list_ForEach(listEntry, get_offer_mappings_request->tags) {
            free(listEntry->data);
        }
        list_freeList(get_offer_mappings_request->tags);
        get_offer_mappings_request->tags = NULL;
    }
    free(get_offer_mappings_request);
}

cJSON *get_offer_mappings_request_convertToJSON(get_offer_mappings_request_t *get_offer_mappings_request) {
    cJSON *item = cJSON_CreateObject();

    // get_offer_mappings_request->offer_ids
    if(get_offer_mappings_request->offer_ids) {
    ...
    list_ForEach(offer_idsListEntry, get_offer_mappings_request->offer_ids) {
    ...
    // get_offer_mappings_request->card_statuses
    if(get_offer_mappings_request->card_statuses != _api__get_offer_mappings_request_CARDSTATUSES_NULL) {
    ...
    if (get_offer_mappings_request->card_statuses) {
    list_ForEach(card_statusesListEntry, get_offer_mappings_request->card_statuses) {
    cJSON *itemLocal = offer_card_status_type_convertToJSON((_api__get_offer_mappings_request__e)card_statusesListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
    ...
    // get_offer_mappings_request->category_ids
    if(get_offer_mappings_request->category_ids) {
    ...
    list_ForEach(category_idsListEntry, get_offer_mappings_request->category_ids) {
    ...
    // get_offer_mappings_request->vendor_names
    if(get_offer_mappings_request->vendor_names) {
    ...
    list_ForEach(vendor_namesListEntry, get_offer_mappings_request->vendor_names) {
    ...
    // get_offer_mappings_request->tags
    if(get_offer_mappings_request->tags) {
    ...
    list_ForEach(tagsListEntry, get_offer_mappings_request->tags) {
    ...
    // get_offer_mappings_request->archived
    if(get_offer_mappings_request->archived) {
    if(cJSON_AddBoolToObject(item, "archived", get_offer_mappings_request->archived) == NULL) {
    goto fail; //Bool
    }
    }
...
get_offer_mappings_request_t *get_offer_mappings_request_parseFromJSON(cJSON *get_offer_mappings_requestJSON){
    ...
    get_offer_mappings_request_t *get_offer_mappings_request_local_var = NULL;

    // define the local list for get_offer_mappings_request->offer_ids
    list_t *offer_idsList = NULL;

    // define the local list for get_offer_mappings_request->card_statuses
    list_t *card_statusesList = NULL;

    // define the local list for get_offer_mappings_request->category_ids
    list_t *category_idsList = NULL;

    // define the local list for get_offer_mappings_request->vendor_names
    list_t *vendor_namesList = NULL;

    // define the local list for get_offer_mappings_request->tags
    list_t *tagsList = NULL;

    // get_offer_mappings_request->offer_ids
    cJSON *offer_ids = cJSON_GetObjectItemCaseSensitive(get_offer_mappings_requestJSON, "offerIds");
    if (offer_ids) { 
    ...
    // get_offer_mappings_request->card_statuses
    cJSON *card_statuses = cJSON_GetObjectItemCaseSensitive(get_offer_mappings_requestJSON, "cardStatuses");
    if (card_statuses) { 
    ...
    cJSON_ArrayForEach(card_statuses_local_nonprimitive,card_statuses )
    {
        ...
        get_offer_mappings_request_offer_card_status_type_e card_statusesItem = offer_card_status_type_parseFromJSON(card_statuses_local_nonprimitive);
    ...
    // get_offer_mappings_request->category_ids
    cJSON *category_ids = cJSON_GetObjectItemCaseSensitive(get_offer_mappings_requestJSON, "categoryIds");
    if (category_ids) { 
    ...
    // get_offer_mappings_request->vendor_names
    cJSON *vendor_names = cJSON_GetObjectItemCaseSensitive(get_offer_mappings_requestJSON, "vendorNames");
    if (vendor_names) { 
    ...
    // get_offer_mappings_request->tags
    cJSON *tags = cJSON_GetObjectItemCaseSensitive(get_offer_mappings_requestJSON, "tags");
    if (tags) { 
    ...
    // get_offer_mappings_request->archived
    cJSON *archived = cJSON_GetObjectItemCaseSensitive(get_offer_mappings_requestJSON, "archived");
    if (archived) { 
    ...
    get_offer_mappings_request_local_var = get_offer_mappings_request_create (
