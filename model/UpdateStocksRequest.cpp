...
std::string UpdateStocksRequest::toJsonString(bool prettyJson /* = false */) const
{
	std::stringstream ss;
...
ptree UpdateStocksRequest::toPropertyTree() const
{
	ptree pt;
	ptree tmp_node;
...
void UpdateStocksRequest::fromPropertyTree(ptree const &pt)
{
	ptree tmp_node;
	// push all items of Skus into member
	if (pt.get_child_optional("skus")) {
        m_Skus = fromPt<std::set<UpdateStockDTO>>(pt.get_child("skus"));
	}
}

std::set<UpdateStockDTO> UpdateStocksRequest::getSkus() const
{
    return m_Skus;
}

void UpdateStocksRequest::setSkus(std::set<UpdateStockDTO> value)
{
    m_Skus = value;
}
