...
std::string GoodsFeedbackCommentAuthorType::toJsonString(bool prettyJson /* = false */) const
{
	std::stringstream ss;
...
ptree GoodsFeedbackCommentAuthorType::toPropertyTree() const
{
	ptree pt;
	ptree tmp_node;
	return pt;
...
void GoodsFeedbackCommentAuthorType::fromPropertyTree(ptree const &pt)
{
	ptree tmp_node;
}

std::string GoodsFeedbackCommentAuthorType::toString() const {
    return boost::lexical_cast<std::string>(getEnumValue());
}

void GoodsFeedbackCommentAuthorType::fromString(const std::string& str) {
    setEnumValue(boost::lexical_cast<std::string>(str));
}

std::string GoodsFeedbackCommentAuthorType::getEnumValue() const {
    return m_GoodsFeedbackCommentAuthorTypeEnumValue;
}

void GoodsFeedbackCommentAuthorType::setEnumValue(const std::string& val) {
    static const std::array<std::string, 2> allowedValues = {
        "USER", "BUSINESS"
    };
    if (std::find(allowedValues.begin(), allowedValues.end(), val) != allowedValues.end()) {
        m_GoodsFeedbackCommentAuthorTypeEnumValue = val;
    } else {
