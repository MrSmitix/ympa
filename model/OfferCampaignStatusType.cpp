...
std::string OfferCampaignStatusType::toJsonString(bool prettyJson /* = false */) const
{
	std::stringstream ss;
...
ptree OfferCampaignStatusType::toPropertyTree() const
{
	ptree pt;
	ptree tmp_node;
	return pt;
...
void OfferCampaignStatusType::fromPropertyTree(ptree const &pt)
{
	ptree tmp_node;
}

std::string OfferCampaignStatusType::toString() const {
    return boost::lexical_cast<std::string>(getEnumValue());
}

void OfferCampaignStatusType::fromString(const std::string& str) {
    setEnumValue(boost::lexical_cast<std::string>(str));
}

std::string OfferCampaignStatusType::getEnumValue() const {
    return m_OfferCampaignStatusTypeEnumValue;
}

void OfferCampaignStatusType::setEnumValue(const std::string& val) {
    ...
    if (std::find(allowedValues.begin(), allowedValues.end(), val) != allowedValues.end()) {
        m_OfferCampaignStatusTypeEnumValue = val;
    } else {
