...
std::string SkipGoodsFeedbackReactionRequest::toJsonString(bool prettyJson /* = false */) const
{
	std::stringstream ss;
...
ptree SkipGoodsFeedbackReactionRequest::toPropertyTree() const
{
	ptree pt;
	ptree tmp_node;
...
void SkipGoodsFeedbackReactionRequest::fromPropertyTree(ptree const &pt)
{
	ptree tmp_node;
	// push all items of FeedbackIds into member
	if (pt.get_child_optional("feedbackIds")) {
        m_FeedbackIds = fromPt<std::set<int64_t>>(pt.get_child("feedbackIds"));
	}
}
