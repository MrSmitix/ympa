...
std::string OutletType::toJsonString(bool prettyJson /* = false */) const
{
	std::stringstream ss;
...
ptree OutletType::toPropertyTree() const
{
	ptree pt;
	ptree tmp_node;
	return pt;
...
void OutletType::fromPropertyTree(ptree const &pt)
{
	ptree tmp_node;
}

std::string OutletType::toString() const {
    return boost::lexical_cast<std::string>(getEnumValue());
}

void OutletType::fromString(const std::string& str) {
    setEnumValue(boost::lexical_cast<std::string>(str));
}

std::string OutletType::getEnumValue() const {
    return m_OutletTypeEnumValue;
}

void OutletType::setEnumValue(const std::string& val) {
    static const std::array<std::string, 4> allowedValues = {
        "DEPOT", "MIXED", "RETAIL", "NOT_DEFINED"
    };
    if (std::find(allowedValues.begin(), allowedValues.end(), val) != allowedValues.end()) {
        m_OutletTypeEnumValue = val;
    } else {
