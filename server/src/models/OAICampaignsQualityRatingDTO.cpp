...
void OAICampaignsQualityRatingDTO::fromJsonObject(QJsonObject json) {
    ...
    m_campaign_ratings_isValid = ::OpenAPI::fromJsonValue(campaign_ratings, json[QString("campaignRatings")]);
...
QJsonObject OAICampaignsQualityRatingDTO::asJsonObject() const {
    QJsonObject obj;
    if (campaign_ratings.size() > 0) {
        obj.insert(QString("campaignRatings"), ::OpenAPI::toJsonValue(campaign_ratings));
    }
    return obj;
}
...
void OAICampaignsQualityRatingDTO::setCampaignRatings(const QList<OAICampaignQualityRatingDTO> &campaign_ratings) {
    this->campaign_ratings = campaign_ratings;
    this->m_campaign_ratings_isSet = true;
}
