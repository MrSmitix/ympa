...
void OAIOutletWorkingScheduleDTO::fromJsonObject(QJsonObject json) {
    ...
    m_work_in_holiday_isValid = ::OpenAPI::fromJsonValue(work_in_holiday, json[QString("workInHoliday")]);
    m_work_in_holiday_isSet = !json[QString("workInHoliday")].isNull() && m_work_in_holiday_isValid;

    m_schedule_items_isValid = ::OpenAPI::fromJsonValue(schedule_items, json[QString("scheduleItems")]);
...
QJsonObject OAIOutletWorkingScheduleDTO::asJsonObject() const {
    ...
    if (m_work_in_holiday_isSet) {
        obj.insert(QString("workInHoliday"), ::OpenAPI::toJsonValue(work_in_holiday));
    }
    if (schedule_items.size() > 0) {
        obj.insert(QString("scheduleItems"), ::OpenAPI::toJsonValue(schedule_items));
    }
    return obj;
}

bool OAIOutletWorkingScheduleDTO::isWorkInHoliday() const {
    return work_in_holiday;
}
void OAIOutletWorkingScheduleDTO::setWorkInHoliday(const bool &work_in_holiday) {
    this->work_in_holiday = work_in_holiday;
    this->m_work_in_holiday_isSet = true;
}
...
void OAIOutletWorkingScheduleDTO::setScheduleItems(const QList<OAIOutletWorkingScheduleItemDTO> &schedule_items) {
    ...
    this->m_schedule_items_isSet = true;
}
