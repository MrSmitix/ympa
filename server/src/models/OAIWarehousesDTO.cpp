...
void OAIWarehousesDTO::fromJsonObject(QJsonObject json) {
    ...
    m_warehouse_groups_isValid = ::OpenAPI::fromJsonValue(warehouse_groups, json[QString("warehouseGroups")]);
...
QJsonObject OAIWarehousesDTO::asJsonObject() const {
    ...
    if (warehouse_groups.size() > 0) {
        obj.insert(QString("warehouseGroups"), ::OpenAPI::toJsonValue(warehouse_groups));
    }
    return obj;
}
...
void OAIWarehousesDTO::setWarehouseGroups(const QList<OAIWarehouseGroupDTO> &warehouse_groups) {
    this->warehouse_groups = warehouse_groups;
    this->m_warehouse_groups_isSet = true;
}
