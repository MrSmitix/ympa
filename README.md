# Go API client for ympa_go_client
...
```go
import ympa_go_client "github.com/yandex-market/yandex-market-partner-api"
```
...
For using other server than the one defined on index 0 set context value `ympa_go_client.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), ympa_go_client.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `ympa_go_client.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), ympa_go_client.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `ympa_go_client.ContextOperationServerIndices` and `ympa_go_client.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), ympa_go_client.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), ympa_go_client.ContextOperationServerVariables, map[string]map[string]string{
...
```go
auth := context.WithValue(context.Background(), ympa_go_client.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, ympa_go_client.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
