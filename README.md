## OpenAPI Auto-Generated [http-client](https://www.stackage.org/lts-10.0/package/http-client-0.5.7.1) Bindings to `Партнерский API Маркета`

The library in `lib` provides auto-generated-from-OpenAPI [http-client](https://www.stackage.org/lts-10.0/package/http-client-0.5.7.1) bindings to the Партнерский API Маркета API.
...
| baseModule                      | Set the base module namespace                                                                                                 |          | ПартнерскийAPIМаркета                      |
| cabalPackage                    | Set the cabal package name, which consists of one or more alphanumeric words separated by hyphens                             |          | партнерский-api-маркета                    |
...
| configType                      | Set the name of the type used for configuration                                                                               |          | ПартнерскийAPIМаркетаConfig                      |
...
| requestType                     | Set the name of the type used to generate requests                                                                            |          | ПартнерскийAPIМаркетаRequest                     |
...
| MODULE              | NOTES                                               |
| ------------------- | --------------------------------------------------- |
| ПартнерскийAPIМаркета.Client    | use the "dispatch" functions to send requests       |
| ПартнерскийAPIМаркета.Core      | core functions, config and request types            |
| ПартнерскийAPIМаркета.API       | construct api requests                              |
| ПартнерскийAPIМаркета.Model     | describes api models                                |
| ПартнерскийAPIМаркета.MimeTypes | encoding/decoding MIME types (content-types/accept) |
| ПартнерскийAPIМаркета.ModelLens | lenses for model fields                             |
| ПартнерскийAPIМаркета.Logging   | logging functions and utils                         |
