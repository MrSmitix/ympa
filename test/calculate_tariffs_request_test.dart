//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for CalculateTariffsRequest
void main() {
  // final instance = CalculateTariffsRequest();

  group('test CalculateTariffsRequest', () {
    // CalculateTariffsParametersDTO parameters
    test('to test the property `parameters`', () async {
      // TODO
    });

    // Товары, для которых нужно рассчитать стоимость услуг.
    // List<CalculateTariffsOfferDTO> offers (default value: const [])
    test('to test the property `offers`', () async {
      // TODO
    });


  });

}
