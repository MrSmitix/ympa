...
/* Список SKU.  {% note warning \"Такой список возвращается только целиком\" %}  Если вы запрашиваете информацию по конкретным SKU, не заполняйте:  * `page_token` * `limit`  {% endnote %}     */
  ...
  def asData : GetPricesByOfferIdsRequestData = {
    GetPricesByOfferIdsRequestData(
            offerIds = offerIds
    )
  }
...
object GetPricesByOfferIdsRequest{
    ...
    given RW[GetPricesByOfferIdsRequest] = GetPricesByOfferIdsRequestData.readWriter.bimap[GetPricesByOfferIdsRequest](_.asData, _.asModel)
