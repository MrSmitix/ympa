...
/** BaseOfferDTOData a data transfer object, primarily for simple json serialisation.
  * It has no validation - there may be nulls, values out of range, etc
  */
case class BaseOfferDTOData(
/* Ваш SKU — идентификатор товара в вашей системе.  Разрешена любая последовательность длиной до 255 знаков.  Правила использования SKU:  * У каждого товара SKU должен быть свой.  * SKU товара нельзя менять — можно только удалить товар и добавить заново с новым SKU.  * Уже заданный SKU нельзя освободить и использовать заново для другого товара. Каждый товар должен получать новый идентификатор, до того никогда не использовавшийся в вашем каталоге.  [Что такое SKU и как его назначать](https://yandex.ru/support/marketplace/assortment/add/index.html#fields)  */
    ...
    val errors = scala.collection.mutable.ListBuffer[ValidationError]()
        ...
        if (errors.isEmpty || !failFast) {
           ...
           if offerId == null || !regex.r.matches(offerId) then
              errors += ValidationError(path :+ BaseOfferDTO.Fields.offerId, s"value '$offerId' doesn't match pattern $regex")
        }
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if offerId == null then 0 else offerId.length
            if (len < 1) {
               errors += ValidationError(path :+ BaseOfferDTO.Fields.offerId, s"length $len is shorter than the min length 1")
            }
        }
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if offerId == null then 0 else offerId.length
            if (len < 255) {
               errors += ValidationError(path :+ BaseOfferDTO.Fields.offerId, s"length $len is longer than the max length 255")
            }
        }
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if name == null then 0 else name.length
            if (len < 256) {
               errors += ValidationError(path :+ BaseOfferDTO.Fields.name, s"length $len is longer than the max length 256")
            }
        }
        ...
        if (errors.isEmpty || !failFast) {
          val len = if videos == null then 0 else videos.size
            if (len > 6) {
               errors += ValidationError(path :+ BaseOfferDTO.Fields.videos, s"videos has $len, which is greater than the max items 6")
            }
        }
        ...
        if (errors.isEmpty || !failFast) {
          val len = if manuals == null then 0 else manuals.size
            if (len > 6) {
               errors += ValidationError(path :+ BaseOfferDTO.Fields.manuals, s"manuals has $len, which is greater than the max items 6")
            }
        }
                ...
                manuals.zipWithIndex.foreach {
                    case (value, i) if errors.isEmpty || !failFast =>
                      errors ++= value.validationErrors(
                        path :+ BaseOfferDTO.Fields.manuals :+ Field(i.toString),
                        failFast)
                    case (value, i) =>
                }
            }
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if description == null then 0 else description.length
            if (len < 6000) {
               errors += ValidationError(path :+ BaseOfferDTO.Fields.description, s"length $len is longer than the max length 6000")
            }
        }
        ...
        if (errors.isEmpty || !failFast) {
            if weightDimensions != null then errors ++= weightDimensions.validationErrors(path :+ BaseOfferDTO.Fields.weightDimensions, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if shelfLife != null then errors ++= shelfLife.validationErrors(path :+ BaseOfferDTO.Fields.shelfLife, failFast)
        }
        ...
        // validating lifeTime
        if (errors.isEmpty || !failFast) {
            if lifeTime != null then errors ++= lifeTime.validationErrors(path :+ BaseOfferDTO.Fields.lifeTime, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if guaranteePeriod != null then errors ++= guaranteePeriod.validationErrors(path :+ BaseOfferDTO.Fields.guaranteePeriod, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if condition != null then errors ++= condition.validationErrors(path :+ BaseOfferDTO.Fields.condition, failFast)
        }
        ...
        // validating age
        if (errors.isEmpty || !failFast) {
            if age != null then errors ++= age.validationErrors(path :+ BaseOfferDTO.Fields.age, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if (params != null) {
                params.zipWithIndex.foreach {
                    case (value, i) if errors.isEmpty || !failFast =>
                      errors ++= value.validationErrors(
                        path :+ BaseOfferDTO.Fields.params :+ Field(i.toString),
                        failFast)
                    case (value, i) =>
                }
            }
  ...
  def validated(failFast : Boolean = false) : scala.util.Try[BaseOfferDTO] = {
...
object BaseOfferDTOData {

  given readWriter : RW[BaseOfferDTOData] = macroRW

  def fromJsonString(jason : String) : BaseOfferDTOData = try {
        read[BaseOfferDTOData](jason)
     } catch {
          case NonFatal(e) => sys.error(s"Error parsing json '$jason': $e")
     }

  def manyFromJsonString(jason : String) : Seq[BaseOfferDTOData] = try {
        read[List[BaseOfferDTOData]](jason)
    } catch {
        case NonFatal(e) => sys.error(s"Error parsing json '$jason' as list: $e")
    }

  def manyFromJsonStringValidated(jason : String, failFast : Boolean = false) : Try[Seq[BaseOfferDTO]] = {
      Try(manyFromJsonString(jason)).flatMap { list =>
        list.zipWithIndex.foldLeft(Try(Vector[BaseOfferDTO]())) {
  ...
  def mapFromJsonString(jason : String) : Map[String, BaseOfferDTOData] = try {
        read[Map[String, BaseOfferDTOData]](jason)
    } catch {
        case NonFatal(e) => sys.error(s"Error parsing json '$jason' as map: $e")
    }


  def mapFromJsonStringValidated(jason : String, failFast : Boolean = false) : Try[Map[String, BaseOfferDTO]] = {
     Try(mapFromJsonString(jason)).flatMap { map =>
       map.foldLeft(Try(Map[String, BaseOfferDTO]())) {
