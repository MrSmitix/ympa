...
/** GetOfferDTOData a data transfer object, primarily for simple json serialisation.
  * It has no validation - there may be nulls, values out of range, etc
  */
case class GetOfferDTOData(
/* Ваш SKU — идентификатор товара в вашей системе.  Разрешена любая последовательность длиной до 255 знаков.  Правила использования SKU:  * У каждого товара SKU должен быть свой.  * SKU товара нельзя менять — можно только удалить товар и добавить заново с новым SKU.  * Уже заданный SKU нельзя освободить и использовать заново для другого товара. Каждый товар должен получать новый идентификатор, до того никогда не использовавшийся в вашем каталоге.  [Что такое SKU и как его назначать](https://yandex.ru/support/marketplace/assortment/add/index.html#fields)  */
  ...
  params: Seq[OfferParamDTOData] = Nil ,
    ...
    purchasePrice: GetPriceDTOData = null ,

    additionalExpenses: GetPriceDTOData = null ,

    cofinancePrice: GetPriceDTOData = null ,

    cardStatus: OfferCardStatusType = null ,

  /* Список магазинов, в которых размещен товар.  */
  campaigns: Seq[OfferCampaignStatusDTOData] = Nil ,

  /* Информация о том, какие для товара доступны модели размещения.  */
  sellingPrograms: Seq[OfferSellingProgramDTOData] = Nil ,
    ...
    val errors = scala.collection.mutable.ListBuffer[ValidationError]()
        ...
        if (errors.isEmpty || !failFast) {
           ...
           if offerId == null || !regex.r.matches(offerId) then
              errors += ValidationError(path :+ GetOfferDTO.Fields.offerId, s"value '$offerId' doesn't match pattern $regex")
        }
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if offerId == null then 0 else offerId.length
            if (len < 1) {
               errors += ValidationError(path :+ GetOfferDTO.Fields.offerId, s"length $len is shorter than the min length 1")
            }
        }
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if offerId == null then 0 else offerId.length
            if (len < 255) {
               errors += ValidationError(path :+ GetOfferDTO.Fields.offerId, s"length $len is longer than the max length 255")
            }
        }
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if name == null then 0 else name.length
            if (len < 256) {
               errors += ValidationError(path :+ GetOfferDTO.Fields.name, s"length $len is longer than the max length 256")
            }
        }
        ...
        if (errors.isEmpty || !failFast) {
          val len = if videos == null then 0 else videos.size
            if (len > 6) {
               errors += ValidationError(path :+ GetOfferDTO.Fields.videos, s"videos has $len, which is greater than the max items 6")
            }
        }
        ...
        if (errors.isEmpty || !failFast) {
          val len = if manuals == null then 0 else manuals.size
            if (len > 6) {
               errors += ValidationError(path :+ GetOfferDTO.Fields.manuals, s"manuals has $len, which is greater than the max items 6")
            }
        }
                ...
                manuals.zipWithIndex.foreach {
                    case (value, i) if errors.isEmpty || !failFast =>
                      errors ++= value.validationErrors(
                        path :+ GetOfferDTO.Fields.manuals :+ Field(i.toString),
                        failFast)
                    case (value, i) =>
                }
            }
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if description == null then 0 else description.length
            if (len < 6000) {
               errors += ValidationError(path :+ GetOfferDTO.Fields.description, s"length $len is longer than the max length 6000")
            }
        }
        ...
        if (errors.isEmpty || !failFast) {
            if weightDimensions != null then errors ++= weightDimensions.validationErrors(path :+ GetOfferDTO.Fields.weightDimensions, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if shelfLife != null then errors ++= shelfLife.validationErrors(path :+ GetOfferDTO.Fields.shelfLife, failFast)
        }
        ...
        // validating lifeTime
        if (errors.isEmpty || !failFast) {
            if lifeTime != null then errors ++= lifeTime.validationErrors(path :+ GetOfferDTO.Fields.lifeTime, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if guaranteePeriod != null then errors ++= guaranteePeriod.validationErrors(path :+ GetOfferDTO.Fields.guaranteePeriod, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if condition != null then errors ++= condition.validationErrors(path :+ GetOfferDTO.Fields.condition, failFast)
        }
        ...
        // validating age
        if (errors.isEmpty || !failFast) {
            if age != null then errors ++= age.validationErrors(path :+ GetOfferDTO.Fields.age, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if (params != null) {
                params.zipWithIndex.foreach {
                    case (value, i) if errors.isEmpty || !failFast =>
                      errors ++= value.validationErrors(
                        path :+ GetOfferDTO.Fields.params :+ Field(i.toString),
                        failFast)
                    case (value, i) =>
                }
            }
        ...
        if (errors.isEmpty || !failFast) {
            if basicPrice != null then errors ++= basicPrice.validationErrors(path :+ GetOfferDTO.Fields.basicPrice, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if purchasePrice != null then errors ++= purchasePrice.validationErrors(path :+ GetOfferDTO.Fields.purchasePrice, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if additionalExpenses != null then errors ++= additionalExpenses.validationErrors(path :+ GetOfferDTO.Fields.additionalExpenses, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if cofinancePrice != null then errors ++= cofinancePrice.validationErrors(path :+ GetOfferDTO.Fields.cofinancePrice, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if (campaigns != null) {
                campaigns.zipWithIndex.foreach {
                    case (value, i) if errors.isEmpty || !failFast =>
                      errors ++= value.validationErrors(
                        path :+ GetOfferDTO.Fields.campaigns :+ Field(i.toString),
                        failFast)
                    case (value, i) =>
                }
            }
        ...
        if (errors.isEmpty || !failFast) {
            if (sellingPrograms != null) {
                sellingPrograms.zipWithIndex.foreach {
                    case (value, i) if errors.isEmpty || !failFast =>
                      errors ++= value.validationErrors(
                        path :+ GetOfferDTO.Fields.sellingPrograms :+ Field(i.toString),
                        failFast)
                    case (value, i) =>
                }
            }
  ...
  def validated(failFast : Boolean = false) : scala.util.Try[GetOfferDTO] = {
...
object GetOfferDTOData {

  given readWriter : RW[GetOfferDTOData] = macroRW

  def fromJsonString(jason : String) : GetOfferDTOData = try {
        read[GetOfferDTOData](jason)
     } catch {
          case NonFatal(e) => sys.error(s"Error parsing json '$jason': $e")
     }

  def manyFromJsonString(jason : String) : Seq[GetOfferDTOData] = try {
        read[List[GetOfferDTOData]](jason)
    } catch {
        case NonFatal(e) => sys.error(s"Error parsing json '$jason' as list: $e")
    }

  def manyFromJsonStringValidated(jason : String, failFast : Boolean = false) : Try[Seq[GetOfferDTO]] = {
      Try(manyFromJsonString(jason)).flatMap { list =>
        list.zipWithIndex.foldLeft(Try(Vector[GetOfferDTO]())) {
  ...
  def mapFromJsonString(jason : String) : Map[String, GetOfferDTOData] = try {
        read[Map[String, GetOfferDTOData]](jason)
    } catch {
        case NonFatal(e) => sys.error(s"Error parsing json '$jason' as map: $e")
    }


  def mapFromJsonStringValidated(jason : String, failFast : Boolean = false) : Try[Map[String, GetOfferDTO]] = {
     Try(mapFromJsonString(jason)).flatMap { map =>
       map.foldLeft(Try(Map[String, GetOfferDTO]())) {
