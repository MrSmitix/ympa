...
/** UpdateMappingsOfferDTOData a data transfer object, primarily for simple json serialisation.
  * It has no validation - there may be nulls, values out of range, etc
  */
case class UpdateMappingsOfferDTOData(
/* Составляйте название по схеме: тип + бренд или производитель + модель + особенности, если есть (например, цвет, размер или вес) и количество в упаковке.  Не включайте в название условия продажи (например, «скидка», «бесплатная доставка» и т. д.), эмоциональные характеристики («хит», «супер» и т. д.). Не пишите слова большими буквами — кроме устоявшихся названий брендов и моделей.  Оптимальная длина — 50–60 символов, максимальная — 256.  [Рекомендации и правила](https://yandex.ru/support/marketplace/assortment/fields/title.html)  */
    ...
    val errors = scala.collection.mutable.ListBuffer[ValidationError]()
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if name == null then 0 else name.length
            if (len < 256) {
               errors += ValidationError(path :+ UpdateMappingsOfferDTO.Fields.name, s"length $len is longer than the max length 256")
            }
        }
        ...
        if (errors.isEmpty || !failFast) {
           ...
           if shopSku == null || !regex.r.matches(shopSku) then
              errors += ValidationError(path :+ UpdateMappingsOfferDTO.Fields.shopSku, s"value '$shopSku' doesn't match pattern $regex")
        }
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if shopSku == null then 0 else shopSku.length
            if (len < 1) {
               errors += ValidationError(path :+ UpdateMappingsOfferDTO.Fields.shopSku, s"length $len is shorter than the min length 1")
            }
        }
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if shopSku == null then 0 else shopSku.length
            if (len < 255) {
               errors += ValidationError(path :+ UpdateMappingsOfferDTO.Fields.shopSku, s"length $len is longer than the max length 255")
            }
        }
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if description == null then 0 else description.length
            if (len < 6000) {
               errors += ValidationError(path :+ UpdateMappingsOfferDTO.Fields.description, s"length $len is longer than the max length 6000")
            }
        }
        ...
        if (errors.isEmpty || !failFast) {
           ...
           if id == null || !regex.r.matches(id) then
              errors += ValidationError(path :+ UpdateMappingsOfferDTO.Fields.id, s"value '$id' doesn't match pattern $regex")
        }
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if id == null then 0 else id.length
            if (len < 1) {
               errors += ValidationError(path :+ UpdateMappingsOfferDTO.Fields.id, s"length $len is shorter than the min length 1")
            }
        }
        ...
        if (errors.isEmpty || !failFast)  {
          val len = if id == null then 0 else id.length
            if (len < 255) {
               errors += ValidationError(path :+ UpdateMappingsOfferDTO.Fields.id, s"length $len is longer than the max length 255")
            }
        }
        ...
        if (errors.isEmpty || !failFast) {
            if weightDimensions != null then errors ++= weightDimensions.validationErrors(path :+ UpdateMappingsOfferDTO.Fields.weightDimensions, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if processingState != null then errors ++= processingState.validationErrors(path :+ UpdateMappingsOfferDTO.Fields.processingState, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if shelfLife != null then errors ++= shelfLife.validationErrors(path :+ UpdateMappingsOfferDTO.Fields.shelfLife, failFast)
        }
        ...
        // validating lifeTime
        if (errors.isEmpty || !failFast) {
            if lifeTime != null then errors ++= lifeTime.validationErrors(path :+ UpdateMappingsOfferDTO.Fields.lifeTime, failFast)
        }
        ...
        if (errors.isEmpty || !failFast) {
            if guaranteePeriod != null then errors ++= guaranteePeriod.validationErrors(path :+ UpdateMappingsOfferDTO.Fields.guaranteePeriod, failFast)
        }
  ...
  def validated(failFast : Boolean = false) : scala.util.Try[UpdateMappingsOfferDTO] = {
  ...
  def asModel : UpdateMappingsOfferDTO = {
...
object UpdateMappingsOfferDTOData {

  given readWriter : RW[UpdateMappingsOfferDTOData] = macroRW

  def fromJsonString(jason : String) : UpdateMappingsOfferDTOData = try {
        read[UpdateMappingsOfferDTOData](jason)
     } catch {
          case NonFatal(e) => sys.error(s"Error parsing json '$jason': $e")
     }

  def manyFromJsonString(jason : String) : Seq[UpdateMappingsOfferDTOData] = try {
        read[List[UpdateMappingsOfferDTOData]](jason)
    } catch {
        case NonFatal(e) => sys.error(s"Error parsing json '$jason' as list: $e")
    }

  def manyFromJsonStringValidated(jason : String, failFast : Boolean = false) : Try[Seq[UpdateMappingsOfferDTO]] = {
      Try(manyFromJsonString(jason)).flatMap { list =>
        list.zipWithIndex.foldLeft(Try(Vector[UpdateMappingsOfferDTO]())) {
  ...
  def mapFromJsonString(jason : String) : Map[String, UpdateMappingsOfferDTOData] = try {
        read[Map[String, UpdateMappingsOfferDTOData]](jason)
    } catch {
        case NonFatal(e) => sys.error(s"Error parsing json '$jason' as map: $e")
    }


  def mapFromJsonStringValidated(jason : String, failFast : Boolean = false) : Try[Map[String, UpdateMappingsOfferDTO]] = {
     Try(mapFromJsonString(jason)).flatMap { map =>
       map.foldLeft(Try(Map[String, UpdateMappingsOfferDTO]())) {
