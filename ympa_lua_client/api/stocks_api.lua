...
local function new_stocks_api(authority, basePath, schemes)
	local schemes_map = {}
...
function stocks_api:get_stocks(campaign_id, page_token, limit, get_warehouse_stocks_request)
...
function stocks_api:update_stocks(campaign_id, update_stocks_request)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
