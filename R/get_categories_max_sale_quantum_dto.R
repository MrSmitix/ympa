#' Create a new GetCategoriesMaxSaleQuantumDTO
#'
#' @description
#' Категории и лимит на установку кванта и минимального количества товаров.
#'
#' @docType class
#' @title GetCategoriesMaxSaleQuantumDTO
#' @description GetCategoriesMaxSaleQuantumDTO Class
...
#' @export
GetCategoriesMaxSaleQuantumDTO <- R6::R6Class(
  "GetCategoriesMaxSaleQuantumDTO",
  public = list(
    `results` = NULL,
    `errors` = NULL,
    #' Initialize a new GetCategoriesMaxSaleQuantumDTO class.
    #'
    #' @description
    #' Initialize a new GetCategoriesMaxSaleQuantumDTO class.
    #'
    ...
    #' @export
    initialize = function(`results`, `errors` = NULL, ...) {
      if (!missing(`results`)) {
    ...
    #' To JSON String
    #'
    #' @return GetCategoriesMaxSaleQuantumDTO in JSON format
    #' @export
    toJSON = function() {
      GetCategoriesMaxSaleQuantumDTOObject <- list()
      if (!is.null(self$`results`)) {
        GetCategoriesMaxSaleQuantumDTOObject[["results"]] <-
          lapply(self$`results`, function(x) x$toJSON())
      }
      if (!is.null(self$`errors`)) {
        GetCategoriesMaxSaleQuantumDTOObject[["errors"]] <-
          lapply(self$`errors`, function(x) x$toJSON())
      }
      GetCategoriesMaxSaleQuantumDTOObject
    },
    #' Deserialize JSON string into an instance of GetCategoriesMaxSaleQuantumDTO
    #'
    #' @description
    #' Deserialize JSON string into an instance of GetCategoriesMaxSaleQuantumDTO
    #'
    #' @param input_json the JSON input
    #' @return the instance of GetCategoriesMaxSaleQuantumDTO
    #' @export
...
',
    ...
    #' Validate JSON input with respect to GetCategoriesMaxSaleQuantumDTO
    #'
    #' @description
    #' Validate JSON input with respect to GetCategoriesMaxSaleQuantumDTO and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GetCategoriesMaxSaleQuantumDTO: the required field `results` is missing."))
      }
    },
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of GetCategoriesMaxSaleQuantumDTO
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# GetCategoriesMaxSaleQuantumDTO$unlock()
#
## Below is an example to define the print function
# GetCategoriesMaxSaleQuantumDTO$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# GetCategoriesMaxSaleQuantumDTO$lock()
