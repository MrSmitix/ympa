# coding: utf-8

from __future__ import absolute_import
from datetime import date, datetime  # noqa: F401

from typing import List, Dict  # noqa: F401

from app.ympa_python_blueplanet_server.models.base_model import Model
from app.ympa_python_blueplanet_server.models.api_response_status_type import ApiResponseStatusType  # noqa: F401,E501
from app.ympa_python_blueplanet_server.models.orders_shipment_info_dto import OrdersShipmentInfoDTO  # noqa: F401,E501
from ympa_python_blueplanet_server import util


class GetShipmentOrdersInfoResponse(Model):
    """NOTE: This class is auto generated by the swagger code generator program.

    Do not edit the class manually.
    """

    def __init__(self, status: ApiResponseStatusType=None, result: OrdersShipmentInfoDTO=None):  # noqa: E501
        """GetShipmentOrdersInfoResponse - a model defined in Swagger

        :param status: The status of this GetShipmentOrdersInfoResponse.  # noqa: E501
        :type status: ApiResponseStatusType
        :param result: The result of this GetShipmentOrdersInfoResponse.  # noqa: E501
        :type result: OrdersShipmentInfoDTO
        """
        self.swagger_types = {
            'status': ApiResponseStatusType,
            'result': OrdersShipmentInfoDTO
        }

        self.attribute_map = {
            'status': 'status',
            'result': 'result'
        }

        self._status = status
        self._result = result

    @classmethod
    def from_dict(cls, dikt) -> 'GetShipmentOrdersInfoResponse':
        """Returns the dict as a model

        :param dikt: A dict.
        :type: dict
        :return: The GetShipmentOrdersInfoResponse of this GetShipmentOrdersInfoResponse.  # noqa: E501
        :rtype: GetShipmentOrdersInfoResponse
        """
        return util.deserialize_model(dikt, cls)

    @property
    def status(self) -> ApiResponseStatusType:
        """Gets the status of this GetShipmentOrdersInfoResponse.


        :return: The status of this GetShipmentOrdersInfoResponse.
        :rtype: ApiResponseStatusType
        """
        return self._status

    @status.setter
    def status(self, status: ApiResponseStatusType):
        """Sets the status of this GetShipmentOrdersInfoResponse.


        :param status: The status of this GetShipmentOrdersInfoResponse.
        :type status: ApiResponseStatusType
        """

        self._status = status

    @property
    def result(self) -> OrdersShipmentInfoDTO:
        """Gets the result of this GetShipmentOrdersInfoResponse.


        :return: The result of this GetShipmentOrdersInfoResponse.
        :rtype: OrdersShipmentInfoDTO
        """
        return self._result

    @result.setter
    def result(self, result: OrdersShipmentInfoDTO):
        """Sets the result of this GetShipmentOrdersInfoResponse.


        :param result: The result of this GetShipmentOrdersInfoResponse.
        :type result: OrdersShipmentInfoDTO
        """

        self._result = result
