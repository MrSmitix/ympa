package model

import play.api.libs.json._

/**
  * Represents the Swagger definition for CalculateTariffsRequest.
  * @param offers Товары, для которых нужно рассчитать стоимость услуг.
  */
@javax.annotation.Generated(value = Array("org.openapitools.codegen.languages.ScalaPlayFrameworkServerCodegen"), date = "2024-09-09T22:33:09.055496365Z[Etc/UTC]", comments = "Generator version: 7.8.0")
case class CalculateTariffsRequest(
  parameters: CalculateTariffsParametersDTO,
  offers: List[CalculateTariffsOfferDTO]
)

object CalculateTariffsRequest {
  implicit lazy val calculateTariffsRequestJsonFormat: Format[CalculateTariffsRequest] = Json.format[CalculateTariffsRequest]
}

