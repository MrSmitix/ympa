<?php
/**
 * OfferSellingProgramDTO
 *
 * PHP version 8.1.1
 *
 * @category Class
 * @package  OpenAPIServer\Model
 * @author   OpenAPI Generator team
 * @link     https://github.com/openapitools/openapi-generator
 */

/**
 * Партнерский API Маркета
 *
 * API Яндекс Маркета помогает продавцам автоматизировать и упростить работу с маркетплейсом.  В числе возможностей интеграции:  * управление каталогом товаров и витриной,  * обработка заказов,  * изменение настроек магазина,  * получение отчетов.
 *
 * The version of the OpenAPI document: LATEST
 * 
 * Generated by: https://github.com/openapitools/openapi-generator.git
 *
 */


namespace OpenAPIServer\Model;

/**
 * Class representing the OfferSellingProgramDTO model.
 *
 * Информация о том, по каким моделям можно продавать товар, а по каким нельзя.
 *
 * @package OpenAPIServer\Model
 * @author  OpenAPI Generator team
 */

class OfferSellingProgramDTO  implements \JsonSerializable
{
        /**
     * @var SellingProgramType
     * @SerializedName("sellingProgram")
     * @Assert\NotNull()
     * @Assert\Valid()
        * @Accessor(getter="getSerializedSellingProgram")
        * @Type("string")
     */
    public SellingProgramType $sellingProgram;

    /**
     * @var OfferSellingProgramStatusType
     * @SerializedName("status")
     * @Assert\NotNull()
     * @Assert\Valid()
        * @Accessor(getter="getSerializedStatus")
        * @Type("string")
     */
    public OfferSellingProgramStatusType $status;

    /**
     * Constructor
     *
     * @param SellingProgramType $sellingProgram
     * @param OfferSellingProgramStatusType $status
     */
    public function __construct(SellingProgramType $sellingProgram, OfferSellingProgramStatusType $status)
    {
        $this->sellingProgram = $sellingProgram;
        $this->status = $status;
    }

    public static function fromArray(array $data): self
    {
        return new self(
            isset($data['sellingProgram']) ? SellingProgramType::tryFrom($data['sellingProgram']) : null, 
            isset($data['status']) ? OfferSellingProgramStatusType::tryFrom($data['status']) : null, 
        );
    }

    public function jsonSerialize(): mixed {
        return [
            'sellingProgram' => $this->sellingProgram, 
            'status' => $this->status, 
        ];
    }
}


