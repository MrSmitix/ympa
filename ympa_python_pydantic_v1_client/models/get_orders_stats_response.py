...
class GetOrdersStatsResponse(BaseModel):
    """
    Ответ на запрос информации по заказам.  # noqa: E501
    """
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetOrdersStatsResponse:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GetOrdersStatsResponse:
        """Create an instance of GetOrdersStatsResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetOrdersStatsResponse.parse_obj(obj)

        _obj = GetOrdersStatsResponse.parse_obj({
            "status": obj.get("status"),
            "result": OrdersStatsDTO.from_dict(obj.get("result")) if obj.get("result") is not None else None
        })
        return _obj
