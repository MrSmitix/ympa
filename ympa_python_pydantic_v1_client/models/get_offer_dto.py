...
class GetOfferDTO(BaseModel):
    """
    Параметры товара.  # noqa: E501
    """
    ...
    pictures: Optional[conlist(StrictStr)] = Field(default=None, description="Ссылки на изображения товара. Изображение по первой ссылке считается основным, остальные дополнительными.  **Требования к ссылкам**  * Ссылок может быть до 30. * Указывайте ссылку целиком, включая протокол http или https. * Максимальная длина — 512 символов. * Русские буквы в URL можно. * Можно использовать прямые ссылки на изображения и на Яндекс Диск. Ссылки на Яндекс Диске нужно копировать с помощью функции **Поделиться**. Относительные ссылки и ссылки на другие облачные хранилища — не работают.  ✅ `https://example-shop.ru/images/sku12345.jpg`  ✅ `https://yadi.sk/i/NaBoRsimVOLov`  ❌ `/images/sku12345.jpg`  ❌ `https://www.dropbox.com/s/818f/tovar.jpg`  Ссылки на изображение должны быть постоянными. Нельзя использовать динамические ссылки, меняющиеся от выгрузки к выгрузке.  Если нужно заменить изображение, выложите новое изображение по новой ссылке, а ссылку на старое удалите. Если просто заменить изображение по старой ссылке, оно не обновится.  [Требования к изображениям](https://yandex.ru/support/marketplace/assortment/fields/images.html) ")
    videos: Optional[conlist(StrictStr, max_items=6)] = Field(default=None, description="Ссылка (URL) на видео товара.  Максимальное количество ссылок — 6.  **Требования к ссылке**  * Указывайте ссылку целиком, включая протокол http или https. * Максимальная длина — 512 символов. * Русские буквы в URL можно. * Можно использовать прямые ссылки на видео и на Яндекс Диск. Ссылки на Яндекс Диске нужно копировать с помощью функции **Поделиться**. Относительные ссылки и ссылки на другие облачные хранилища — не работают.  ✅ `https://example-shop.ru/video/sku12345.avi`  ✅ `https://yadi.sk/i/NaBoRsimVOLov`  ❌ `/video/sku12345.avi`  ❌ `https://www.dropbox.com/s/818f/super-tovar.avi`  Ссылки на видео должны быть постоянными. Нельзя использовать динамические ссылки, меняющиеся от выгрузки к выгрузке.  Если нужно заменить видео, выложите новое видео по новой ссылке, а ссылку на старое удалите. Если просто заменить видео по старой ссылке, оно не обновится.  [Требования к видео](https://yandex.ru/support/marketplace/assortment/fields/video.html) ")
    manuals: Optional[conlist(OfferManualDTO, max_items=6)] = Field(default=None, description="Список инструкций по использованию товара.  Максимальное количество инструкций — 6.  Если вы передадите пустое поле `manuals`, загруженные ранее инструкции удалятся. ")
    ...
    manufacturer_countries: Optional[conlist(StrictStr)] = Field(default=None, alias="manufacturerCountries", description="Страна, где был произведен товар.  Записывайте названия стран так, как они записаны в [списке](https://yastatic.net/s3/doc-binary/src/support/market/ru/countries.xlsx). ")
    ...
    tags: Optional[conlist(StrictStr)] = Field(default=None, description="Метки товара, используемые магазином. Покупателям теги не видны. По тегам можно группировать и фильтровать разные товары в каталоге — например, товары одной серии, коллекции или линейки.  Максимальная длина тега 20 символов. У одного товара может быть максимум 10 тегов. Всего можно создать не больше 50 разных тегов. ")
    ...
    customs_commodity_code: Optional[StrictStr] = Field(default=None, alias="customsCommodityCode", description="Код товара в единой Товарной номенклатуре внешнеэкономической деятельности (ТН ВЭД) — 10 или 14 цифр без пробелов.  Обязательно укажите, если он есть. ")
    certificates: Optional[conlist(StrictStr)] = Field(default=None, description="Номера документов на товар: сертификата, декларации соответствия и т. п.  Передавать можно только номера документов, сканы которого загружены в кабинете продавца по [инструкции](https://yandex.ru/support/marketplace/assortment/restrictions/certificates.html). ")
    box_count: Optional[StrictInt] = Field(default=None, alias="boxCount", description="Количество грузовых мест.  Параметр используется, если товар представляет собой несколько коробок, упаковок и так далее. Например, кондиционер занимает два места — внешний и внутренний блоки в двух коробках.  Для товаров, занимающих одно место, не передавайте этот параметр. ")
    ...
    params: Optional[conlist(OfferParamDTO)] = Field(default=None, description="{% note warning \"\" %}  Этот параметр устарел. При передаче характеристик используйте `parameterValues`.  {% endnote %}  Характеристики, которые есть только у товаров конкретной категории — например, диаметр колес велосипеда или материал подошвы обуви. ")
    basic_price: Optional[GetPriceWithDiscountDTO] = Field(default=None, alias="basicPrice")
    purchase_price: Optional[GetPriceDTO] = Field(default=None, alias="purchasePrice")
    additional_expenses: Optional[GetPriceDTO] = Field(default=None, alias="additionalExpenses")
    ...
    campaigns: Optional[conlist(OfferCampaignStatusDTO)] = Field(default=None, description="Список магазинов, в которых размещен товар. ")
    selling_programs: Optional[conlist(OfferSellingProgramDTO)] = Field(default=None, alias="sellingPrograms", description="Информация о том, какие для товара доступны модели размещения. ")
    archived: Optional[StrictBool] = Field(default=None, description="Товар помещен в архив. ")
    __properties = ["offerId", "name", "marketCategoryId", "category", "pictures", "videos", "manuals", "vendor", "barcodes", "description", "manufacturerCountries", "weightDimensions", "vendorCode", "tags", "shelfLife", "lifeTime", "guaranteePeriod", "customsCommodityCode", "certificates", "boxCount", "condition", "type", "downloadable", "adult", "age", "params", "basicPrice", "purchasePrice", "additionalExpenses", "cofinancePrice", "cardStatus", "campaigns", "sellingPrograms", "archived"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetOfferDTO:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in selling_programs (list)
        _items = []
        if self.selling_programs:
            for _item in self.selling_programs:
                if _item:
                    _items.append(_item.to_dict())
            _dict['sellingPrograms'] = _items
        ...
        # and __fields_set__ contains the field
        if self.manuals is None and "manuals" in self.__fields_set__:
            _dict['manuals'] = None
        ...
        # set to None if selling_programs (nullable) is None
        # and __fields_set__ contains the field
        if self.selling_programs is None and "selling_programs" in self.__fields_set__:
            _dict['sellingPrograms'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> GetOfferDTO:
        """Create an instance of GetOfferDTO from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetOfferDTO.parse_obj(obj)

        _obj = GetOfferDTO.parse_obj({
            ...
            "customs_commodity_code": obj.get("customsCommodityCode"),
            ...
            "purchase_price": GetPriceDTO.from_dict(obj.get("purchasePrice")) if obj.get("purchasePrice") is not None else None,
            "additional_expenses": GetPriceDTO.from_dict(obj.get("additionalExpenses")) if obj.get("additionalExpenses") is not None else None,
            ...
            "campaigns": [OfferCampaignStatusDTO.from_dict(_item) for _item in obj.get("campaigns")] if obj.get("campaigns") is not None else None,
            "selling_programs": [OfferSellingProgramDTO.from_dict(_item) for _item in obj.get("sellingPrograms")] if obj.get("sellingPrograms") is not None else None,
