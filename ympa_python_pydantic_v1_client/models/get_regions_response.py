...
class GetRegionsResponse(BaseModel):
    ...
    GetRegionsResponse
    """
    regions: conlist(RegionDTO) = Field(default=..., description="Регион доставки.")
    paging: Optional[ForwardScrollingPagerDTO] = None
    __properties = ["regions", "paging"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetRegionsResponse:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GetRegionsResponse:
        """Create an instance of GetRegionsResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetRegionsResponse.parse_obj(obj)

        _obj = GetRegionsResponse.parse_obj({
            "regions": [RegionDTO.from_dict(_item) for _item in obj.get("regions")] if obj.get("regions") is not None else None,
