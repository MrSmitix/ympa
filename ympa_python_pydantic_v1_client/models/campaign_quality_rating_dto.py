...
class CampaignQualityRatingDTO(BaseModel):
    """
    Информация об индексе качества магазина.  # noqa: E501
    """
    campaign_id: StrictInt = Field(default=..., alias="campaignId", description="Идентификатор магазина.")
    ratings: conlist(QualityRatingDTO) = Field(default=..., description="Список значений индекса качества.")
    __properties = ["campaignId", "ratings"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CampaignQualityRatingDTO:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CampaignQualityRatingDTO:
        """Create an instance of CampaignQualityRatingDTO from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CampaignQualityRatingDTO.parse_obj(obj)

        _obj = CampaignQualityRatingDTO.parse_obj({
