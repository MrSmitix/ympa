...
class FeedDownloadErrorDTO(BaseModel):
    """
    Информация об ошибке при загрузке прайс-листа. Выводится, если параметр `download status=ERROR`.   # noqa: E501
    """
    ...
    type: Optional[FeedDownloadErrorType] = None
    ...
    @classmethod
    def from_json(cls, json_str: str) -> FeedDownloadErrorDTO:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> FeedDownloadErrorDTO:
        """Create an instance of FeedDownloadErrorDTO from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return FeedDownloadErrorDTO.parse_obj(obj)

        _obj = FeedDownloadErrorDTO.parse_obj({
