...
class UpdateCampaignOfferDTO(BaseModel):
    ...
    vat: Optional[StrictInt] = Field(default=None, description="Ставка НДС, применяемая для товара. Задается цифрой:  * `2` — 10%. * `5` — 0%. * `6` — не облагается НДС. * `7` — 20%.  Если параметр не указан, используется ставка НДС, установленная в кабинете продавца на Маркете. ")
    __properties = ["offerId", "quantum", "available", "vat"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> UpdateCampaignOfferDTO:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> UpdateCampaignOfferDTO:
        """Create an instance of UpdateCampaignOfferDTO from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return UpdateCampaignOfferDTO.parse_obj(obj)

        _obj = UpdateCampaignOfferDTO.parse_obj({
