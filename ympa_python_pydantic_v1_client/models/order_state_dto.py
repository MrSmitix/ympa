...
class OrderStateDTO(BaseModel):
    """
    Информация по заказу.  # noqa: E501
    """
    id: StrictInt = Field(default=..., description="Идентификатор заказа.")
    status: OrderStatusType = Field(...)
    substatus: Optional[OrderSubstatusType] = None
    __properties = ["id", "status", "substatus"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> OrderStateDTO:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> OrderStateDTO:
        """Create an instance of OrderStateDTO from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return OrderStateDTO.parse_obj(obj)

        _obj = OrderStateDTO.parse_obj({
            "id": obj.get("id"),
