...
class UpdateOfferDTO(BaseModel):
    ...
    parameter_values: Optional[conlist(ParameterValueDTO, max_items=300)] = Field(default=None, alias="parameterValues", description="Список характеристик с их значениями.  С `parameterValues` обязательно передавайте `marketCategoryId` — идентификатор категории на Маркете, к которой относятся указанные характеристики товара.  При **изменении** характеристик передавайте только те, значение которых нужно обновить.  Чтобы **удалить** значение заданной характеристики, передайте ее `parameterId` с пустым `value`.  Максимальное количество характеристик — 300. ")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> UpdateOfferDTO:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> UpdateOfferDTO:
        """Create an instance of UpdateOfferDTO from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return UpdateOfferDTO.parse_obj(obj)

        _obj = UpdateOfferDTO.parse_obj({
            ...
            "basic_price": UpdatePriceWithDiscountDTO.from_dict(obj.get("basicPrice")) if obj.get("basicPrice") is not None else None,
            "purchase_price": BasePriceDTO.from_dict(obj.get("purchasePrice")) if obj.get("purchasePrice") is not None else None,
            "additional_expenses": BasePriceDTO.from_dict(obj.get("additionalExpenses")) if obj.get("additionalExpenses") is not None else None,
            "cofinance_price": BasePriceDTO.from_dict(obj.get("cofinancePrice")) if obj.get("cofinancePrice") is not None else None
        })
        return _obj
