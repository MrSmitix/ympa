...
class FullOutletLicenseDTO(BaseModel):
    """
    ...
    check_status: Optional[LicenseCheckStatusType] = Field(default=None, alias="checkStatus")
    check_comment: Optional[StrictStr] = Field(default=None, alias="checkComment", description="Причина, по которой лицензия не прошла проверку.  Параметр возвращается, только если параметр `checkStatus` имеет значение `FAIL`. ")
    __properties = ["id", "outletId", "licenseType", "number", "dateOfIssue", "dateOfExpiry", "checkStatus", "checkComment"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> FullOutletLicenseDTO:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> FullOutletLicenseDTO:
        """Create an instance of FullOutletLicenseDTO from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return FullOutletLicenseDTO.parse_obj(obj)

        _obj = FullOutletLicenseDTO.parse_obj({
            "id": obj.get("id"),
            ...
            "check_status": obj.get("checkStatus"),
            "check_comment": obj.get("checkComment")
        })
        return _obj
