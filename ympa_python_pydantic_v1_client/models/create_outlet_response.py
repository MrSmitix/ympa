...
class CreateOutletResponse(BaseModel):
    """
    Ответ на запрос о создании точки продаж.  # noqa: E501
    """
    status: Optional[ApiResponseStatusType] = None
    result: Optional[OutletResponseDTO] = None
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CreateOutletResponse:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CreateOutletResponse:
        """Create an instance of CreateOutletResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CreateOutletResponse.parse_obj(obj)

        _obj = CreateOutletResponse.parse_obj({
            "status": obj.get("status"),
            "result": OutletResponseDTO.from_dict(obj.get("result")) if obj.get("result") is not None else None
        })
        return _obj
