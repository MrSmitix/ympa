...
module OpenapiClient
  ...
  class SkuBidRecommendationItemDTO
    ...
    attr_accessor :bid_recommendations

    # Рекомендованные цены.
    attr_accessor :price_recommendations
    ...
    def self.openapi_types
      {
        :'sku' => :'String',
        :'bid' => :'Integer',
        :'bid_recommendations' => :'Array<BidRecommendationItemDTO>',
        :'price_recommendations' => :'Array<PriceRecommendationItemDTO>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SkuBidRecommendationItemDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SkuBidRecommendationItemDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'bid_recommendations')
        if (value = attributes[:'bid_recommendations']).is_a?(Array)
          self.bid_recommendations = value
        end
      end

      if attributes.key?(:'price_recommendations')
        if (value = attributes[:'price_recommendations']).is_a?(Array)
          self.price_recommendations = value
        end
      end
    end
    ...
    def sku=(sku)
      ...
      pattern = Regexp.new(/^[^\x00-\x08\x0A-\x1f\x7f]{1,255}$/)
      if sku !~ pattern
        fail ArgumentError, "invalid value for \"sku\", must conform to the pattern #{pattern}."
      end
