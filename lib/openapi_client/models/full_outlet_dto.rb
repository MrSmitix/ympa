  ...
  class FullOutletDTO
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::FullOutletDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::FullOutletDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'shop_outlet_id')
        self.shop_outlet_id = attributes[:'shop_outlet_id']
      end

      if attributes.key?(:'working_time')
        self.working_time = attributes[:'working_time']
      end

      if attributes.key?(:'moderation_reason')
        self.moderation_reason = attributes[:'moderation_reason']
      end
    end
    ...
    def hash
      [name, type, coords, is_main, shop_outlet_code, visibility, address, phones, working_schedule, delivery_rules, storage_period, id, status, region, shop_outlet_id, working_time, moderation_reason].hash
    end
