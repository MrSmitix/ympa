...
module OpenapiClient
  ...
  class CampaignSettingsDTO
    ...
    attr_accessor :use_open_stat

    attr_accessor :local_region
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CampaignSettingsDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CampaignSettingsDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'show_in_context')
        self.show_in_context = attributes[:'show_in_context']
      end

      if attributes.key?(:'show_in_premium')
        self.show_in_premium = attributes[:'show_in_premium']
      end

      if attributes.key?(:'use_open_stat')
        self.use_open_stat = attributes[:'use_open_stat']
      end

      if attributes.key?(:'local_region')
        self.local_region = attributes[:'local_region']
      end
    end
