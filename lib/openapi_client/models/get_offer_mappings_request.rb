  ...
  class GetOfferMappingsRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::GetOfferMappingsRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::GetOfferMappingsRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@offer_ids.nil? && @offer_ids.length > 200
    ...
    def valid?
      ...
      return false if !@offer_ids.nil? && @offer_ids.length > 200
    ...
    def offer_ids=(offer_ids)
      if !offer_ids.nil? && offer_ids.length > 200
        fail ArgumentError, 'invalid value for "offer_ids", number of items must be less than or equal to 200.'
      end
