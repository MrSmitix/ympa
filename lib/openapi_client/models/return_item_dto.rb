  ...
  class ReturnItemDTO
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ReturnItemDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ReturnItemDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'decisions')
        if (value = attributes[:'decisions']).is_a?(Array)
    ...
    def list_invalid_properties
      ...
      if @shop_sku.nil?
        invalid_properties.push('invalid value for "shop_sku", shop_sku cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @shop_sku.nil?
      return false if @shop_sku.to_s.length > 255
      return false if @shop_sku.to_s.length < 1
