...
module OpenapiClient
  ...
  class OfferDTO
    ...
    attr_accessor :shop_category_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::OfferDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::OfferDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'shop_category_id')
        self.shop_category_id = attributes[:'shop_category_id']
      end
      ...
      if attributes.key?(:'cut_price')
        self.cut_price = attributes[:'cut_price']
      end
      ...
      if attributes.key?(:'cbid')
        self.cbid = attributes[:'cbid']
      end
