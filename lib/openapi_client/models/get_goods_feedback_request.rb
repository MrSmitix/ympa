...
module OpenapiClient
  ...
  class GetGoodsFeedbackRequest
    ...
    attr_accessor :reaction_status

    # Оценка товара.
    attr_accessor :rating_values
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::GetGoodsFeedbackRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::GetGoodsFeedbackRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'reaction_status')
        self.reaction_status = attributes[:'reaction_status']
      end

      if attributes.key?(:'rating_values')
        if (value = attributes[:'rating_values']).is_a?(Array)
          self.rating_values = value
        end
      end

      if attributes.key?(:'model_ids')
    ...
    def valid?
      ...
      return false if !@rating_values.nil? && @rating_values.length > 5
      return false if !@model_ids.nil? && @model_ids.length > 20
      true
    end
    ...
    def rating_values=(rating_values)
      if !rating_values.nil? && rating_values.length > 5
        fail ArgumentError, 'invalid value for "rating_values", number of items must be less than or equal to 5.'
      end
    ...
    def model_ids=(model_ids)
      if !model_ids.nil? && model_ids.length > 20
        fail ArgumentError, 'invalid value for "model_ids", number of items must be less than or equal to 20.'
      end
