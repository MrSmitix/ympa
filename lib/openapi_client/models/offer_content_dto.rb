  ...
  class OfferContentDTO
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::OfferContentDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::OfferContentDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @parameter_values.nil?
        invalid_properties.push('invalid value for "parameter_values", parameter_values cannot be nil.')
      end

      if @parameter_values.length > 300
        invalid_properties.push('invalid value for "parameter_values", number of items must be less than or equal to 300.')
      end

      if @parameter_values.length < 1
        invalid_properties.push('invalid value for "parameter_values", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @parameter_values.nil?
      return false if @parameter_values.length > 300
      return false if @parameter_values.length < 1
      true
    end
    ...
    def parameter_values=(parameter_values)
      if parameter_values.nil?
        fail ArgumentError, 'parameter_values cannot be nil'
      end
      ...
      if parameter_values.length < 1
        fail ArgumentError, 'invalid value for "parameter_values", number of items must be greater than or equal to 1.'
      end
    ...
    def hash
      [offer_id, category_id, parameter_values].hash
    end
