...
class BaseShipmentDTO {
  /// Returns a new [BaseShipmentDTO] instance.
  BaseShipmentDTO({
    this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is BaseShipmentDTO &&
    other.id == id &&
  ...
  @override
  String toString() => 'BaseShipmentDTO[id=$id, planIntervalFrom=$planIntervalFrom, planIntervalTo=$planIntervalTo, shipmentType=$shipmentType, warehouse=$warehouse, warehouseTo=$warehouseTo, externalId=$externalId, deliveryService=$deliveryService, palletsCount=$palletsCount, orderIds=$orderIds, draftCount=$draftCount, plannedCount=$plannedCount, factCount=$factCount]';
  ...
  /// Returns a new [BaseShipmentDTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static BaseShipmentDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "BaseShipmentDTO[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "BaseShipmentDTO[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<BaseShipmentDTO> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <BaseShipmentDTO>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = BaseShipmentDTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, BaseShipmentDTO> mapFromJson(dynamic json) {
    final map = <String, BaseShipmentDTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = BaseShipmentDTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of BaseShipmentDTO-objects as value to a dart map
  static Map<String, List<BaseShipmentDTO>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<BaseShipmentDTO>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = BaseShipmentDTO.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
