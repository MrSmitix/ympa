...
class OutletDTO {
  /// Returns a new [OutletDTO] instance.
  OutletDTO({
    required this.name,
    required this.type,
    this.coords,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is OutletDTO &&
    other.name == name &&
    other.type == type &&
    other.coords == coords &&
    other.isMain == isMain &&
  ...
  @override
  String toString() => 'OutletDTO[name=$name, type=$type, coords=$coords, isMain=$isMain, shopOutletCode=$shopOutletCode, visibility=$visibility, address=$address, phones=$phones, workingSchedule=$workingSchedule, deliveryRules=$deliveryRules, storagePeriod=$storagePeriod]';
  ...
  /// Returns a new [OutletDTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static OutletDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "OutletDTO[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "OutletDTO[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<OutletDTO> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <OutletDTO>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = OutletDTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, OutletDTO> mapFromJson(dynamic json) {
    final map = <String, OutletDTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = OutletDTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of OutletDTO-objects as value to a dart map
  static Map<String, List<OutletDTO>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<OutletDTO>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = OutletDTO.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
