...
class DocumentDTO {
  ...
  @override
  String toString() => 'DocumentDTO[status=$status, number=$number, date=$date]';
  ...
  static DocumentDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      return DocumentDTO(
        status: OrderDocumentStatusType.fromJson(json[r'status']),
  ...
  static Map<String, List<DocumentDTO>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = DocumentDTO.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
