...
#
package WWW::OpenAPIClient::Object::OfferCardDTO;
...
__PACKAGE__->class_documentation({description => 'Информация о состоянии карточки товара.  Если поле &#x60;mapping&#x60; отсутствует в ответе, Маркет еще не успел обработать информацию о товаре. Чтобы определить категорию такого товара, повторите запрос через несколько минут. ',
                                  class => 'OfferCardDTO',
                                  required => [], # TODO
}                                 );
...
__PACKAGE__->method_documentation({
    ...
    'parameter_values' => {
        datatype => 'ARRAY[ParameterValueDTO]',
        base_name => 'parameterValues',
    ...
    'content_rating' => {
        datatype => 'int',
        base_name => 'contentRating',
        description => 'Процент заполненности карточки.',
        format => '',
        read_only => '',
            },
    'recommendations' => {
        datatype => 'ARRAY[OfferCardRecommendationDTO]',
        base_name => 'recommendations',
        description => 'Список рекомендаций к заполнению карточки.  Рекомендации Маркета помогают заполнять карточку так, чтобы покупателям было проще найти ваш товар и решиться на покупку. ',
        format => '',
        read_only => '',
            },
    'errors' => {
        datatype => 'ARRAY[OfferErrorDTO]',
        base_name => 'errors',
