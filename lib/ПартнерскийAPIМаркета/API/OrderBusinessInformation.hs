...
module ПартнерскийAPIМаркета.API.OrderBusinessInformation where
...
getOrderBusinessBuyerInfo
  ...
  -> ПартнерскийAPIМаркетаRequest GetOrderBusinessBuyerInfo MimeNoContent GetBusinessBuyerInfoResponse MimeJSON
getOrderBusinessBuyerInfo (CampaignId campaignId) (OrderId orderId) =
...
data GetOrderBusinessBuyerInfo  
-- | @application/json@
instance Produces GetOrderBusinessBuyerInfo MimeJSON
...
getOrderBusinessDocumentsInfo
  :: CampaignId -- ^ "campaignId" -  Идентификатор кампании в API и магазина в кабинете. Каждая кампания в API соответствует магазину в кабинете.  Чтобы узнать идентификаторы своих магазинов, воспользуйтесь запросом [GET campaigns](../../reference/campaigns/getCampaigns.md).  ℹ️ [Что такое кабинет и магазин на Маркете](https://yandex.ru/support/marketplace/account/introduction.html) 
  -> OrderId -- ^ "orderId" -  Идентификатор заказа.
  -> ПартнерскийAPIМаркетаRequest GetOrderBusinessDocumentsInfo MimeNoContent GetBusinessDocumentsInfoResponse MimeJSON
getOrderBusinessDocumentsInfo (CampaignId campaignId) (OrderId orderId) =
  _mkRequest "POST" ["/campaigns/",toPath campaignId,"/orders/",toPath orderId,"/documents"]
    `_hasAuthType` (P.Proxy :: P.Proxy AuthOAuthOAuth)

data GetOrderBusinessDocumentsInfo  
-- | @application/json@
instance Produces GetOrderBusinessDocumentsInfo MimeJSON
