...
module ПартнерскийAPIМаркета.API.Feedbacks where
...
getFeedbackAndCommentUpdates
  ...
  -> ПартнерскийAPIМаркетаRequest GetFeedbackAndCommentUpdates MimeNoContent GetFeedbackListResponse MimeJSON
getFeedbackAndCommentUpdates (CampaignId campaignId) =
  _mkRequest "GET" ["/campaigns/",toPath campaignId,"/feedback/updates"]
    `_hasAuthType` (P.Proxy :: P.Proxy AuthOAuthOAuth)

{-# DEPRECATED getFeedbackAndCommentUpdates "" #-}

data GetFeedbackAndCommentUpdates  
...
instance HasOptionalParam GetFeedbackAndCommentUpdates PageToken where
...
instance HasOptionalParam GetFeedbackAndCommentUpdates Limit where
...
instance HasOptionalParam GetFeedbackAndCommentUpdates FromDate where
  applyOptionalParam req (FromDate xs) =
    req `addQuery` toQuery ("from_date", Just xs)
-- | @application/json@
instance Produces GetFeedbackAndCommentUpdates MimeJSON
