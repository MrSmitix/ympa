
/*
 * GetOutletLicensesResponse.h
 *
 * 
 */

#ifndef TINY_CPP_CLIENT_GetOutletLicensesResponse_H_
#define TINY_CPP_CLIENT_GetOutletLicensesResponse_H_


#include <string>
#include "bourne/json.hpp"
#include "Helpers.h"
#include "ApiResponseStatusType.h"
#include "OutletLicensesResponseDTO.h"

namespace Tiny {


/*! \brief 
 *
 *  \ingroup Models
 *
 */

class GetOutletLicensesResponse{
public:

    /*! \brief Constructor.
	 */
    GetOutletLicensesResponse();
    GetOutletLicensesResponse(std::string jsonString);


    /*! \brief Destructor.
	 */
    virtual ~GetOutletLicensesResponse();


    /*! \brief Retrieve a bourne JSON representation of this class.
	 */
    bourne::json toJson();


    /*! \brief Fills in members of this class from bourne JSON object representing it.
	 */
    void fromJson(std::string jsonObj);

	/*! \brief Get 
	 */
	ApiResponseStatusType getStatus();

	/*! \brief Set 
	 */
	void setStatus(ApiResponseStatusType  status);
	/*! \brief Get 
	 */
	OutletLicensesResponseDTO getResult();

	/*! \brief Set 
	 */
	void setResult(OutletLicensesResponseDTO  result);


    private:
    ApiResponseStatusType status;
    OutletLicensesResponseDTO result;
};
}

#endif /* TINY_CPP_CLIENT_GetOutletLicensesResponse_H_ */
