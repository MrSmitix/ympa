...
void OAIFeedbackListDTO::fromJsonObject(QJsonObject json) {
    ...
    m_feedback_list_isValid = ::OpenAPI::fromJsonValue(m_feedback_list, json[QString("feedbackList")]);
...
QJsonObject OAIFeedbackListDTO::asJsonObject() const {
    ...
    if (m_feedback_list.size() > 0) {
        obj.insert(QString("feedbackList"), ::OpenAPI::toJsonValue(m_feedback_list));
    }
    return obj;
}
...
void OAIFeedbackListDTO::setFeedbackList(const QList<OAIFeedbackDTO> &feedback_list) {
    m_feedback_list = feedback_list;
    m_feedback_list_isSet = true;
}
