...
void OAIAddOffersToArchiveDTO::fromJsonObject(QJsonObject json) {
    ...
    m_not_archived_offers_isValid = ::OpenAPI::fromJsonValue(m_not_archived_offers, json[QString("notArchivedOffers")]);
...
QJsonObject OAIAddOffersToArchiveDTO::asJsonObject() const {
    QJsonObject obj;
    if (m_not_archived_offers.size() > 0) {
        obj.insert(QString("notArchivedOffers"), ::OpenAPI::toJsonValue(m_not_archived_offers));
    }
    return obj;
}

QList<OAIAddOffersToArchiveErrorDTO> OAIAddOffersToArchiveDTO::getNotArchivedOffers() const {
    return m_not_archived_offers;
}
void OAIAddOffersToArchiveDTO::setNotArchivedOffers(const QList<OAIAddOffersToArchiveErrorDTO> &not_archived_offers) {
    m_not_archived_offers = not_archived_offers;
    m_not_archived_offers_isSet = true;
}
