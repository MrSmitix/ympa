...
// AssertQuantumDtoRequired checks if the required fields are not zero-ed
func AssertQuantumDtoRequired(obj QuantumDto) error {
	return nil
}

// AssertQuantumDtoConstraints checks if the values respects the defined constraints
func AssertQuantumDtoConstraints(obj QuantumDto) error {
	if obj.MinQuantity < 1 {
		return &ParsingError{Param: "MinQuantity", Err: errors.New(errMsgMinValueConstraint)}
	}
	if obj.StepQuantity < 1 {
		return &ParsingError{Param: "StepQuantity", Err: errors.New(errMsgMinValueConstraint)}
	}
	if obj.StepQuantity > 100 {
		return &ParsingError{Param: "StepQuantity", Err: errors.New(errMsgMaxValueConstraint)}
	}
	return nil
}
