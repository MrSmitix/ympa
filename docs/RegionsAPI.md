...
// Информация о дочерних регионах
RegionsAPI.searchRegionChildren(regionId: regionId, page: page, pageSize: pageSize) { (response, error) in
...
// Информация о регионе
RegionsAPI.searchRegionsById(regionId: regionId) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Поиск регионов по их имени
RegionsAPI.searchRegionsByName(name: name, pageToken: pageToken, limit: limit) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
