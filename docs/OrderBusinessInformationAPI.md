...
// Информация о покупателе — юридическом лице
OrderBusinessInformationAPI.getOrderBusinessBuyerInfo(campaignId: campaignId, orderId: orderId) { (response, error) in
...
// Информация о документах
OrderBusinessInformationAPI.getOrderBusinessDocumentsInfo(campaignId: campaignId, orderId: orderId) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
