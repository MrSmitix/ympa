...
To initiate an instance of `BidsApi`, you can use micronaut's `ApplicationContext`:
```java
/* imports
...
PetApi apiInstance = context.getBean(BidsApi.class);
```
...
```java
Mono<GetBidsInfoResponse> BidsApi.getBidsInfoForBusiness(businessIdpageTokenlimitgetBidsInfoRequest)
```
...
```java
Mono<GetBidsRecommendationsResponse> BidsApi.getBidsRecommendations(businessIdgetBidsRecommendationsRequest)
```
...
# **putBidsForBusiness**
```java
Mono<EmptyApiResponse> BidsApi.putBidsForBusiness(businessIdputSkuBidsRequest)
```
...
# **putBidsForCampaign**
```java
Mono<EmptyApiResponse> BidsApi.putBidsForCampaign(campaignIdputSkuBidsRequest)
```
