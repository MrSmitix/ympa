...
To initiate an instance of `FeedsApi`, you can use micronaut's `ApplicationContext`:
```java
/* imports
...
PetApi apiInstance = context.getBean(FeedsApi.class);
```
...
# **getFeed**
```java
Mono<GetFeedResponse> FeedsApi.getFeed(campaignIdfeedId)
```
...
# **getFeedIndexLogs**
```java
Mono<GetFeedIndexLogsResponse> FeedsApi.getFeedIndexLogs(campaignIdfeedIdlimitpublishedTimeFrompublishedTimeTostatus)
```
...
# **getFeeds**
```java
Mono<GetFeedsResponse> FeedsApi.getFeeds(campaignId)
```
...
# **refreshFeed**
```java
Mono<EmptyApiResponse> FeedsApi.refreshFeed(campaignIdfeedId)
```
...
# **setFeedParams**
```java
Mono<EmptyApiResponse> FeedsApi.setFeedParams(campaignIdfeedIdsetFeedParamsRequest)
```
