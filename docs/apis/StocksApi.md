...
To initiate an instance of `StocksApi`, you can use micronaut's `ApplicationContext`:
```java
/* imports
...
PetApi apiInstance = context.getBean(StocksApi.class);
```
...
# **getStocks**
```java
Mono<GetWarehouseStocksResponse> StocksApi.getStocks(campaignIdpageTokenlimitgetWarehouseStocksRequest)
```
...
# **updateStocks**
```java
Mono<EmptyApiResponse> StocksApi.updateStocks(campaignIdupdateStocksRequest)
```
