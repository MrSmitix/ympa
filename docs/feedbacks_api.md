...
> models::GetFeedbackListResponse getFeedbackAndCommentUpdates(ctx, campaign_id, optional)
 ...
 **from_date** | **chrono::naive::NaiveDate**| Начальная дата обновления отзывов.  Если параметр указан, возвращаются отзывы, которые были написаны или обновлены с этой даты.  Формат даты: `ГГГГ-ММ-ДД`.  | 
