# ympa_python_client.DeliveryServicesApi
...
with ympa_python_client.ApiClient(configuration) as api_client:
    ...
    api_instance = ympa_python_client.DeliveryServicesApi(api_client)

    try:
        # Справочник служб доставки
        api_response = api_instance.get_delivery_services()
        print("The response of DeliveryServicesApi->get_delivery_services:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling DeliveryServicesApi->get_delivery_services: %s\n" % e)
```
