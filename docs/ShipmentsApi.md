...
```bash
 confirmShipment campaignId=value shipmentId=value
```
 ...
 **shipmentId** | **integer** | Идентификатор отгрузки. | [default to null]
...
```bash
 downloadShipmentAct campaignId=value shipmentId=value
```
...
```bash
 downloadShipmentDiscrepancyAct campaignId=value shipmentId=value
```
...
```bash
 downloadShipmentInboundAct campaignId=value shipmentId=value
```
...
```bash
 downloadShipmentPalletLabels campaignId=value shipmentId=value  format=value
```
...
ℹ️ [Что такое кабинет и магазин на Маркете](https://yandex.ru/support/marketplace/account/introduction.html) | [default to null]
 **shipmentId** | **integer** | Идентификатор отгрузки. | [default to null]
 **format** | [**ShipmentPalletLabelPageFormatType**](.md) | Формат страниц PDF-файла с ярлыками:

* 'A4' — по 16 ярлыков на странице.
* 'A8' — по одному ярлыку на странице. | [optional] [default to null]
...
```bash
 downloadShipmentReceptionTransferAct campaignId=value  warehouse_id=value
```
...
```bash
 downloadShipmentTransportationWaybill campaignId=value shipmentId=value
```
...
```bash
 getShipment campaignId=value shipmentId=value  cancelledOrders=value
```
...
ℹ️ [Что такое кабинет и магазин на Маркете](https://yandex.ru/support/marketplace/account/introduction.html) | [default to null]
 **shipmentId** | **integer** | Идентификатор отгрузки. | [default to null]
 **cancelledOrders** | **boolean** | Возвращать ли отмененные заказы.

Значение по умолчанию — 'true'. Если возвращать отмененные заказы не нужно, передайте значение 'false'. | [optional] [default to true]
...
```bash
 getShipmentOrdersInfo campaignId=value shipmentId=value
```
...
```bash
 searchShipments campaignId=value  page_token=value  limit=value
```
...
```bash
 setShipmentPalletsCount campaignId=value shipmentId=value
```
...
```bash
 transferOrdersFromShipment campaignId=value shipmentId=value
```
