...
// Заказы, которые повлияли на индекс качества
RatingsAPI.getQualityRatingDetails(campaignId: campaignId) { (response, error) in
...
// Индекс качества магазинов
RatingsAPI.getQualityRatings(businessId: businessId, getQualityRatingRequest: getQualityRatingRequest) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
