...
// Информация о прайс-листе
FeedsAPI.getFeed(campaignId: campaignId, feedId: feedId) { (response, error) in
...
// Отчет по индексации прайс-листа
FeedsAPI.getFeedIndexLogs(campaignId: campaignId, feedId: feedId, limit: limit, publishedTimeFrom: publishedTimeFrom, publishedTimeTo: publishedTimeTo, status: status) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Список прайс-листов магазина
FeedsAPI.getFeeds(campaignId: campaignId) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Сообщить, что прайс-лист обновился
FeedsAPI.refreshFeed(campaignId: campaignId, feedId: feedId) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Изменение параметров прайс-листа
FeedsAPI.setFeedParams(campaignId: campaignId, feedId: feedId, setFeedParamsRequest: setFeedParamsRequest) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
