...
class OrdersStatsPaymentDTOTest extends AnyWordSpec with Matchers {

    "OrdersStatsPaymentDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(OrdersStatsPaymentDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = OrdersStatsPaymentDTOData.fromJsonString("""""").validated()
