...
class ShipmentInfoDTOTest extends AnyWordSpec with Matchers {

    "ShipmentInfoDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(ShipmentInfoDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = ShipmentInfoDTOData.fromJsonString("""""").validated()
