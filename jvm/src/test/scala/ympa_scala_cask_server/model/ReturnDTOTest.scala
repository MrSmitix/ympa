...
class ReturnDTOTest extends AnyWordSpec with Matchers {

    "ReturnDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(ReturnDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = ReturnDTOData.fromJsonString("""""").validated()
