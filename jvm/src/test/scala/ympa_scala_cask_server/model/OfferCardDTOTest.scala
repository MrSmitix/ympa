...
class OfferCardDTOTest extends AnyWordSpec with Matchers {

    "OfferCardDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(OfferCardDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = OfferCardDTOData.fromJsonString("""""").validated()
