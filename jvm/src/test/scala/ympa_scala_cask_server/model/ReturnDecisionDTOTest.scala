...
class ReturnDecisionDTOTest extends AnyWordSpec with Matchers {

    "ReturnDecisionDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(ReturnDecisionDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = ReturnDecisionDTOData.fromJsonString("""""").validated()
