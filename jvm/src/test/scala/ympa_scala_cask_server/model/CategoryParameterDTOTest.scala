...
class CategoryParameterDTOTest extends AnyWordSpec with Matchers {

    "CategoryParameterDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(CategoryParameterDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = CategoryParameterDTOData.fromJsonString("""""").validated()
