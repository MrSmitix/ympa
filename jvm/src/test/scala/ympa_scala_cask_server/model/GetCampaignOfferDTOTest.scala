...
class GetCampaignOfferDTOTest extends AnyWordSpec with Matchers {

    "GetCampaignOfferDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(GetCampaignOfferDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = GetCampaignOfferDTOData.fromJsonString("""""").validated()
