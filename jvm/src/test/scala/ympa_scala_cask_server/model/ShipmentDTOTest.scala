...
class ShipmentDTOTest extends AnyWordSpec with Matchers {

    "ShipmentDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(ShipmentDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = ShipmentDTOData.fromJsonString("""""").validated()
