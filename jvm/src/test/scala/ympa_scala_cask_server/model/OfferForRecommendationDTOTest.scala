...
class OfferForRecommendationDTOTest extends AnyWordSpec with Matchers {

    "OfferForRecommendationDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(OfferForRecommendationDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = OfferForRecommendationDTOData.fromJsonString("""""").validated()
