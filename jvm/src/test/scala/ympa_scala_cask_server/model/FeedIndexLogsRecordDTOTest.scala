...
class FeedIndexLogsRecordDTOTest extends AnyWordSpec with Matchers {

    "FeedIndexLogsRecordDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(FeedIndexLogsRecordDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = FeedIndexLogsRecordDTOData.fromJsonString("""""").validated()
