...
class UpdateMappingsOfferDTOTest extends AnyWordSpec with Matchers {

    "UpdateMappingsOfferDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(UpdateMappingsOfferDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = UpdateMappingsOfferDTOData.fromJsonString("""""").validated()
