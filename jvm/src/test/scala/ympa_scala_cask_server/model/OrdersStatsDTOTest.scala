...
class OrdersStatsDTOTest extends AnyWordSpec with Matchers {

    "OrdersStatsDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(OrdersStatsDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = OrdersStatsDTOData.fromJsonString("""""").validated()
