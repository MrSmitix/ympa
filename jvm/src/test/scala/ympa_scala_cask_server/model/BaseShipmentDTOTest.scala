...
class BaseShipmentDTOTest extends AnyWordSpec with Matchers {

    "BaseShipmentDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(BaseShipmentDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = BaseShipmentDTOData.fromJsonString("""""").validated()
