...
class ApiErrorResponseTest extends AnyWordSpec with Matchers {

    "ApiErrorResponse.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(ApiErrorResponseData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = ApiErrorResponseData.fromJsonString("""""").validated()
