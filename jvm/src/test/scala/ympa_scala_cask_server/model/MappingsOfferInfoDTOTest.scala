...
class MappingsOfferInfoDTOTest extends AnyWordSpec with Matchers {

    "MappingsOfferInfoDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(MappingsOfferInfoDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = MappingsOfferInfoDTOData.fromJsonString("""""").validated()
