...
class ChatMessageDTOTest extends AnyWordSpec with Matchers {

    "ChatMessageDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(ChatMessageDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = ChatMessageDTOData.fromJsonString("""""").validated()
