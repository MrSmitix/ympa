...
class UpdateOfferDTOTest extends AnyWordSpec with Matchers {

    "UpdateOfferDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(UpdateOfferDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = UpdateOfferDTOData.fromJsonString("""""").validated()
