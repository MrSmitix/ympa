...
class EnrichedMappingsOfferDTOTest extends AnyWordSpec with Matchers {

    "EnrichedMappingsOfferDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(EnrichedMappingsOfferDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = EnrichedMappingsOfferDTOData.fromJsonString("""""").validated()
