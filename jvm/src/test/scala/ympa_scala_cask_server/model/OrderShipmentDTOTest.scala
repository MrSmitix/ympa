...
class OrderShipmentDTOTest extends AnyWordSpec with Matchers {

    "OrderShipmentDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(OrderShipmentDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = OrderShipmentDTOData.fromJsonString("""""").validated()
