...
class GetChatInfoDTOTest extends AnyWordSpec with Matchers {

    "GetChatInfoDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(GetChatInfoDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = GetChatInfoDTOData.fromJsonString("""""").validated()
