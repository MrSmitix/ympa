...
class BaseOfferDTOTest extends AnyWordSpec with Matchers {

    "BaseOfferDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(BaseOfferDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = BaseOfferDTOData.fromJsonString("""""").validated()
