...
class MappingsOfferDTOTest extends AnyWordSpec with Matchers {

    "MappingsOfferDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(MappingsOfferDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = MappingsOfferDTOData.fromJsonString("""""").validated()
