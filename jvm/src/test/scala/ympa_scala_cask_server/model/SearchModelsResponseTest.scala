...
class SearchModelsResponseTest extends AnyWordSpec with Matchers {

    "SearchModelsResponse.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(SearchModelsResponseData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = SearchModelsResponseData.fromJsonString("""""").validated()
