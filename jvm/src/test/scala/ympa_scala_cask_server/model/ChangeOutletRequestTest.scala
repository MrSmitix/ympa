...
class ChangeOutletRequestTest extends AnyWordSpec with Matchers {

    "ChangeOutletRequest.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(ChangeOutletRequestData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = ChangeOutletRequestData.fromJsonString("""""").validated()
