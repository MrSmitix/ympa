...
class FeedPublicationDTOTest extends AnyWordSpec with Matchers {

    "FeedPublicationDTO.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(FeedPublicationDTOData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = FeedPublicationDTOData.fromJsonString("""""").validated()
