(ns партнерский-api-маркета.api.dbs
...
(defn-spec create-outlet-with-http-info any?
...
|-|"
  [campaignId int?, change-outlet-request change-outlet-request]
  (check-required-params campaignId change-outlet-request)
  (call-api "/campaigns/{campaignId}/outlets" :post
            {:path-params   {"campaignId" campaignId }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    change-outlet-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["OAuth"]}))

(defn-spec create-outlet create-outlet-response-spec
  "Создание точки продаж
  Создает точку продаж магазина на Маркете.
...
|-|"
  [campaignId int?, change-outlet-request change-outlet-request]
  (let [res (:data (create-outlet-with-http-info campaignId change-outlet-request))]
    (if (:decode-models *api-context*)
       (st/decode create-outlet-response-spec res st/string-transformer)
       res)))
...
(defn-spec delete-outlet-with-http-info any?
...
|**⚙️ Лимит:** 100 000 запросов в час|
|-|"
  [campaignId int?, outletId int?]
  (check-required-params campaignId outletId)
  (call-api "/campaigns/{campaignId}/outlets/{outletId}" :delete
            {:path-params   {"campaignId" campaignId "outletId" outletId }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec delete-outlet empty-api-response-spec
  "Удаление точки продаж
  Удаляет точку продаж магазина на Маркете.
...
|-|"
  [campaignId int?, outletId int?]
  (let [res (:data (delete-outlet-with-http-info campaignId outletId))]
    (if (:decode-models *api-context*)
       (st/decode empty-api-response-spec res st/string-transformer)
       res)))


(defn-spec delete-outlet-licenses-with-http-info any?
...
|**⚙️ Лимит:** 100 000 запросов в час|
|-|"
  ([campaignId int?, ] (delete-outlet-licenses-with-http-info campaignId nil))
  ([campaignId int?, {:keys [ids]} (s/map-of keyword? any?)]
   (check-required-params campaignId)
   (call-api "/campaigns/{campaignId}/outlets/licenses" :delete
             {:path-params   {"campaignId" campaignId }
              :header-params {}
              :query-params  {"ids" (with-collection-format ids :csv) }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["OAuth"]})))

(defn-spec delete-outlet-licenses empty-api-response-spec
  "Удаление лицензий для точек продаж
  Удаляет информацию о лицензиях для точек продаж.
...
|**⚙️ Лимит:** 100 000 запросов в час|
|-|"
  ([campaignId int?, ] (delete-outlet-licenses campaignId nil))
  ([campaignId int?, optional-params any?]
   (let [res (:data (delete-outlet-licenses-with-http-info campaignId optional-params))]
     (if (:decode-models *api-context*)
        (st/decode empty-api-response-spec res st/string-transformer)
        res))))
...
(defn-spec get-outlet-with-http-info any?
...
|-|"
  [campaignId int?, outletId int?]
  (check-required-params campaignId outletId)
  (call-api "/campaigns/{campaignId}/outlets/{outletId}" :get
            {:path-params   {"campaignId" campaignId "outletId" outletId }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec get-outlet get-outlet-response-spec
  "Информация об одной точке продаж
  Возвращает информацию о точках продаж магазина.
...
|-|"
  [campaignId int?, outletId int?]
  (let [res (:data (get-outlet-with-http-info campaignId outletId))]
    (if (:decode-models *api-context*)
       (st/decode get-outlet-response-spec res st/string-transformer)
       res)))


(defn-spec get-outlet-licenses-with-http-info any?
...
|**⚙️ Лимит:** 100 000 запросов в час|
|-|"
  ([campaignId int?, ] (get-outlet-licenses-with-http-info campaignId nil))
  ([campaignId int?, {:keys [outletIds ids]} (s/map-of keyword? any?)]
   (check-required-params campaignId)
   (call-api "/campaigns/{campaignId}/outlets/licenses" :get
             {:path-params   {"campaignId" campaignId }
              :header-params {}
              :query-params  {"outletIds" (with-collection-format outletIds :csv) "ids" (with-collection-format ids :csv) }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["OAuth"]})))

(defn-spec get-outlet-licenses get-outlet-licenses-response-spec
  "Информация о лицензиях для точек продаж
  Возвращает информацию о лицензиях для точек продаж.
...
|**⚙️ Лимит:** 100 000 запросов в час|
|-|"
  ([campaignId int?, ] (get-outlet-licenses campaignId nil))
  ([campaignId int?, optional-params any?]
   (let [res (:data (get-outlet-licenses-with-http-info campaignId optional-params))]
     (if (:decode-models *api-context*)
        (st/decode get-outlet-licenses-response-spec res st/string-transformer)
        res))))


(defn-spec get-outlets-with-http-info any?
...
|**⚙️ Лимит:** 100 000 запросов в час|
|-|"
  ([campaignId int?, ] (get-outlets-with-http-info campaignId nil))
  ([campaignId int?, {:keys [page_token region_id shop_outlet_code regionId]} (s/map-of keyword? any?)]
   (check-required-params campaignId)
   (call-api "/campaigns/{campaignId}/outlets" :get
             {:path-params   {"campaignId" campaignId }
              :header-params {}
              :query-params  {"page_token" page_token "region_id" region_id "shop_outlet_code" shop_outlet_code "regionId" regionId }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["OAuth"]})))

(defn-spec get-outlets get-outlets-response-spec
  "Информация о нескольких точках продаж
  Возвращает список точек продаж магазина.
...
|**⚙️ Лимит:** 100 000 запросов в час|
|-|"
  ([campaignId int?, ] (get-outlets campaignId nil))
  ([campaignId int?, optional-params any?]
   (let [res (:data (get-outlets-with-http-info campaignId optional-params))]
     (if (:decode-models *api-context*)
        (st/decode get-outlets-response-spec res st/string-transformer)
        res))))
...
(defn-spec set-return-decision-with-http-info any?
...
|**⚙️ Лимит:** 10 000 запросов в час|
|-|"
  [campaignId int?, orderId int?, returnId int?, set-return-decision-request set-return-decision-request]
  (check-required-params campaignId orderId returnId set-return-decision-request)
  (call-api "/campaigns/{campaignId}/orders/{orderId}/returns/{returnId}/decision" :post
            {:path-params   {"campaignId" campaignId "orderId" orderId "returnId" returnId }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    set-return-decision-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["OAuth"]}))

(defn-spec set-return-decision empty-api-response-spec
  "Принятие или изменение решения по возврату
  Выбирает решение по возврату от покупателя. После этого для подтверждения решения нужно выполнить запрос [POST campaigns/{campaignId}/orders/{orderId}/returns/{returnId}/decision/submit](../../reference/orders/submitReturnDecision.md).
...
|-|"
  [campaignId int?, orderId int?, returnId int?, set-return-decision-request set-return-decision-request]
  (let [res (:data (set-return-decision-with-http-info campaignId orderId returnId set-return-decision-request))]
    (if (:decode-models *api-context*)
       (st/decode empty-api-response-spec res st/string-transformer)
       res)))
...
(defn-spec submit-return-decision-with-http-info any?
...
|-|"
  [campaignId int?, orderId int?, returnId int?]
  (check-required-params campaignId orderId returnId)
  (call-api "/campaigns/{campaignId}/orders/{orderId}/returns/{returnId}/decision/submit" :post
            {:path-params   {"campaignId" campaignId "orderId" orderId "returnId" returnId }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec submit-return-decision empty-api-response-spec
  "Подтверждение решения по возврату
  Подтверждает выбранное решение по возврату, отправленное в запросе [POST campaigns/{campaignId}/orders/{orderId}/returns/{returnId}/decision](../../reference/orders/setReturnDecision.md).
...
|-|"
  [campaignId int?, orderId int?, returnId int?]
  (let [res (:data (submit-return-decision-with-http-info campaignId orderId returnId))]
    (if (:decode-models *api-context*)
       (st/decode empty-api-response-spec res st/string-transformer)
       res)))
...
(defn-spec update-outlet-with-http-info any?
...
|**⚙️ Лимит:** 100 000 запросов в час|
|-|"
  [campaignId int?, outletId int?, change-outlet-request change-outlet-request]
  (check-required-params campaignId outletId change-outlet-request)
  (call-api "/campaigns/{campaignId}/outlets/{outletId}" :put
            {:path-params   {"campaignId" campaignId "outletId" outletId }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec update-outlet empty-api-response-spec
  "Изменение информации о точке продаж
  Изменяет информацию о точке продаж магазина на Маркете.
...
|-|"
  [campaignId int?, outletId int?, change-outlet-request change-outlet-request]
  (let [res (:data (update-outlet-with-http-info campaignId outletId change-outlet-request))]
    (if (:decode-models *api-context*)
       (st/decode empty-api-response-spec res st/string-transformer)
       res)))


(defn-spec update-outlet-licenses-with-http-info any?
...
|**⚙️ Лимит:** 100 000 запросов в час|
|-|"
  [campaignId int?, update-outlet-license-request update-outlet-license-request]
  (check-required-params campaignId update-outlet-license-request)
  (call-api "/campaigns/{campaignId}/outlets/licenses" :post
            {:path-params   {"campaignId" campaignId }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    update-outlet-license-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["OAuth"]}))

(defn-spec update-outlet-licenses empty-api-response-spec
  "Создание и изменение лицензий для точек продаж
  Передает информацию о новых и существующих лицензиях для точек продаж. Поддерживаются только лицензии на розничную продажу алкоголя.
...
|-|"
  [campaignId int?, update-outlet-license-request update-outlet-license-request]
  (let [res (:data (update-outlet-licenses-with-http-info campaignId update-outlet-license-request))]
    (if (:decode-models *api-context*)
       (st/decode empty-api-response-spec res st/string-transformer)
       res)))
