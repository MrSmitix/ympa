-module(ympa_erlang_server_server).


-define(DEFAULT_LOGIC_HANDLER, ympa_erlang_server_default_logic_handler).
...
get_socket_transport(IP, Port, Options) ->
    ...
    case ympa_erlang_server_utils:get_opt(ssl, Options) of
        SslOpts = [_|_] ->
            {ssl, Opts ++ SslOpts};
        undefined ->
            {tcp, Opts}
...
get_default_dispatch(LogicHandler) ->
    Paths = ympa_erlang_server_router:get_paths(LogicHandler),
