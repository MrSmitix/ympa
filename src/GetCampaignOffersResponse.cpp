...
void
GetCampaignOffersResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new GetCampaignOffersResultDTO();
}

void
GetCampaignOffersResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
GetCampaignOffersResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("GetCampaignOffersResultDTO")) {
			jsonToValue(&result, node, "GetCampaignOffersResultDTO", "GetCampaignOffersResultDTO");
		} else {
			
			GetCampaignOffersResultDTO* obj = static_cast<GetCampaignOffersResultDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
GetCampaignOffersResponse::GetCampaignOffersResponse(char* json)
{
	this->fromJson(json);
}
...
GetCampaignOffersResponse::toJson()
{
	...
	if (isprimitive("GetCampaignOffersResultDTO")) {
		GetCampaignOffersResultDTO obj = getResult();
		node = converttoJson(&obj, "GetCampaignOffersResultDTO", "");
	}
	else {
		
		GetCampaignOffersResultDTO obj = static_cast<GetCampaignOffersResultDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
