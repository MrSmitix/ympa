...
void
DeleteCampaignOffersResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new DeleteCampaignOffersDTO();
}

void
DeleteCampaignOffersResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
DeleteCampaignOffersResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("DeleteCampaignOffersDTO")) {
			jsonToValue(&result, node, "DeleteCampaignOffersDTO", "DeleteCampaignOffersDTO");
		} else {
			
			DeleteCampaignOffersDTO* obj = static_cast<DeleteCampaignOffersDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
DeleteCampaignOffersResponse::DeleteCampaignOffersResponse(char* json)
{
	this->fromJson(json);
}
...
DeleteCampaignOffersResponse::toJson()
{
	...
	if (isprimitive("DeleteCampaignOffersDTO")) {
		DeleteCampaignOffersDTO obj = getResult();
		node = converttoJson(&obj, "DeleteCampaignOffersDTO", "");
	}
	else {
		
		DeleteCampaignOffersDTO obj = static_cast<DeleteCampaignOffersDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
