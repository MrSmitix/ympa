...
void
GetCategoriesResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
GetCategoriesResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("CategoryDTO")) {
			jsonToValue(&result, node, "CategoryDTO", "CategoryDTO");
		} else {
			
			CategoryDTO* obj = static_cast<CategoryDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
GetCategoriesResponse::GetCategoriesResponse(char* json)
{
	this->fromJson(json);
}
...
GetCategoriesResponse::toJson()
{
	...
	if (isprimitive("CategoryDTO")) {
		CategoryDTO obj = getResult();
		node = converttoJson(&obj, "CategoryDTO", "");
	}
	else {
		
		CategoryDTO obj = static_cast<CategoryDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
