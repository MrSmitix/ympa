...
void
GetBidsInfoResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new GetBidsInfoResponseDTO();
}

void
GetBidsInfoResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
GetBidsInfoResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("GetBidsInfoResponseDTO")) {
			jsonToValue(&result, node, "GetBidsInfoResponseDTO", "GetBidsInfoResponseDTO");
		} else {
			
			GetBidsInfoResponseDTO* obj = static_cast<GetBidsInfoResponseDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
GetBidsInfoResponse::GetBidsInfoResponse(char* json)
{
	this->fromJson(json);
}
...
GetBidsInfoResponse::toJson()
{
	...
	if (isprimitive("GetBidsInfoResponseDTO")) {
		GetBidsInfoResponseDTO obj = getResult();
		node = converttoJson(&obj, "GetBidsInfoResponseDTO", "");
	}
	else {
		
		GetBidsInfoResponseDTO obj = static_cast<GetBidsInfoResponseDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
