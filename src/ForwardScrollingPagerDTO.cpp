#include <map>
#include <cstdlib>
#include <glib-object.h>
#include <json-glib/json-glib.h>
#include "Helpers.h"


#include "ForwardScrollingPagerDTO.h"

using namespace std;
using namespace Tizen::ArtikCloud;

ForwardScrollingPagerDTO::ForwardScrollingPagerDTO()
{
	//__init();
}

ForwardScrollingPagerDTO::~ForwardScrollingPagerDTO()
{
	//__cleanup();
}

void
ForwardScrollingPagerDTO::__init()
{
	//nextPageToken = std::string();
}

void
ForwardScrollingPagerDTO::__cleanup()
{
	//if(nextPageToken != NULL) {
	//
	//delete nextPageToken;
	//nextPageToken = NULL;
	//}
	//
}

void
ForwardScrollingPagerDTO::fromJson(char* jsonStr)
{
	JsonObject *pJsonObject = json_node_get_object(json_from_string(jsonStr,NULL));
	JsonNode *node;
	const gchar *nextPageTokenKey = "nextPageToken";
	node = json_object_get_member(pJsonObject, nextPageTokenKey);
	if (node !=NULL) {
	

		if (isprimitive("std::string")) {
			jsonToValue(&nextPageToken, node, "std::string", "");
		} else {
			
		}
	}
}

ForwardScrollingPagerDTO::ForwardScrollingPagerDTO(char* json)
{
	this->fromJson(json);
}

char*
ForwardScrollingPagerDTO::toJson()
{
	JsonObject *pJsonObject = json_object_new();
	JsonNode *node;
	if (isprimitive("std::string")) {
		std::string obj = getNextPageToken();
		node = converttoJson(&obj, "std::string", "");
	}
	else {
		
	}
	const gchar *nextPageTokenKey = "nextPageToken";
	json_object_set_member(pJsonObject, nextPageTokenKey, node);
	node = json_node_alloc();
	json_node_init(node, JSON_NODE_OBJECT);
	json_node_take_object(node, pJsonObject);
	char * ret = json_to_string(node, false);
	json_node_free(node);
	return ret;
}

std::string
ForwardScrollingPagerDTO::getNextPageToken()
{
	return nextPageToken;
}

void
ForwardScrollingPagerDTO::setNextPageToken(std::string  nextPageToken)
{
	this->nextPageToken = nextPageToken;
}


