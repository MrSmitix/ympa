...
#include "DeliveryServicesManager.h"
#include "NetClient.h"
#include "Helpers.h"
#include "Error.h"
...
DeliveryServicesManager::DeliveryServicesManager()
{
...
static gboolean __DeliveryServicesManagerresponseHandler(gpointer data)
{
...
static gpointer __DeliveryServicesManagerthreadFunc(gpointer data)
{
	RequestInfo *request = static_cast<RequestInfo*>(data);
	...
	g_idle_add(__DeliveryServicesManagerresponseHandler, static_cast<gpointer>(request));
	...
	, void* userData, bool isAsync)
{
	...
	if(!isAsync){
		NetClient::easycurl(DeliveryServicesManager::getBasePath(), url, myhttpmethod, queryParams,
		...
		requestInfo = new(nothrow) RequestInfo (DeliveryServicesManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), getDeliveryServicesProcessor);;
		if(requestInfo == NULL)
			return false;

		thread = g_thread_new(NULL, __DeliveryServicesManagerthreadFunc, static_cast<gpointer>(requestInfo));
		return true;
	}
}
...
bool DeliveryServicesManager::getDeliveryServicesAsync(char * accessToken,
...
bool DeliveryServicesManager::getDeliveryServicesSync(char * accessToken,
