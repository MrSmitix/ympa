...
void
GetFeedbackListResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new FeedbackListDTO();
}

void
GetFeedbackListResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
GetFeedbackListResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("FeedbackListDTO")) {
			jsonToValue(&result, node, "FeedbackListDTO", "FeedbackListDTO");
		} else {
			
			FeedbackListDTO* obj = static_cast<FeedbackListDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
GetFeedbackListResponse::GetFeedbackListResponse(char* json)
{
	this->fromJson(json);
}
...
GetFeedbackListResponse::toJson()
{
	...
	if (isprimitive("FeedbackListDTO")) {
		FeedbackListDTO obj = getResult();
		node = converttoJson(&obj, "FeedbackListDTO", "");
	}
	else {
		
		FeedbackListDTO obj = static_cast<FeedbackListDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
