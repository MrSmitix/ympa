...
void
GetShipmentResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("ShipmentDTO")) {
			jsonToValue(&result, node, "ShipmentDTO", "ShipmentDTO");
		} else {
			
			ShipmentDTO* obj = static_cast<ShipmentDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
GetShipmentResponse::GetShipmentResponse(char* json)
{
	this->fromJson(json);
}
...
GetShipmentResponse::toJson()
{
	...
	if (isprimitive("ShipmentDTO")) {
		ShipmentDTO obj = getResult();
		node = converttoJson(&obj, "ShipmentDTO", "");
	}
	else {
		
		ShipmentDTO obj = static_cast<ShipmentDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
