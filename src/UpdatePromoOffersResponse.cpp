...
void
UpdatePromoOffersResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new UpdatePromoOffersResultDTO();
}

void
UpdatePromoOffersResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
UpdatePromoOffersResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("UpdatePromoOffersResultDTO")) {
			jsonToValue(&result, node, "UpdatePromoOffersResultDTO", "UpdatePromoOffersResultDTO");
		} else {
			
			UpdatePromoOffersResultDTO* obj = static_cast<UpdatePromoOffersResultDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
UpdatePromoOffersResponse::UpdatePromoOffersResponse(char* json)
{
	this->fromJson(json);
}
...
UpdatePromoOffersResponse::toJson()
{
	...
	if (isprimitive("UpdatePromoOffersResultDTO")) {
		UpdatePromoOffersResultDTO obj = getResult();
		node = converttoJson(&obj, "UpdatePromoOffersResultDTO", "");
	}
	else {
		
		UpdatePromoOffersResultDTO obj = static_cast<UpdatePromoOffersResultDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
