...
public class TariffsApi  {
   private final TariffsApiService delegate;

   public TariffsApi(@Context ServletConfig servletContext) {
      TariffsApiService delegate = null;

      if (servletContext != null) {
         String implClass = servletContext.getInitParameter("TariffsApi.implementation");
         if (implClass != null && !"".equals(implClass.trim())) {
            try {
               delegate = (TariffsApiService) Class.forName(implClass).getDeclaredConstructor().newInstance();
            } catch (Exception e) {
      ...
      if (delegate == null) {
         delegate = TariffsApiServiceFactory.getTariffsApi();
      }
    ...
    public Response calculateTariffs(@ApiParam(value = "", required = true) @NotNull @Valid  CalculateTariffsRequest calculateTariffsRequest,@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.calculateTariffs(calculateTariffsRequest, securityContext);
    }
}
