...
void
CalculateTariffsResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new CalculateTariffsResponseDTO();
}

void
CalculateTariffsResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
CalculateTariffsResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("CalculateTariffsResponseDTO")) {
			jsonToValue(&result, node, "CalculateTariffsResponseDTO", "CalculateTariffsResponseDTO");
		} else {
			
			CalculateTariffsResponseDTO* obj = static_cast<CalculateTariffsResponseDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
CalculateTariffsResponse::CalculateTariffsResponse(char* json)
{
	this->fromJson(json);
}
...
CalculateTariffsResponse::toJson()
{
	...
	if (isprimitive("CalculateTariffsResponseDTO")) {
		CalculateTariffsResponseDTO obj = getResult();
		node = converttoJson(&obj, "CalculateTariffsResponseDTO", "");
	}
	else {
		
		CalculateTariffsResponseDTO obj = static_cast<CalculateTariffsResponseDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
