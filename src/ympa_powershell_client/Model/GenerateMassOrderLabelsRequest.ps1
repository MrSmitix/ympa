...
GenerateMassOrderLabelsRequest<PSCustomObject>
#>

function Initialize-GenerateMassOrderLabelsRequest {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: ympa_powershell_client => GenerateMassOrderLabelsRequest' | Write-Debug
        ...
        if ($OrderIds.length -gt 1000) {
            throw "invalid value for 'OrderIds', number of items must be less than or equal to 1000."
        }
...
Convert from JSON to GenerateMassOrderLabelsRequest<PSCustomObject>
...
GenerateMassOrderLabelsRequest<PSCustomObject>
#>
function ConvertFrom-JsonToGenerateMassOrderLabelsRequest {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: ympa_powershell_client => GenerateMassOrderLabelsRequest' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in GenerateMassOrderLabelsRequest
        $AllProperties = ("businessId", "orderIds")
