...
.PARAMETER SellingPrograms
Информация о том, какие для товара доступны модели размещения. 
.PARAMETER Archived
Товар помещен в архив. 
.OUTPUTS

GetOfferDTO<PSCustomObject>
#>

function Initialize-GetOfferDTO {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: ympa_powershell_client => GetOfferDTO' | Write-Debug
...
Convert from JSON to GetOfferDTO<PSCustomObject>
...
GetOfferDTO<PSCustomObject>
#>
function ConvertFrom-JsonToGetOfferDTO {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: ympa_powershell_client => GetOfferDTO' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in GetOfferDTO
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "sellingPrograms"))) { #optional property not found
            $SellingPrograms = $null
        } else {
            $SellingPrograms = $JsonParameters.PSobject.Properties["sellingPrograms"].value
        }
