...
class DbsApi(
    ...
    dbsMarshaller: DbsApiMarshaller
)  extends MultipartDirectives with StringDirectives {

  
  import dbsMarshaller._
  ...
  lazy val route: Route =
    path("campaigns" / LongNumber / "orders" / LongNumber / "cancellation" / "accept") { (campaignId, orderId) => 
      put {  
            entity(as[AcceptOrderCancellationRequest]){ acceptOrderCancellationRequest =>
              dbsService.acceptOrderCancellation(campaignId = campaignId, orderId = orderId, acceptOrderCancellationRequest = acceptOrderCancellationRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "hidden-offers") { (campaignId) => 
      post {  
            entity(as[AddHiddenOffersRequest]){ addHiddenOffersRequest =>
              dbsService.addHiddenOffers(campaignId = campaignId, addHiddenOffersRequest = addHiddenOffersRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "offer-mappings" / "archive") { (businessId) => 
      post {  
            entity(as[AddOffersToArchiveRequest]){ addOffersToArchiveRequest =>
              dbsService.addOffersToArchive(businessId = businessId, addOffersToArchiveRequest = addOffersToArchiveRequest)
            }
      }
    } ~
    path("tariffs" / "calculate") { 
      post {  
            entity(as[CalculateTariffsRequest]){ calculateTariffsRequest =>
              dbsService.calculateTariffs(calculateTariffsRequest = calculateTariffsRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "price-quarantine" / "confirm") { (businessId) => 
      post {  
            entity(as[ConfirmPricesRequest]){ confirmPricesRequest =>
              dbsService.confirmBusinessPrices(businessId = businessId, confirmPricesRequest = confirmPricesRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "price-quarantine" / "confirm") { (campaignId) => 
      post {  
            entity(as[ConfirmPricesRequest]){ confirmPricesRequest =>
              dbsService.confirmCampaignPrices(campaignId = campaignId, confirmPricesRequest = confirmPricesRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "chats" / "new") { (businessId) => 
      post {  
            entity(as[CreateChatRequest]){ createChatRequest =>
              dbsService.createChat(businessId = businessId, createChatRequest = createChatRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "outlets") { (campaignId) => 
      post {  
            entity(as[ChangeOutletRequest]){ changeOutletRequest =>
              dbsService.createOutlet(campaignId = campaignId, changeOutletRequest = changeOutletRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "offers" / "delete") { (campaignId) => 
      post {  
            entity(as[DeleteCampaignOffersRequest]){ deleteCampaignOffersRequest =>
              dbsService.deleteCampaignOffers(campaignId = campaignId, deleteCampaignOffersRequest = deleteCampaignOffersRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "goods-feedback" / "comments" / "delete") { (businessId) => 
      post {  
            entity(as[DeleteGoodsFeedbackCommentRequest]){ deleteGoodsFeedbackCommentRequest =>
              dbsService.deleteGoodsFeedbackComment(businessId = businessId, deleteGoodsFeedbackCommentRequest = deleteGoodsFeedbackCommentRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "hidden-offers" / "delete") { (campaignId) => 
      post {  
            entity(as[DeleteHiddenOffersRequest]){ deleteHiddenOffersRequest =>
              dbsService.deleteHiddenOffers(campaignId = campaignId, deleteHiddenOffersRequest = deleteHiddenOffersRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "offer-mappings" / "delete") { (businessId) => 
      post {  
            entity(as[DeleteOffersRequest]){ deleteOffersRequest =>
              dbsService.deleteOffers(businessId = businessId, deleteOffersRequest = deleteOffersRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "offer-mappings" / "unarchive") { (businessId) => 
      post {  
            entity(as[DeleteOffersFromArchiveRequest]){ deleteOffersFromArchiveRequest =>
              dbsService.deleteOffersFromArchive(businessId = businessId, deleteOffersFromArchiveRequest = deleteOffersFromArchiveRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "outlets" / LongNumber) { (campaignId, outletId) => 
      delete {  
            dbsService.deleteOutlet(campaignId = campaignId, outletId = outletId)
      }
    } ~
    path("campaigns" / LongNumber / "outlets" / "licenses") { (campaignId) => 
      delete { 
        parameters("ids".as[String].?) { (ids) => 
            dbsService.deleteOutletLicenses(campaignId = campaignId, ids = ids)
        }
      }
    } ~
    path("businesses" / LongNumber / "promos" / "offers" / "delete") { (businessId) => 
      post {  
            entity(as[DeletePromoOffersRequest]){ deletePromoOffersRequest =>
              dbsService.deletePromoOffers(businessId = businessId, deletePromoOffersRequest = deletePromoOffersRequest)
            }
      }
    } ~
      ...
      post { 
        parameters("format".as[String].?) { (format) => 
            entity(as[GenerateBoostConsolidatedRequest]){ generateBoostConsolidatedRequest =>
              dbsService.generateBoostConsolidatedReport(generateBoostConsolidatedRequest = generateBoostConsolidatedRequest, format = format)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("format".as[String].?) { (format) => 
            entity(as[GenerateCompetitorsPositionReportRequest]){ generateCompetitorsPositionReportRequest =>
              dbsService.generateCompetitorsPositionReport(generateCompetitorsPositionReportRequest = generateCompetitorsPositionReportRequest, format = format)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("format".as[String].?) { (format) => 
            entity(as[GenerateGoodsFeedbackRequest]){ generateGoodsFeedbackRequest =>
              dbsService.generateGoodsFeedbackReport(generateGoodsFeedbackRequest = generateGoodsFeedbackRequest, format = format)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("format".as[String].?) { (format) => 
            entity(as[GenerateMassOrderLabelsRequest]){ generateMassOrderLabelsRequest =>
              dbsService.generateMassOrderLabelsReport(generateMassOrderLabelsRequest = generateMassOrderLabelsRequest, format = format)
            }
        }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "delivery" / "shipments" / LongNumber / "boxes" / LongNumber / "label") { (campaignId, orderId, shipmentId, boxId) => 
      get { 
        parameters("format".as[String].?) { (format) => 
            dbsService.generateOrderLabel(campaignId = campaignId, orderId = orderId, shipmentId = shipmentId, boxId = boxId, format = format)
        }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "delivery" / "labels") { (campaignId, orderId) => 
      get { 
        parameters("format".as[String].?) { (format) => 
            dbsService.generateOrderLabels(campaignId = campaignId, orderId = orderId, format = format)
        }
      }
    } ~
      ...
      post { 
        parameters("format".as[String].?) { (format) => 
            entity(as[GeneratePricesReportRequest]){ generatePricesReportRequest =>
              dbsService.generatePricesReport(generatePricesReportRequest = generatePricesReportRequest, format = format)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("format".as[String].?) { (format) => 
            entity(as[GenerateShelfsStatisticsRequest]){ generateShelfsStatisticsRequest =>
              dbsService.generateShelfsStatisticsReport(generateShelfsStatisticsRequest = generateShelfsStatisticsRequest, format = format)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("format".as[String].?) { (format) => 
            entity(as[GenerateShowsSalesReportRequest]){ generateShowsSalesReportRequest =>
              dbsService.generateShowsSalesReport(generateShowsSalesReportRequest = generateShowsSalesReportRequest, format = format)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("format".as[String].?) { (format) => 
            entity(as[GenerateStocksOnWarehousesReportRequest]){ generateStocksOnWarehousesReportRequest =>
              dbsService.generateStocksOnWarehousesReport(generateStocksOnWarehousesReportRequest = generateStocksOnWarehousesReportRequest, format = format)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("format".as[String].?) { (format) => 
            entity(as[GenerateUnitedMarketplaceServicesReportRequest]){ generateUnitedMarketplaceServicesReportRequest =>
              dbsService.generateUnitedMarketplaceServicesReport(generateUnitedMarketplaceServicesReportRequest = generateUnitedMarketplaceServicesReportRequest, format = format)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("format".as[String].?) { (format) => 
            entity(as[GenerateUnitedNettingReportRequest]){ generateUnitedNettingReportRequest =>
              dbsService.generateUnitedNettingReport(generateUnitedNettingReportRequest = generateUnitedNettingReportRequest, format = format)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("format".as[String].?) { (format) => 
            entity(as[GenerateUnitedOrdersRequest]){ generateUnitedOrdersRequest =>
              dbsService.generateUnitedOrdersReport(generateUnitedOrdersRequest = generateUnitedOrdersRequest, format = format)
            }
        }
      }
    } ~
    path("campaigns" / LongNumber / "offers" / "all") { (campaignId) => 
      get { 
        parameters("feedId".as[Long].?, "chunk".as[Int].?) { (feedId, chunk) => 
            dbsService.getAllOffers(campaignId = campaignId, feedId = feedId, chunk = chunk)
        }
      }
    } ~
      ...
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetBidsInfoRequest]){ getBidsInfoRequest =>
              dbsService.getBidsInfoForBusiness(businessId = businessId, pageToken = pageToken, limit = limit, getBidsInfoRequest = getBidsInfoRequest)
            }
        }
      }
    } ~
    path("businesses" / LongNumber / "bids" / "recommendations") { (businessId) => 
      post {  
            entity(as[GetBidsRecommendationsRequest]){ getBidsRecommendationsRequest =>
              dbsService.getBidsRecommendations(businessId = businessId, getBidsRecommendationsRequest = getBidsRecommendationsRequest)
            }
      }
    } ~
      ...
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetQuarantineOffersRequest]){ getQuarantineOffersRequest =>
              dbsService.getBusinessQuarantineOffers(businessId = businessId, getQuarantineOffersRequest = getQuarantineOffersRequest, pageToken = pageToken, limit = limit)
            }
        }
      }
    } ~
    path("businesses" / LongNumber / "settings") { (businessId) => 
      post {  
            dbsService.getBusinessSettings(businessId = businessId)
      }
    } ~
    path("campaigns" / LongNumber) { (campaignId) => 
      get {  
            dbsService.getCampaign(campaignId = campaignId)
      }
    } ~
    path("campaigns" / LongNumber / "logins") { (campaignId) => 
      get {  
            dbsService.getCampaignLogins(campaignId = campaignId)
      }
    } ~
      ...
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetCampaignOffersRequest]){ getCampaignOffersRequest =>
              dbsService.getCampaignOffers(campaignId = campaignId, getCampaignOffersRequest = getCampaignOffersRequest, pageToken = pageToken, limit = limit)
            }
        }
      }
    } ~
    path("campaigns" / LongNumber / "price-quarantine") { (campaignId) => 
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetQuarantineOffersRequest]){ getQuarantineOffersRequest =>
              dbsService.getCampaignQuarantineOffers(campaignId = campaignId, getQuarantineOffersRequest = getQuarantineOffersRequest, pageToken = pageToken, limit = limit)
            }
        }
      }
    } ~
    path("campaigns" / LongNumber / "region") { (campaignId) => 
      get {  
            dbsService.getCampaignRegion(campaignId = campaignId)
      }
    } ~
    path("campaigns" / LongNumber / "settings") { (campaignId) => 
      get {  
            dbsService.getCampaignSettings(campaignId = campaignId)
      }
    } ~
    path("campaigns") { 
      get { 
        parameters("page".as[Int].?(1), "pageSize".as[Int].?) { (page, pageSize) => 
            dbsService.getCampaigns(page = page, pageSize = pageSize)
        }
      }
    } ~
    path("campaigns" / "by_login" / Segment) { (login) => 
      get { 
        parameters("page".as[Int].?(1), "pageSize".as[Int].?) { (page, pageSize) => 
            dbsService.getCampaignsByLogin(login = login, page = page, pageSize = pageSize)
        }
      }
    } ~
    path("categories" / "max-sale-quantum") { 
      post {  
            entity(as[GetCategoriesMaxSaleQuantumRequest]){ getCategoriesMaxSaleQuantumRequest =>
              dbsService.getCategoriesMaxSaleQuantum(getCategoriesMaxSaleQuantumRequest = getCategoriesMaxSaleQuantumRequest)
            }
      }
    } ~
    path("categories" / "tree") { 
      post {  
            entity(as[GetCategoriesRequest]){ getCategoriesRequest =>
              dbsService.getCategoriesTree(getCategoriesRequest = getCategoriesRequest)
            }
      }
    } ~
    path("category" / LongNumber / "parameters") { (categoryId) => 
      post {  
            dbsService.getCategoryContentParameters(categoryId = categoryId)
      }
    } ~
      ...
      post { 
        parameters("chatId".as[Long], "page_token".as[String].?, "limit".as[Int].?) { (chatId, pageToken, limit) => 
            entity(as[GetChatHistoryRequest]){ getChatHistoryRequest =>
              dbsService.getChatHistory(businessId = businessId, chatId = chatId, getChatHistoryRequest = getChatHistoryRequest, pageToken = pageToken, limit = limit)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetChatsRequest]){ getChatsRequest =>
              dbsService.getChats(businessId = businessId, getChatsRequest = getChatsRequest, pageToken = pageToken, limit = limit)
            }
        }
      }
    } ~
    path("delivery" / "services") { 
      get {  
            dbsService.getDeliveryServices()
      }
    } ~
    path("campaigns" / LongNumber / "feeds" / LongNumber) { (campaignId, feedId) => 
      get {  
            dbsService.getFeed(campaignId = campaignId, feedId = feedId)
      }
    } ~
    path("campaigns" / LongNumber / "feeds" / LongNumber / "index-logs") { (campaignId, feedId) => 
      get { 
        parameters("limit".as[Int].?, "published_time_from".as[String].?, "published_time_to".as[String].?, "status".as[String].?) { (limit, publishedTimeFrom, publishedTimeTo, status) => 
            dbsService.getFeedIndexLogs(campaignId = campaignId, feedId = feedId, limit = limit, publishedTimeFrom = publishedTimeFrom, publishedTimeTo = publishedTimeTo, status = status)
        }
      }
    } ~
    path("campaigns" / LongNumber / "feedback" / "updates") { (campaignId) => 
      get { 
        parameters("page_token".as[String].?, "limit".as[Int].?, "from_date".as[String].?) { (pageToken, limit, fromDate) => 
            dbsService.getFeedbackAndCommentUpdates(campaignId = campaignId, pageToken = pageToken, limit = limit, fromDate = fromDate)
        }
      }
    } ~
    path("campaigns" / LongNumber / "feeds") { (campaignId) => 
      get {  
            dbsService.getFeeds(campaignId = campaignId)
      }
    } ~
      ...
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetGoodsFeedbackCommentsRequest]){ getGoodsFeedbackCommentsRequest =>
              dbsService.getGoodsFeedbackComments(businessId = businessId, getGoodsFeedbackCommentsRequest = getGoodsFeedbackCommentsRequest, pageToken = pageToken, limit = limit)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetGoodsFeedbackRequest]){ getGoodsFeedbackRequest =>
              dbsService.getGoodsFeedbacks(businessId = businessId, pageToken = pageToken, limit = limit, getGoodsFeedbackRequest = getGoodsFeedbackRequest)
            }
        }
      }
    } ~
    path("campaigns" / LongNumber / "stats" / "skus") { (campaignId) => 
      post {  
            entity(as[GetGoodsStatsRequest]){ getGoodsStatsRequest =>
              dbsService.getGoodsStats(campaignId = campaignId, getGoodsStatsRequest = getGoodsStatsRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "hidden-offers") { (campaignId) => 
      get { 
        parameters("offer_id".as[String].?, "page_token".as[String].?, "limit".as[Int].?, "offset".as[Int].?, "page".as[Int].?(1), "pageSize".as[Int].?) { (offerId, pageToken, limit, offset, page, pageSize) => 
            dbsService.getHiddenOffers(campaignId = campaignId, offerId = offerId, pageToken = pageToken, limit = limit, offset = offset, page = page, pageSize = pageSize)
        }
      }
    } ~
    path("models" / LongNumber) { (modelId) => 
      get { 
        parameters("regionId".as[Long], "currency".as[String].?) { (regionId, currency) => 
            dbsService.getModel(modelId = modelId, regionId = regionId, currency = currency)
        }
      }
    } ~
    path("models" / LongNumber / "offers") { (modelId) => 
      get { 
        parameters("regionId".as[Long], "currency".as[String].?, "orderByPrice".as[String].?, "count".as[Int].?(10), "page".as[Int].?(1)) { (regionId, currency, orderByPrice, count, page) => 
            dbsService.getModelOffers(modelId = modelId, regionId = regionId, currency = currency, orderByPrice = orderByPrice, count = count, page = page)
        }
      }
    } ~
    path("models") { 
      post { 
        parameters("regionId".as[Long], "currency".as[String].?) { (regionId, currency) => 
            entity(as[GetModelsRequest]){ getModelsRequest =>
              dbsService.getModels(regionId = regionId, getModelsRequest = getModelsRequest, currency = currency)
            }
        }
      }
    } ~
    path("models" / "offers") { 
      post { 
        parameters("regionId".as[Long], "currency".as[String].?, "orderByPrice".as[String].?) { (regionId, currency, orderByPrice) => 
            entity(as[GetModelsRequest]){ getModelsRequest =>
              dbsService.getModelsOffers(regionId = regionId, getModelsRequest = getModelsRequest, currency = currency, orderByPrice = orderByPrice)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetOfferCardsContentStatusRequest]){ getOfferCardsContentStatusRequest =>
              dbsService.getOfferCardsContentStatus(businessId = businessId, pageToken = pageToken, limit = limit, getOfferCardsContentStatusRequest = getOfferCardsContentStatusRequest)
            }
        }
      }
    } ~
    path("campaigns" / LongNumber / "offer-mapping-entries") { (campaignId) => 
      get { 
        parameters("offer_id".as[String].?, "shop_sku".as[String].?, "mapping_kind".as[String].?, "status".as[String].?, "availability".as[String].?, "category_id".as[String].?, "vendor".as[String].?, "page_token".as[String].?, "limit".as[Int].?) { (offerId, shopSku, mappingKind, status, availability, categoryId, vendor, pageToken, limit) => 
            dbsService.getOfferMappingEntries(campaignId = campaignId, offerId = offerId, shopSku = shopSku, mappingKind = mappingKind, status = status, availability = availability, categoryId = categoryId, vendor = vendor, pageToken = pageToken, limit = limit)
        }
      }
    } ~
      ...
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetOfferMappingsRequest]){ getOfferMappingsRequest =>
              dbsService.getOfferMappings(businessId = businessId, pageToken = pageToken, limit = limit, getOfferMappingsRequest = getOfferMappingsRequest)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetOfferRecommendationsRequest]){ getOfferRecommendationsRequest =>
              dbsService.getOfferRecommendations(businessId = businessId, getOfferRecommendationsRequest = getOfferRecommendationsRequest, pageToken = pageToken, limit = limit)
            }
        }
      }
    } ~
    path("campaigns" / LongNumber / "offers") { (campaignId) => 
      get { 
        parameters("query".as[String].?, "feedId".as[Long].?, "shopCategoryId".as[String].?, "currency".as[String].?, "matched".as[Boolean].?, "page".as[Int].?(1), "pageSize".as[Int].?) { (query, feedId, shopCategoryId, currency, matched, page, pageSize) => 
            dbsService.getOffers(campaignId = campaignId, query = query, feedId = feedId, shopCategoryId = shopCategoryId, currency = currency, matched = matched, page = page, pageSize = pageSize)
        }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber) { (campaignId, orderId) => 
      get {  
            dbsService.getOrder(campaignId = campaignId, orderId = orderId)
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "business-buyer") { (campaignId, orderId) => 
      post {  
            dbsService.getOrderBusinessBuyerInfo(campaignId = campaignId, orderId = orderId)
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "documents") { (campaignId, orderId) => 
      post {  
            dbsService.getOrderBusinessDocumentsInfo(campaignId = campaignId, orderId = orderId)
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "buyer") { (campaignId, orderId) => 
      get {  
            dbsService.getOrderBuyerInfo(campaignId = campaignId, orderId = orderId)
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "delivery" / "labels" / "data") { (campaignId, orderId) => 
      get {  
            dbsService.getOrderLabelsData(campaignId = campaignId, orderId = orderId)
      }
    } ~
      ...
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetOrdersStatsRequest]){ getOrdersStatsRequest =>
              dbsService.getOrdersStats(campaignId = campaignId, pageToken = pageToken, limit = limit, getOrdersStatsRequest = getOrdersStatsRequest)
            }
        }
      }
    } ~
    path("campaigns" / LongNumber / "outlets" / LongNumber) { (campaignId, outletId) => 
      get {  
            dbsService.getOutlet(campaignId = campaignId, outletId = outletId)
      }
    } ~
    path("campaigns" / LongNumber / "outlets" / "licenses") { (campaignId) => 
      get { 
        parameters("outletIds".as[String].?, "ids".as[String].?) { (outletIds, ids) => 
            dbsService.getOutletLicenses(campaignId = campaignId, outletIds = outletIds, ids = ids)
        }
      }
    } ~
    path("campaigns" / LongNumber / "outlets") { (campaignId) => 
      get { 
        parameters("page_token".as[String].?, "region_id".as[Long].?, "shop_outlet_code".as[String].?, "regionId".as[Long].?) { (pageToken, regionId, shopOutletCode, regionId2) => 
            dbsService.getOutlets(campaignId = campaignId, pageToken = pageToken, regionId = regionId, shopOutletCode = shopOutletCode, regionId2 = regionId2)
        }
      }
    } ~
    path("campaigns" / LongNumber / "offer-prices") { (campaignId) => 
      get { 
        parameters("page_token".as[String].?, "limit".as[Int].?, "archived".as[Boolean].?(false)) { (pageToken, limit, archived) => 
            dbsService.getPrices(campaignId = campaignId, pageToken = pageToken, limit = limit, archived = archived)
        }
      }
    } ~
    path("campaigns" / LongNumber / "offer-prices") { (campaignId) => 
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetPricesByOfferIdsRequest]){ getPricesByOfferIdsRequest =>
              dbsService.getPricesByOfferIds(campaignId = campaignId, pageToken = pageToken, limit = limit, getPricesByOfferIdsRequest = getPricesByOfferIdsRequest)
            }
        }
      }
    } ~
      ...
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetPromoOffersRequest]){ getPromoOffersRequest =>
              dbsService.getPromoOffers(businessId = businessId, getPromoOffersRequest = getPromoOffersRequest, pageToken = pageToken, limit = limit)
            }
        }
      }
    } ~
    path("businesses" / LongNumber / "promos") { (businessId) => 
      post {  
            entity(as[GetPromosRequest]){ getPromosRequest =>
              dbsService.getPromos(businessId = businessId, getPromosRequest = getPromosRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "ratings" / "quality" / "details") { (campaignId) => 
      post {  
            dbsService.getQualityRatingDetails(campaignId = campaignId)
      }
    } ~
    path("businesses" / LongNumber / "ratings" / "quality") { (businessId) => 
      post {  
            entity(as[GetQualityRatingRequest]){ getQualityRatingRequest =>
              dbsService.getQualityRatings(businessId = businessId, getQualityRatingRequest = getQualityRatingRequest)
            }
      }
    } ~
    path("reports" / "info" / Segment) { (reportId) => 
      get {  
            dbsService.getReportInfo(reportId = reportId)
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "returns" / LongNumber) { (campaignId, orderId, returnId) => 
      get {  
            dbsService.getReturn(campaignId = campaignId, orderId = orderId, returnId = returnId)
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "returns" / LongNumber / "application") { (campaignId, orderId, returnId) => 
      get {  
            dbsService.getReturnApplication(campaignId = campaignId, orderId = orderId, returnId = returnId)
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "returns" / LongNumber / "decision" / LongNumber / "image" / Segment) { (campaignId, orderId, returnId, itemId, imageHash) => 
      get {  
            dbsService.getReturnPhoto(campaignId = campaignId, orderId = orderId, returnId = returnId, itemId = itemId, imageHash = imageHash)
      }
    } ~
    path("campaigns" / LongNumber / "returns") { (campaignId) => 
      get { 
        parameters("page_token".as[String].?, "limit".as[Int].?, "orderIds".as[String].?, "statuses".as[String].?, "type".as[String].?, "fromDate".as[String].?, "toDate".as[String].?, "from_date".as[String].?, "to_date".as[String].?) { (pageToken, limit, orderIds, statuses, `type`, fromDate, toDate, fromDate2, toDate2) => 
            dbsService.getReturns(campaignId = campaignId, pageToken = pageToken, limit = limit, orderIds = orderIds, statuses = statuses, `type` = `type`, fromDate = fromDate, toDate = toDate, fromDate2 = fromDate2, toDate2 = toDate2)
        }
      }
    } ~
      ...
      post { 
        parameters("page_token".as[String].?, "limit".as[Int].?) { (pageToken, limit) => 
            entity(as[GetWarehouseStocksRequest]){ getWarehouseStocksRequest =>
              dbsService.getStocks(campaignId = campaignId, pageToken = pageToken, limit = limit, getWarehouseStocksRequest = getWarehouseStocksRequest)
            }
        }
      }
    } ~
    path("campaigns" / LongNumber / "offer-mapping-entries" / "suggestions") { (campaignId) => 
      post {  
            entity(as[GetSuggestedOfferMappingEntriesRequest]){ getSuggestedOfferMappingEntriesRequest =>
              dbsService.getSuggestedOfferMappingEntries(campaignId = campaignId, getSuggestedOfferMappingEntriesRequest = getSuggestedOfferMappingEntriesRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "offer-mappings" / "suggestions") { (businessId) => 
      post {  
            entity(as[GetSuggestedOfferMappingsRequest]){ getSuggestedOfferMappingsRequest =>
              dbsService.getSuggestedOfferMappings(businessId = businessId, getSuggestedOfferMappingsRequest = getSuggestedOfferMappingsRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "offer-prices" / "suggestions") { (campaignId) => 
      post {  
            entity(as[SuggestPricesRequest]){ suggestPricesRequest =>
              dbsService.getSuggestedPrices(campaignId = campaignId, suggestPricesRequest = suggestPricesRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "warehouses") { (businessId) => 
      get {  
            dbsService.getWarehouses(businessId = businessId)
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "deliverDigitalGoods") { (campaignId, orderId) => 
      post {  
            entity(as[ProvideOrderDigitalCodesRequest]){ provideOrderDigitalCodesRequest =>
              dbsService.provideOrderDigitalCodes(campaignId = campaignId, orderId = orderId, provideOrderDigitalCodesRequest = provideOrderDigitalCodesRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "identifiers") { (campaignId, orderId) => 
      put {  
            entity(as[ProvideOrderItemIdentifiersRequest]){ provideOrderItemIdentifiersRequest =>
              dbsService.provideOrderItemIdentifiers(campaignId = campaignId, orderId = orderId, provideOrderItemIdentifiersRequest = provideOrderItemIdentifiersRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "bids") { (businessId) => 
      put {  
            entity(as[PutSkuBidsRequest]){ putSkuBidsRequest =>
              dbsService.putBidsForBusiness(businessId = businessId, putSkuBidsRequest = putSkuBidsRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "bids") { (campaignId) => 
      put {  
            entity(as[PutSkuBidsRequest]){ putSkuBidsRequest =>
              dbsService.putBidsForCampaign(campaignId = campaignId, putSkuBidsRequest = putSkuBidsRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "feeds" / LongNumber / "refresh") { (campaignId, feedId) => 
      post {  
            dbsService.refreshFeed(campaignId = campaignId, feedId = feedId)
      }
    } ~
    path("models") { 
      get { 
        parameters("query".as[String], "regionId".as[Long], "currency".as[String].?, "page".as[Int].?(1), "pageSize".as[Int].?) { (query, regionId, currency, page, pageSize) => 
            dbsService.searchModels(query = query, regionId = regionId, currency = currency, page = page, pageSize = pageSize)
        }
      }
    } ~
    path("regions" / LongNumber / "children") { (regionId) => 
      get { 
        parameters("page".as[Int].?(1), "pageSize".as[Int].?) { (page, pageSize) => 
            dbsService.searchRegionChildren(regionId = regionId, page = page, pageSize = pageSize)
        }
      }
    } ~
    path("regions" / LongNumber) { (regionId) => 
      get {  
            dbsService.searchRegionsById(regionId = regionId)
      }
    } ~
    path("regions") { 
      get { 
        parameters("name".as[String], "page_token".as[String].?, "limit".as[Int].?) { (name, pageToken, limit) => 
            dbsService.searchRegionsByName(name = name, pageToken = pageToken, limit = limit)
        }
      }
    } ~
        ...
        formAndFiles(FileField("file")),  { partsAndFiles => 
          val _____ : Try[Route] = for {
            ...
            implicit val vp: StringValueProvider = partsAndFiles.form
              dbsService.sendFileToChat(businessId = businessId, chatId = chatId, file = file)
            }
          }
      ...
      post { 
        parameters("chatId".as[Long]) { (chatId) => 
            entity(as[SendMessageToChatRequest]){ sendMessageToChatRequest =>
              dbsService.sendMessageToChat(businessId = businessId, chatId = chatId, sendMessageToChatRequest = sendMessageToChatRequest)
            }
        }
      }
    } ~
    path("campaigns" / LongNumber / "feeds" / LongNumber / "params") { (campaignId, feedId) => 
      post {  
            entity(as[SetFeedParamsRequest]){ setFeedParamsRequest =>
              dbsService.setFeedParams(campaignId = campaignId, feedId = feedId, setFeedParamsRequest = setFeedParamsRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "boxes") { (campaignId, orderId) => 
      put {  
            entity(as[SetOrderBoxLayoutRequest]){ setOrderBoxLayoutRequest =>
              dbsService.setOrderBoxLayout(campaignId = campaignId, orderId = orderId, setOrderBoxLayoutRequest = setOrderBoxLayoutRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "delivery" / "date") { (campaignId, orderId) => 
      put {  
            entity(as[SetOrderDeliveryDateRequest]){ setOrderDeliveryDateRequest =>
              dbsService.setOrderDeliveryDate(campaignId = campaignId, orderId = orderId, setOrderDeliveryDateRequest = setOrderDeliveryDateRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "delivery" / "track") { (campaignId, orderId) => 
      post {  
            entity(as[SetOrderDeliveryTrackCodeRequest]){ setOrderDeliveryTrackCodeRequest =>
              dbsService.setOrderDeliveryTrackCode(campaignId = campaignId, orderId = orderId, setOrderDeliveryTrackCodeRequest = setOrderDeliveryTrackCodeRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "delivery" / "shipments" / LongNumber / "boxes") { (campaignId, orderId, shipmentId) => 
      put {  
            entity(as[SetOrderShipmentBoxesRequest]){ setOrderShipmentBoxesRequest =>
              dbsService.setOrderShipmentBoxes(campaignId = campaignId, orderId = orderId, shipmentId = shipmentId, setOrderShipmentBoxesRequest = setOrderShipmentBoxesRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "returns" / LongNumber / "decision") { (campaignId, orderId, returnId) => 
      post {  
            entity(as[SetReturnDecisionRequest]){ setReturnDecisionRequest =>
              dbsService.setReturnDecision(campaignId = campaignId, orderId = orderId, returnId = returnId, setReturnDecisionRequest = setReturnDecisionRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "goods-feedback" / "skip-reaction") { (businessId) => 
      post {  
            entity(as[SkipGoodsFeedbackReactionRequest]){ skipGoodsFeedbackReactionRequest =>
              dbsService.skipGoodsFeedbacksReaction(businessId = businessId, skipGoodsFeedbackReactionRequest = skipGoodsFeedbackReactionRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "returns" / LongNumber / "decision" / "submit") { (campaignId, orderId, returnId) => 
      post {  
            dbsService.submitReturnDecision(campaignId = campaignId, orderId = orderId, returnId = returnId)
      }
    } ~
    path("businesses" / LongNumber / "offer-prices" / "updates") { (businessId) => 
      post {  
            entity(as[UpdateBusinessPricesRequest]){ updateBusinessPricesRequest =>
              dbsService.updateBusinessPrices(businessId = businessId, updateBusinessPricesRequest = updateBusinessPricesRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "offers" / "update") { (campaignId) => 
      post {  
            entity(as[UpdateCampaignOffersRequest]){ updateCampaignOffersRequest =>
              dbsService.updateCampaignOffers(campaignId = campaignId, updateCampaignOffersRequest = updateCampaignOffersRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "goods-feedback" / "comments" / "update") { (businessId) => 
      post {  
            entity(as[UpdateGoodsFeedbackCommentRequest]){ updateGoodsFeedbackCommentRequest =>
              dbsService.updateGoodsFeedbackComment(businessId = businessId, updateGoodsFeedbackCommentRequest = updateGoodsFeedbackCommentRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "offer-cards" / "update") { (businessId) => 
      post {  
            entity(as[UpdateOfferContentRequest]){ updateOfferContentRequest =>
              dbsService.updateOfferContent(businessId = businessId, updateOfferContentRequest = updateOfferContentRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "offer-mapping-entries" / "updates") { (campaignId) => 
      post {  
            entity(as[UpdateOfferMappingEntryRequest]){ updateOfferMappingEntryRequest =>
              dbsService.updateOfferMappingEntries(campaignId = campaignId, updateOfferMappingEntryRequest = updateOfferMappingEntryRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "offer-mappings" / "update") { (businessId) => 
      post {  
            entity(as[UpdateOfferMappingsRequest]){ updateOfferMappingsRequest =>
              dbsService.updateOfferMappings(businessId = businessId, updateOfferMappingsRequest = updateOfferMappingsRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "items") { (campaignId, orderId) => 
      put {  
            entity(as[UpdateOrderItemRequest]){ updateOrderItemRequest =>
              dbsService.updateOrderItems(campaignId = campaignId, orderId = orderId, updateOrderItemRequest = updateOrderItemRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "status") { (campaignId, orderId) => 
      put {  
            entity(as[UpdateOrderStatusRequest]){ updateOrderStatusRequest =>
              dbsService.updateOrderStatus(campaignId = campaignId, orderId = orderId, updateOrderStatusRequest = updateOrderStatusRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / "status-update") { (campaignId) => 
      post {  
            entity(as[UpdateOrderStatusesRequest]){ updateOrderStatusesRequest =>
              dbsService.updateOrderStatuses(campaignId = campaignId, updateOrderStatusesRequest = updateOrderStatusesRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "orders" / LongNumber / "delivery" / "storage-limit") { (campaignId, orderId) => 
      put {  
            entity(as[UpdateOrderStorageLimitRequest]){ updateOrderStorageLimitRequest =>
              dbsService.updateOrderStorageLimit(campaignId = campaignId, orderId = orderId, updateOrderStorageLimitRequest = updateOrderStorageLimitRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "outlets" / LongNumber) { (campaignId, outletId) => 
      put {  
            entity(as[ChangeOutletRequest]){ changeOutletRequest =>
              dbsService.updateOutlet(campaignId = campaignId, outletId = outletId, changeOutletRequest = changeOutletRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "outlets" / "licenses") { (campaignId) => 
      post {  
            entity(as[UpdateOutletLicenseRequest]){ updateOutletLicenseRequest =>
              dbsService.updateOutletLicenses(campaignId = campaignId, updateOutletLicenseRequest = updateOutletLicenseRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "offer-prices" / "updates") { (campaignId) => 
      post {  
            entity(as[UpdatePricesRequest]){ updatePricesRequest =>
              dbsService.updatePrices(campaignId = campaignId, updatePricesRequest = updatePricesRequest)
            }
      }
    } ~
    path("businesses" / LongNumber / "promos" / "offers" / "update") { (businessId) => 
      post {  
            entity(as[UpdatePromoOffersRequest]){ updatePromoOffersRequest =>
              dbsService.updatePromoOffers(businessId = businessId, updatePromoOffersRequest = updatePromoOffersRequest)
            }
      }
    } ~
    path("campaigns" / LongNumber / "offers" / "stocks") { (campaignId) => 
      put {  
            entity(as[UpdateStocksRequest]){ updateStocksRequest =>
              dbsService.updateStocks(campaignId = campaignId, updateStocksRequest = updateStocksRequest)
            }
      }
    }
}
...
trait DbsApiService {
  ...
  def deleteOutletLicenses200(responseEmptyApiResponse: EmptyApiResponse)(implicit toEntityMarshallerEmptyApiResponse: ToEntityMarshaller[EmptyApiResponse]): Route =
    complete((200, responseEmptyApiResponse))
  def deleteOutletLicenses400(responseApiClientDataErrorResponse: ApiClientDataErrorResponse)(implicit toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse]): Route =
    complete((400, responseApiClientDataErrorResponse))
  def deleteOutletLicenses401(responseApiUnauthorizedErrorResponse: ApiUnauthorizedErrorResponse)(implicit toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route =
    complete((401, responseApiUnauthorizedErrorResponse))
  def deleteOutletLicenses403(responseApiForbiddenErrorResponse: ApiForbiddenErrorResponse)(implicit toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse]): Route =
    complete((403, responseApiForbiddenErrorResponse))
  def deleteOutletLicenses404(responseApiNotFoundErrorResponse: ApiNotFoundErrorResponse)(implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse]): Route =
    complete((404, responseApiNotFoundErrorResponse))
  def deleteOutletLicenses420(responseApiLimitErrorResponse: ApiLimitErrorResponse)(implicit toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse]): Route =
    complete((420, responseApiLimitErrorResponse))
  def deleteOutletLicenses500(responseApiServerErrorResponse: ApiServerErrorResponse)(implicit toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse]): Route =
    complete((500, responseApiServerErrorResponse))
  /**
   * Code: 200, Message: Пустой ответ., DataType: EmptyApiResponse
   * Code: 400, Message: Запрос содержит неправильные данные., DataType: ApiClientDataErrorResponse
   ...
   * Code: 500, Message: Внутренняя ошибка сервера., DataType: ApiServerErrorResponse
   */
  def deleteOutletLicenses(campaignId: Long, ids: Option[String])
      (implicit toEntityMarshallerEmptyApiResponse: ToEntityMarshaller[EmptyApiResponse], toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse], toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse], toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse], toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse], toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse], toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route
  ...
  def getModel200(responseGetModelsResponse: GetModelsResponse)(implicit toEntityMarshallerGetModelsResponse: ToEntityMarshaller[GetModelsResponse]): Route =
    complete((200, responseGetModelsResponse))
  def getModel400(responseApiClientDataErrorResponse: ApiClientDataErrorResponse)(implicit toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse]): Route =
    complete((400, responseApiClientDataErrorResponse))
  def getModel401(responseApiUnauthorizedErrorResponse: ApiUnauthorizedErrorResponse)(implicit toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route =
    complete((401, responseApiUnauthorizedErrorResponse))
  def getModel403(responseApiForbiddenErrorResponse: ApiForbiddenErrorResponse)(implicit toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse]): Route =
    complete((403, responseApiForbiddenErrorResponse))
  def getModel404(responseApiNotFoundErrorResponse: ApiNotFoundErrorResponse)(implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse]): Route =
    complete((404, responseApiNotFoundErrorResponse))
  def getModel420(responseApiLimitErrorResponse: ApiLimitErrorResponse)(implicit toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse]): Route =
    complete((420, responseApiLimitErrorResponse))
  def getModel500(responseApiServerErrorResponse: ApiServerErrorResponse)(implicit toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse]): Route =
    complete((500, responseApiServerErrorResponse))
  /**
   * Code: 200, Message: Информация о модели., DataType: GetModelsResponse
   * Code: 400, Message: Запрос содержит неправильные данные., DataType: ApiClientDataErrorResponse
   * Code: 401, Message: В запросе не указаны данные для авторизации., DataType: ApiUnauthorizedErrorResponse
   * Code: 403, Message: Данные для авторизации неверны или доступ к ресурсу запрещен., DataType: ApiForbiddenErrorResponse
   * Code: 404, Message: Запрашиваемый ресурс не найден., DataType: ApiNotFoundErrorResponse
   ...
   * Code: 500, Message: Внутренняя ошибка сервера., DataType: ApiServerErrorResponse
   */
  def getModel(modelId: Long, regionId: Long, currency: Option[String])
      (implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse], toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse], toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse], toEntityMarshallerGetModelsResponse: ToEntityMarshaller[GetModelsResponse], toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse], toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse], toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route

  def getModelOffers200(responseGetModelsOffersResponse: GetModelsOffersResponse)(implicit toEntityMarshallerGetModelsOffersResponse: ToEntityMarshaller[GetModelsOffersResponse]): Route =
    complete((200, responseGetModelsOffersResponse))
  def getModelOffers400(responseApiClientDataErrorResponse: ApiClientDataErrorResponse)(implicit toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse]): Route =
    complete((400, responseApiClientDataErrorResponse))
  def getModelOffers401(responseApiUnauthorizedErrorResponse: ApiUnauthorizedErrorResponse)(implicit toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route =
    complete((401, responseApiUnauthorizedErrorResponse))
  def getModelOffers403(responseApiForbiddenErrorResponse: ApiForbiddenErrorResponse)(implicit toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse]): Route =
    complete((403, responseApiForbiddenErrorResponse))
  def getModelOffers404(responseApiNotFoundErrorResponse: ApiNotFoundErrorResponse)(implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse]): Route =
    complete((404, responseApiNotFoundErrorResponse))
  def getModelOffers420(responseApiLimitErrorResponse: ApiLimitErrorResponse)(implicit toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse]): Route =
    complete((420, responseApiLimitErrorResponse))
  def getModelOffers500(responseApiServerErrorResponse: ApiServerErrorResponse)(implicit toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse]): Route =
    complete((500, responseApiServerErrorResponse))
  /**
   * Code: 200, Message: Список предложений для модели., DataType: GetModelsOffersResponse
   * Code: 400, Message: Запрос содержит неправильные данные., DataType: ApiClientDataErrorResponse
   * Code: 401, Message: В запросе не указаны данные для авторизации., DataType: ApiUnauthorizedErrorResponse
   * Code: 403, Message: Данные для авторизации неверны или доступ к ресурсу запрещен., DataType: ApiForbiddenErrorResponse
   * Code: 404, Message: Запрашиваемый ресурс не найден., DataType: ApiNotFoundErrorResponse
   ...
   * Code: 500, Message: Внутренняя ошибка сервера., DataType: ApiServerErrorResponse
   */
  def getModelOffers(modelId: Long, regionId: Long, currency: Option[String], orderByPrice: Option[String], count: Int, page: Int)
      (implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse], toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse], toEntityMarshallerGetModelsOffersResponse: ToEntityMarshaller[GetModelsOffersResponse], toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse], toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse], toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse], toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route

  def getModels200(responseGetModelsResponse: GetModelsResponse)(implicit toEntityMarshallerGetModelsResponse: ToEntityMarshaller[GetModelsResponse]): Route =
    complete((200, responseGetModelsResponse))
  def getModels400(responseApiClientDataErrorResponse: ApiClientDataErrorResponse)(implicit toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse]): Route =
    complete((400, responseApiClientDataErrorResponse))
  def getModels401(responseApiUnauthorizedErrorResponse: ApiUnauthorizedErrorResponse)(implicit toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route =
    complete((401, responseApiUnauthorizedErrorResponse))
  def getModels403(responseApiForbiddenErrorResponse: ApiForbiddenErrorResponse)(implicit toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse]): Route =
    complete((403, responseApiForbiddenErrorResponse))
  def getModels404(responseApiNotFoundErrorResponse: ApiNotFoundErrorResponse)(implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse]): Route =
    complete((404, responseApiNotFoundErrorResponse))
  def getModels420(responseApiLimitErrorResponse: ApiLimitErrorResponse)(implicit toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse]): Route =
    complete((420, responseApiLimitErrorResponse))
  def getModels500(responseApiServerErrorResponse: ApiServerErrorResponse)(implicit toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse]): Route =
    complete((500, responseApiServerErrorResponse))
  /**
   * Code: 200, Message: Информация о моделях., DataType: GetModelsResponse
   * Code: 400, Message: Запрос содержит неправильные данные., DataType: ApiClientDataErrorResponse
   * Code: 401, Message: В запросе не указаны данные для авторизации., DataType: ApiUnauthorizedErrorResponse
   * Code: 403, Message: Данные для авторизации неверны или доступ к ресурсу запрещен., DataType: ApiForbiddenErrorResponse
   * Code: 404, Message: Запрашиваемый ресурс не найден., DataType: ApiNotFoundErrorResponse
   ...
   * Code: 500, Message: Внутренняя ошибка сервера., DataType: ApiServerErrorResponse
   */
  def getModels(regionId: Long, getModelsRequest: GetModelsRequest, currency: Option[String])
      (implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse], toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse], toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse], toEntityMarshallerGetModelsResponse: ToEntityMarshaller[GetModelsResponse], toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse], toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse], toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route

  def getModelsOffers200(responseGetModelsOffersResponse: GetModelsOffersResponse)(implicit toEntityMarshallerGetModelsOffersResponse: ToEntityMarshaller[GetModelsOffersResponse]): Route =
    complete((200, responseGetModelsOffersResponse))
  def getModelsOffers400(responseApiClientDataErrorResponse: ApiClientDataErrorResponse)(implicit toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse]): Route =
    complete((400, responseApiClientDataErrorResponse))
  def getModelsOffers401(responseApiUnauthorizedErrorResponse: ApiUnauthorizedErrorResponse)(implicit toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route =
    complete((401, responseApiUnauthorizedErrorResponse))
  def getModelsOffers403(responseApiForbiddenErrorResponse: ApiForbiddenErrorResponse)(implicit toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse]): Route =
    complete((403, responseApiForbiddenErrorResponse))
  def getModelsOffers404(responseApiNotFoundErrorResponse: ApiNotFoundErrorResponse)(implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse]): Route =
    complete((404, responseApiNotFoundErrorResponse))
  def getModelsOffers420(responseApiLimitErrorResponse: ApiLimitErrorResponse)(implicit toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse]): Route =
    complete((420, responseApiLimitErrorResponse))
  def getModelsOffers500(responseApiServerErrorResponse: ApiServerErrorResponse)(implicit toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse]): Route =
    complete((500, responseApiServerErrorResponse))
  /**
   * Code: 200, Message: Список предложений для моделей., DataType: GetModelsOffersResponse
   * Code: 400, Message: Запрос содержит неправильные данные., DataType: ApiClientDataErrorResponse
   * Code: 401, Message: В запросе не указаны данные для авторизации., DataType: ApiUnauthorizedErrorResponse
   * Code: 403, Message: Данные для авторизации неверны или доступ к ресурсу запрещен., DataType: ApiForbiddenErrorResponse
   * Code: 404, Message: Запрашиваемый ресурс не найден., DataType: ApiNotFoundErrorResponse
   ...
   * Code: 500, Message: Внутренняя ошибка сервера., DataType: ApiServerErrorResponse
   */
  def getModelsOffers(regionId: Long, getModelsRequest: GetModelsRequest, currency: Option[String], orderByPrice: Option[String])
      (implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse], toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse], toEntityMarshallerGetModelsOffersResponse: ToEntityMarshaller[GetModelsOffersResponse], toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse], toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse], toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse], toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route
  ...
  def getOutletLicenses200(responseGetOutletLicensesResponse: GetOutletLicensesResponse)(implicit toEntityMarshallerGetOutletLicensesResponse: ToEntityMarshaller[GetOutletLicensesResponse]): Route =
    complete((200, responseGetOutletLicensesResponse))
  def getOutletLicenses400(responseApiClientDataErrorResponse: ApiClientDataErrorResponse)(implicit toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse]): Route =
    complete((400, responseApiClientDataErrorResponse))
  def getOutletLicenses401(responseApiUnauthorizedErrorResponse: ApiUnauthorizedErrorResponse)(implicit toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route =
    complete((401, responseApiUnauthorizedErrorResponse))
  def getOutletLicenses403(responseApiForbiddenErrorResponse: ApiForbiddenErrorResponse)(implicit toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse]): Route =
    complete((403, responseApiForbiddenErrorResponse))
  def getOutletLicenses404(responseApiNotFoundErrorResponse: ApiNotFoundErrorResponse)(implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse]): Route =
    complete((404, responseApiNotFoundErrorResponse))
  def getOutletLicenses420(responseApiLimitErrorResponse: ApiLimitErrorResponse)(implicit toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse]): Route =
    complete((420, responseApiLimitErrorResponse))
  def getOutletLicenses500(responseApiServerErrorResponse: ApiServerErrorResponse)(implicit toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse]): Route =
    complete((500, responseApiServerErrorResponse))
  /**
   * Code: 200, Message: Найденные лицензии собственных точек продаж., DataType: GetOutletLicensesResponse
   * Code: 400, Message: Запрос содержит неправильные данные., DataType: ApiClientDataErrorResponse
   * Code: 401, Message: В запросе не указаны данные для авторизации., DataType: ApiUnauthorizedErrorResponse
   * Code: 403, Message: Данные для авторизации неверны или доступ к ресурсу запрещен., DataType: ApiForbiddenErrorResponse
   * Code: 404, Message: Запрашиваемый ресурс не найден., DataType: ApiNotFoundErrorResponse
   ...
   * Code: 500, Message: Внутренняя ошибка сервера., DataType: ApiServerErrorResponse
   */
  def getOutletLicenses(campaignId: Long, outletIds: Option[String], ids: Option[String])
      (implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse], toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse], toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse], toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse], toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse], toEntityMarshallerGetOutletLicensesResponse: ToEntityMarshaller[GetOutletLicensesResponse], toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route
  ...
  def searchModels200(responseSearchModelsResponse: SearchModelsResponse)(implicit toEntityMarshallerSearchModelsResponse: ToEntityMarshaller[SearchModelsResponse]): Route =
    complete((200, responseSearchModelsResponse))
  def searchModels400(responseApiClientDataErrorResponse: ApiClientDataErrorResponse)(implicit toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse]): Route =
    complete((400, responseApiClientDataErrorResponse))
  def searchModels401(responseApiUnauthorizedErrorResponse: ApiUnauthorizedErrorResponse)(implicit toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route =
    complete((401, responseApiUnauthorizedErrorResponse))
  def searchModels403(responseApiForbiddenErrorResponse: ApiForbiddenErrorResponse)(implicit toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse]): Route =
    complete((403, responseApiForbiddenErrorResponse))
  def searchModels404(responseApiNotFoundErrorResponse: ApiNotFoundErrorResponse)(implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse]): Route =
    complete((404, responseApiNotFoundErrorResponse))
  def searchModels420(responseApiLimitErrorResponse: ApiLimitErrorResponse)(implicit toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse]): Route =
    complete((420, responseApiLimitErrorResponse))
  def searchModels500(responseApiServerErrorResponse: ApiServerErrorResponse)(implicit toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse]): Route =
    complete((500, responseApiServerErrorResponse))
  /**
   * Code: 200, Message: Информация о моделях., DataType: SearchModelsResponse
   * Code: 400, Message: Запрос содержит неправильные данные., DataType: ApiClientDataErrorResponse
   * Code: 401, Message: В запросе не указаны данные для авторизации., DataType: ApiUnauthorizedErrorResponse
   * Code: 403, Message: Данные для авторизации неверны или доступ к ресурсу запрещен., DataType: ApiForbiddenErrorResponse
   * Code: 404, Message: Запрашиваемый ресурс не найден., DataType: ApiNotFoundErrorResponse
   ...
   * Code: 500, Message: Внутренняя ошибка сервера., DataType: ApiServerErrorResponse
   */
  def searchModels(query: String, regionId: Long, currency: Option[String], page: Int, pageSize: Option[Int])
      (implicit toEntityMarshallerSearchModelsResponse: ToEntityMarshaller[SearchModelsResponse], toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse], toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse], toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse], toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse], toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse], toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route
  ...
  def setReturnDecision200(responseEmptyApiResponse: EmptyApiResponse)(implicit toEntityMarshallerEmptyApiResponse: ToEntityMarshaller[EmptyApiResponse]): Route =
    complete((200, responseEmptyApiResponse))
  def setReturnDecision400(responseApiClientDataErrorResponse: ApiClientDataErrorResponse)(implicit toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse]): Route =
    complete((400, responseApiClientDataErrorResponse))
  def setReturnDecision401(responseApiUnauthorizedErrorResponse: ApiUnauthorizedErrorResponse)(implicit toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route =
    complete((401, responseApiUnauthorizedErrorResponse))
  def setReturnDecision403(responseApiForbiddenErrorResponse: ApiForbiddenErrorResponse)(implicit toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse]): Route =
    complete((403, responseApiForbiddenErrorResponse))
  def setReturnDecision404(responseApiNotFoundErrorResponse: ApiNotFoundErrorResponse)(implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse]): Route =
    complete((404, responseApiNotFoundErrorResponse))
  def setReturnDecision420(responseApiLimitErrorResponse: ApiLimitErrorResponse)(implicit toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse]): Route =
    complete((420, responseApiLimitErrorResponse))
  def setReturnDecision500(responseApiServerErrorResponse: ApiServerErrorResponse)(implicit toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse]): Route =
    complete((500, responseApiServerErrorResponse))
  /**
   * Code: 200, Message: Детали возврата., DataType: EmptyApiResponse
   * Code: 400, Message: Запрос содержит неправильные данные., DataType: ApiClientDataErrorResponse
   * Code: 401, Message: В запросе не указаны данные для авторизации., DataType: ApiUnauthorizedErrorResponse
   * Code: 403, Message: Данные для авторизации неверны или доступ к ресурсу запрещен., DataType: ApiForbiddenErrorResponse
   * Code: 404, Message: Запрашиваемый ресурс не найден., DataType: ApiNotFoundErrorResponse
  ...
  def submitReturnDecision200(responseEmptyApiResponse: EmptyApiResponse)(implicit toEntityMarshallerEmptyApiResponse: ToEntityMarshaller[EmptyApiResponse]): Route =
    complete((200, responseEmptyApiResponse))
  def submitReturnDecision400(responseApiClientDataErrorResponse: ApiClientDataErrorResponse)(implicit toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse]): Route =
    complete((400, responseApiClientDataErrorResponse))
  def submitReturnDecision401(responseApiUnauthorizedErrorResponse: ApiUnauthorizedErrorResponse)(implicit toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route =
    complete((401, responseApiUnauthorizedErrorResponse))
  def submitReturnDecision403(responseApiForbiddenErrorResponse: ApiForbiddenErrorResponse)(implicit toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse]): Route =
    complete((403, responseApiForbiddenErrorResponse))
  def submitReturnDecision404(responseApiNotFoundErrorResponse: ApiNotFoundErrorResponse)(implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse]): Route =
    complete((404, responseApiNotFoundErrorResponse))
  def submitReturnDecision420(responseApiLimitErrorResponse: ApiLimitErrorResponse)(implicit toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse]): Route =
    complete((420, responseApiLimitErrorResponse))
  def submitReturnDecision500(responseApiServerErrorResponse: ApiServerErrorResponse)(implicit toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse]): Route =
    complete((500, responseApiServerErrorResponse))
  /**
   * Code: 200, Message: Статус выполнения операции., DataType: EmptyApiResponse
   * Code: 400, Message: Запрос содержит неправильные данные., DataType: ApiClientDataErrorResponse
  ...
  def updateOutletLicenses200(responseEmptyApiResponse: EmptyApiResponse)(implicit toEntityMarshallerEmptyApiResponse: ToEntityMarshaller[EmptyApiResponse]): Route =
    complete((200, responseEmptyApiResponse))
  def updateOutletLicenses400(responseApiClientDataErrorResponse: ApiClientDataErrorResponse)(implicit toEntityMarshallerApiClientDataErrorResponse: ToEntityMarshaller[ApiClientDataErrorResponse]): Route =
    complete((400, responseApiClientDataErrorResponse))
  def updateOutletLicenses401(responseApiUnauthorizedErrorResponse: ApiUnauthorizedErrorResponse)(implicit toEntityMarshallerApiUnauthorizedErrorResponse: ToEntityMarshaller[ApiUnauthorizedErrorResponse]): Route =
    complete((401, responseApiUnauthorizedErrorResponse))
  def updateOutletLicenses403(responseApiForbiddenErrorResponse: ApiForbiddenErrorResponse)(implicit toEntityMarshallerApiForbiddenErrorResponse: ToEntityMarshaller[ApiForbiddenErrorResponse]): Route =
    complete((403, responseApiForbiddenErrorResponse))
  def updateOutletLicenses404(responseApiNotFoundErrorResponse: ApiNotFoundErrorResponse)(implicit toEntityMarshallerApiNotFoundErrorResponse: ToEntityMarshaller[ApiNotFoundErrorResponse]): Route =
    complete((404, responseApiNotFoundErrorResponse))
  def updateOutletLicenses420(responseApiLimitErrorResponse: ApiLimitErrorResponse)(implicit toEntityMarshallerApiLimitErrorResponse: ToEntityMarshaller[ApiLimitErrorResponse]): Route =
    complete((420, responseApiLimitErrorResponse))
  def updateOutletLicenses500(responseApiServerErrorResponse: ApiServerErrorResponse)(implicit toEntityMarshallerApiServerErrorResponse: ToEntityMarshaller[ApiServerErrorResponse]): Route =
    complete((500, responseApiServerErrorResponse))
  /**
   * Code: 200, Message: Пустой ответ., DataType: EmptyApiResponse
   * Code: 400, Message: Запрос содержит неправильные данные., DataType: ApiClientDataErrorResponse
...
trait DbsApiMarshaller {
  ...
  implicit def fromEntityUnmarshallerGetModelsRequest: FromEntityUnmarshaller[GetModelsRequest]
  ...
  implicit def fromEntityUnmarshallerUpdateOutletLicenseRequest: FromEntityUnmarshaller[UpdateOutletLicenseRequest]
  ...
  implicit def fromEntityUnmarshallerSetReturnDecisionRequest: FromEntityUnmarshaller[SetReturnDecisionRequest]
  ...
  implicit def toEntityMarshallerSearchModelsResponse: ToEntityMarshaller[SearchModelsResponse]
  ...
  implicit def toEntityMarshallerGetModelsOffersResponse: ToEntityMarshaller[GetModelsOffersResponse]
  ...
  implicit def toEntityMarshallerGetOutletLicensesResponse: ToEntityMarshaller[GetOutletLicensesResponse]
  ...
  implicit def toEntityMarshallerGetModelsResponse: ToEntityMarshaller[GetModelsResponse]
