...
class TariffsApi(
    tariffsService: TariffsApiService,
    tariffsMarshaller: TariffsApiMarshaller
) {

  
  import tariffsMarshaller._

  lazy val route: Route =
    path("tariffs" / "calculate") { 
      post {  
            entity(as[CalculateTariffsRequest]){ calculateTariffsRequest =>
              tariffsService.calculateTariffs(calculateTariffsRequest = calculateTariffsRequest)
            }
      }
    }
}
