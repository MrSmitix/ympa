...
public class GetOfferDTO {
  ...
  public static final String SERIALIZED_NAME_SELLING_PROGRAMS = "sellingPrograms";
  @SerializedName(SERIALIZED_NAME_SELLING_PROGRAMS)
  ...
  static {
    ...
    openapiFields.add("sellingPrograms");
   ...
   * @throws IOException if the JSON Element is invalid with respect to GetOfferDTO
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!GetOfferDTO.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in GetOfferDTO is not found in the empty JSON string", GetOfferDTO.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GetOfferDTO.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GetOfferDTO` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : GetOfferDTO.openapiRequiredFields) {
      ...
      if (jsonObj.get("purchasePrice") != null && !jsonObj.get("purchasePrice").isJsonNull()) {
        GetPriceDTO.validateJsonElement(jsonObj.get("purchasePrice"));
      }
      ...
      if (jsonObj.get("additionalExpenses") != null && !jsonObj.get("additionalExpenses").isJsonNull()) {
        GetPriceDTO.validateJsonElement(jsonObj.get("additionalExpenses"));
      }
      ...
      if (jsonObj.get("campaigns") != null && !jsonObj.get("campaigns").isJsonNull()) {
        ...
        if (jsonArraycampaigns != null) {
          ...
          for (int i = 0; i < jsonArraycampaigns.size(); i++) {
            OfferCampaignStatusDTO.validateJsonElement(jsonArraycampaigns.get(i));
          };
        }
      }
      if (jsonObj.get("sellingPrograms") != null && !jsonObj.get("sellingPrograms").isJsonNull()) {
        JsonArray jsonArraysellingPrograms = jsonObj.getAsJsonArray("sellingPrograms");
        if (jsonArraysellingPrograms != null) {
          // ensure the json data is an array
          if (!jsonObj.get("sellingPrograms").isJsonArray()) {
            throw new IllegalArgumentException(String.format("Expected the field `sellingPrograms` to be an array in the JSON string but got `%s`", jsonObj.get("sellingPrograms").toString()));
          }

          // validate the optional field `sellingPrograms` (array)
          for (int i = 0; i < jsonArraysellingPrograms.size(); i++) {
            OfferSellingProgramDTO.validateJsonElement(jsonArraysellingPrograms.get(i));
          };
        }
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!GetOfferDTO.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'GetOfferDTO' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<GetOfferDTO> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(GetOfferDTO.class));

       return (TypeAdapter<T>) new TypeAdapter<GetOfferDTO>() {
           @Override
           public void write(JsonWriter out, GetOfferDTO value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public GetOfferDTO read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of GetOfferDTO given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of GetOfferDTO
   * @throws IOException if the JSON string is invalid with respect to GetOfferDTO
   */
  public static GetOfferDTO fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, GetOfferDTO.class);
  }

  /**
   * Convert an instance of GetOfferDTO to an JSON string
   *
   * @return JSON string
   */
