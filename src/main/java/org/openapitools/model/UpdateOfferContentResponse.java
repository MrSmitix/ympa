...
@Schema(name = "UpdateOfferContentResponse", description = "Описывает проблемы, которые появились при сохранении товара.")
...
public class UpdateOfferContentResponse {
  ...
  @Valid
  private JsonNullable<List<@Valid UpdateOfferContentResultDTO>> results = JsonNullable.<List<@Valid UpdateOfferContentResultDTO>>undefined();
   ...
   */
  @Valid 
  @Schema(name = "results", description = "Ошибки и предупреждения, которые появились при обработке переданных значений. Каждый элемент списка соответствует одному товару.  Если ошибок и предупреждений нет, поле не передается. ", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
  @JsonProperty("results")
  public JsonNullable<List<@Valid UpdateOfferContentResultDTO>> getResults() {
    return results;
  }

  public void setResults(JsonNullable<List<@Valid UpdateOfferContentResultDTO>> results) {
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.status, updateOfferContentResponse.status) &&
        equalsNullable(this.results, updateOfferContentResponse.results);
  }
