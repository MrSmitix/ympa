...
@Schema(name = "GetPricesByOfferIdsRequest", description = "Запрос списка цен.")
...
public class GetPricesByOfferIdsRequest {
   ...
   */
  @Size(min = 1, max = 2000) 
  @Schema(name = "offerIds", description = "Список SKU.  {% note warning \"Такой список возвращается только целиком\" %}  Если вы запрашиваете информацию по конкретным SKU, не заполняйте:  * `page_token` * `limit`  {% endnote %}    ", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
  ...
  public boolean equals(Object o) {
    ...
    return equalsNullable(this.offerIds, getPricesByOfferIdsRequest.offerIds);
  }
  ...
  @Override
  public int hashCode() {
    return Objects.hash(hashCodeNullable(offerIds));
  }
