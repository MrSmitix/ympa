...
public class BusinessesApiVerticle extends AbstractVerticle {
    static final Logger LOGGER = LoggerFactory.getLogger(BusinessesApiVerticle.class);

    static final String GETBUSINESSSETTINGS_SERVICE_ID = "getBusinessSettings";
    
    final BusinessesApi service;

    public BusinessesApiVerticle() {
        try {
            Class serviceImplClass = getClass().getClassLoader().loadClass("org.openapitools.server.api.verticle.BusinessesApiImpl");
            service = (BusinessesApi)serviceImplClass.newInstance();
        } catch (Exception e) {
            logUnexpectedError("BusinessesApiVerticle constructor", e);
