...
public class ExpressApiVerticle extends AbstractVerticle {
    static final Logger LOGGER = LoggerFactory.getLogger(ExpressApiVerticle.class);
    ...
    final ExpressApi service;

    public ExpressApiVerticle() {
        try {
            Class serviceImplClass = getClass().getClassLoader().loadClass("org.openapitools.server.api.verticle.ExpressApiImpl");
            service = (ExpressApi)serviceImplClass.newInstance();
        } catch (Exception e) {
            logUnexpectedError("ExpressApiVerticle constructor", e);
