...
public class OrdersStatsApiVerticle extends AbstractVerticle {
    static final Logger LOGGER = LoggerFactory.getLogger(OrdersStatsApiVerticle.class);

    static final String GETORDERSSTATS_SERVICE_ID = "getOrdersStats";
    
    final OrdersStatsApi service;

    public OrdersStatsApiVerticle() {
        try {
            Class serviceImplClass = getClass().getClassLoader().loadClass("org.openapitools.server.api.verticle.OrdersStatsApiImpl");
            service = (OrdersStatsApi)serviceImplClass.newInstance();
        } catch (Exception e) {
            logUnexpectedError("OrdersStatsApiVerticle constructor", e);
