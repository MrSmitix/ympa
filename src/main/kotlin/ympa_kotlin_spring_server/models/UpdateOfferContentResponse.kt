...
import ympa_kotlin_spring_server.models.UpdateOfferContentResultDTO
...
data class UpdateOfferContentResponse(
    ...
    @field:Valid
    @Schema(example = "null", description = "Ошибки и предупреждения, которые появились при обработке переданных значений. Каждый элемент списка соответствует одному товару.  Если ошибок и предупреждений нет, поле не передается. ")
    @get:JsonProperty("results") val results: kotlin.collections.List<UpdateOfferContentResultDTO>? = null
    ) {
