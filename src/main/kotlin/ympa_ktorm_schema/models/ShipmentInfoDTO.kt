 ...
 */
object ShipmentInfoDTOs : BaseTable<ShipmentInfoDTO>("ShipmentInfoDTO") {
    ...
    val statusDescription = text("statusDescription") /* null */ /* Описание статуса отгрузки. */
    val statusUpdateTime = datetime("statusUpdateTime") /* null */ /* Время последнего изменения статуса отгрузки. */

    /**
     * Create an entity of type ShipmentInfoDTO from the model
     */
    override fun doCreateEntity(row: QueryRowSet, withReferences: Boolean) = ShipmentInfoDTO(
        ...
        statusDescription = row[statusDescription]  /* kotlin.String? */ /* Описание статуса отгрузки. */,
        statusUpdateTime = row[statusUpdateTime]  /* java.time.LocalDateTime? */ /* Время последнего изменения статуса отгрузки. */
    )

    /**
    * Assign all the columns from the entity of type ShipmentInfoDTO to the DML expression.
    *
    * Usage:
    *
    * ```kotlin
    * let entity = ShipmentInfoDTO()
    * database.update(ShipmentInfoDTOs, {
    *     assignFrom(entity)
    * })
    * ```
    * @return the builder with the columns for the update or insert.
    */
    fun AssignmentsBuilder.assignFrom(entity: ShipmentInfoDTO) {
        this.apply {
            set(ShipmentInfoDTOs.id, entity.id)
            set(ShipmentInfoDTOs.planIntervalFrom, entity.planIntervalFrom)
            set(ShipmentInfoDTOs.planIntervalTo, entity.planIntervalTo)
            set(ShipmentInfoDTOs.shipmentType, entity.shipmentType)
            set(ShipmentInfoDTOs.warehouse, entity.warehouse)
            set(ShipmentInfoDTOs.warehouseTo, entity.warehouseTo)
            set(ShipmentInfoDTOs.externalId, entity.externalId)
            set(ShipmentInfoDTOs.deliveryService, entity.deliveryService)
            set(ShipmentInfoDTOs.palletsCount, entity.palletsCount)
            set(ShipmentInfoDTOs.draftCount, entity.draftCount)
            set(ShipmentInfoDTOs.plannedCount, entity.plannedCount)
            set(ShipmentInfoDTOs.factCount, entity.factCount)
            set(ShipmentInfoDTOs.status, entity.status)
            set(ShipmentInfoDTOs.statusDescription, entity.statusDescription)
            set(ShipmentInfoDTOs.statusUpdateTime, entity.statusUpdateTime)
        }
    }
...
object ShipmentInfoDTOOrderIds : BaseTable<Pair<kotlin.Long, kotlin.Long>>("ShipmentInfoDTOOrderIds") {
    val shipmentInfoDTO = long("shipmentInfoDTO")
    val orderIds = long("orderIds")

    override fun doCreateEntity(row: QueryRowSet, withReferences: Boolean): Pair<kotlin.Long, kotlin.Long> =
        Pair(row[shipmentInfoDTO] ?: 0, row[orderIds] ?: 0)

    fun AssignmentsBuilder.assignFrom(entity: Pair<kotlin.Long, kotlin.Long>) {
        this.apply {
            set(ShipmentInfoDTOOrderIds.shipmentInfoDTO, entity.first)
            set(ShipmentInfoDTOOrderIds.orderIds, entity.second)
        }
    }
