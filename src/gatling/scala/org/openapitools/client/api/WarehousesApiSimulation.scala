...
class WarehousesApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val getFulfillmentWarehousesPerSecond = config.getDouble("performance.operationsPerSecond.getFulfillmentWarehouses") * rateMultiplier * instanceMultiplier
    ...
    val scngetFulfillmentWarehouses = scenario("getFulfillmentWarehousesSimulation")
        .exec(http("getFulfillmentWarehouses")
        .httpRequest("GET","/warehouses")
)

    // Run scngetFulfillmentWarehouses with warm up and reach a constant rate for entire duration
    scenarioBuilders += scngetFulfillmentWarehouses.inject(
        rampUsersPerSec(1) to(getFulfillmentWarehousesPerSecond) during(rampUpSeconds),
        constantUsersPerSec(getFulfillmentWarehousesPerSecond) during(durationSeconds),
        rampUsersPerSec(getFulfillmentWarehousesPerSecond) to(1) during(rampDownSeconds)
    )
