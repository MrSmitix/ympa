...
void
SearchShipmentsResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new SearchShipmentsResponseDTO();
}

void
SearchShipmentsResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
SearchShipmentsResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("SearchShipmentsResponseDTO")) {
			jsonToValue(&result, node, "SearchShipmentsResponseDTO", "SearchShipmentsResponseDTO");
		} else {
			
			SearchShipmentsResponseDTO* obj = static_cast<SearchShipmentsResponseDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
SearchShipmentsResponse::SearchShipmentsResponse(char* json)
{
	this->fromJson(json);
}
...
SearchShipmentsResponse::toJson()
{
	...
	if (isprimitive("SearchShipmentsResponseDTO")) {
		SearchShipmentsResponseDTO obj = getResult();
		node = converttoJson(&obj, "SearchShipmentsResponseDTO", "");
	}
	else {
		
		SearchShipmentsResponseDTO obj = static_cast<SearchShipmentsResponseDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
