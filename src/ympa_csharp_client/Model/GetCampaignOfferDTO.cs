/*
 * Партнерский API Маркета
 *
 * API Яндекс Маркета помогает продавцам автоматизировать и упростить работу с маркетплейсом.  В числе возможностей интеграции:  * управление каталогом товаров и витриной,  * обработка заказов,  * изменение настроек магазина,  * получение отчетов. 
 *
 * The version of the OpenAPI document: LATEST
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = ympa_csharp_client.Client.OpenAPIDateConverter;

namespace ympa_csharp_client.Model
{
    /// <summary>
    /// Параметры размещения товара в магазине.
    /// </summary>
    [DataContract(Name = "GetCampaignOfferDTO")]
    public partial class GetCampaignOfferDTO : IValidatableObject
    {

        /// <summary>
        /// Gets or Sets Status
        /// </summary>
        [DataMember(Name = "status", EmitDefaultValue = false)]
        public OfferCampaignStatusType? Status { get; set; }
        /// <summary>
        /// Initializes a new instance of the <see cref="GetCampaignOfferDTO" /> class.
        /// </summary>
        [JsonConstructorAttribute]
        protected GetCampaignOfferDTO() { }
        /// <summary>
        /// Initializes a new instance of the <see cref="GetCampaignOfferDTO" /> class.
        /// </summary>
        /// <param name="offerId">Ваш SKU — идентификатор товара в вашей системе.  Разрешена любая последовательность длиной до 255 знаков.  Правила использования SKU:  * У каждого товара SKU должен быть свой.  * SKU товара нельзя менять — можно только удалить товар и добавить заново с новым SKU.  * Уже заданный SKU нельзя освободить и использовать заново для другого товара. Каждый товар должен получать новый идентификатор, до того никогда не использовавшийся в вашем каталоге.  [Что такое SKU и как его назначать](https://yandex.ru/support/marketplace/assortment/add/index.html#fields)  (required).</param>
        /// <param name="quantum">quantum.</param>
        /// <param name="available">Есть ли товар в продаже. .</param>
        /// <param name="basicPrice">basicPrice.</param>
        /// <param name="campaignPrice">campaignPrice.</param>
        /// <param name="status">status.</param>
        /// <param name="errors">Ошибки, препятствующие размещению товара на витрине. .</param>
        /// <param name="warnings">Предупреждения, не препятствующие размещению товара на витрине. .</param>
        public GetCampaignOfferDTO(string offerId = default(string), QuantumDTO quantum = default(QuantumDTO), bool available = default(bool), GetPriceWithDiscountDTO basicPrice = default(GetPriceWithDiscountDTO), GetPriceWithVatDTO campaignPrice = default(GetPriceWithVatDTO), OfferCampaignStatusType? status = default(OfferCampaignStatusType?), List<OfferErrorDTO> errors = default(List<OfferErrorDTO>), List<OfferErrorDTO> warnings = default(List<OfferErrorDTO>))
        {
            // to ensure "offerId" is required (not null)
            if (offerId == null)
            {
                throw new ArgumentNullException("offerId is a required property for GetCampaignOfferDTO and cannot be null");
            }
            this.OfferId = offerId;
            this.Quantum = quantum;
            this.Available = available;
            this.BasicPrice = basicPrice;
            this.CampaignPrice = campaignPrice;
            this.Status = status;
            this.Errors = errors;
            this.Warnings = warnings;
        }

        /// <summary>
        /// Ваш SKU — идентификатор товара в вашей системе.  Разрешена любая последовательность длиной до 255 знаков.  Правила использования SKU:  * У каждого товара SKU должен быть свой.  * SKU товара нельзя менять — можно только удалить товар и добавить заново с новым SKU.  * Уже заданный SKU нельзя освободить и использовать заново для другого товара. Каждый товар должен получать новый идентификатор, до того никогда не использовавшийся в вашем каталоге.  [Что такое SKU и как его назначать](https://yandex.ru/support/marketplace/assortment/add/index.html#fields) 
        /// </summary>
        /// <value>Ваш SKU — идентификатор товара в вашей системе.  Разрешена любая последовательность длиной до 255 знаков.  Правила использования SKU:  * У каждого товара SKU должен быть свой.  * SKU товара нельзя менять — можно только удалить товар и добавить заново с новым SKU.  * Уже заданный SKU нельзя освободить и использовать заново для другого товара. Каждый товар должен получать новый идентификатор, до того никогда не использовавшийся в вашем каталоге.  [Что такое SKU и как его назначать](https://yandex.ru/support/marketplace/assortment/add/index.html#fields) </value>
        [DataMember(Name = "offerId", IsRequired = true, EmitDefaultValue = true)]
        public string OfferId { get; set; }

        /// <summary>
        /// Gets or Sets Quantum
        /// </summary>
        [DataMember(Name = "quantum", EmitDefaultValue = false)]
        public QuantumDTO Quantum { get; set; }

        /// <summary>
        /// Есть ли товар в продаже. 
        /// </summary>
        /// <value>Есть ли товар в продаже. </value>
        [DataMember(Name = "available", EmitDefaultValue = true)]
        public bool Available { get; set; }

        /// <summary>
        /// Gets or Sets BasicPrice
        /// </summary>
        [DataMember(Name = "basicPrice", EmitDefaultValue = false)]
        public GetPriceWithDiscountDTO BasicPrice { get; set; }

        /// <summary>
        /// Gets or Sets CampaignPrice
        /// </summary>
        [DataMember(Name = "campaignPrice", EmitDefaultValue = false)]
        public GetPriceWithVatDTO CampaignPrice { get; set; }

        /// <summary>
        /// Ошибки, препятствующие размещению товара на витрине. 
        /// </summary>
        /// <value>Ошибки, препятствующие размещению товара на витрине. </value>
        [DataMember(Name = "errors", EmitDefaultValue = true)]
        public List<OfferErrorDTO> Errors { get; set; }

        /// <summary>
        /// Предупреждения, не препятствующие размещению товара на витрине. 
        /// </summary>
        /// <value>Предупреждения, не препятствующие размещению товара на витрине. </value>
        [DataMember(Name = "warnings", EmitDefaultValue = true)]
        public List<OfferErrorDTO> Warnings { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class GetCampaignOfferDTO {\n");
            sb.Append("  OfferId: ").Append(OfferId).Append("\n");
            sb.Append("  Quantum: ").Append(Quantum).Append("\n");
            sb.Append("  Available: ").Append(Available).Append("\n");
            sb.Append("  BasicPrice: ").Append(BasicPrice).Append("\n");
            sb.Append("  CampaignPrice: ").Append(CampaignPrice).Append("\n");
            sb.Append("  Status: ").Append(Status).Append("\n");
            sb.Append("  Errors: ").Append(Errors).Append("\n");
            sb.Append("  Warnings: ").Append(Warnings).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            // OfferId (string) maxLength
            if (this.OfferId != null && this.OfferId.Length > 255)
            {
                yield return new ValidationResult("Invalid value for OfferId, length must be less than 255.", new [] { "OfferId" });
            }

            // OfferId (string) minLength
            if (this.OfferId != null && this.OfferId.Length < 1)
            {
                yield return new ValidationResult("Invalid value for OfferId, length must be greater than 1.", new [] { "OfferId" });
            }

            if (this.OfferId != null) {
                // OfferId (string) pattern
                Regex regexOfferId = new Regex(@"^[^\x00-\x08\x0A-\x1f\x7f]{1,255}$", RegexOptions.CultureInvariant);
                if (!regexOfferId.Match(this.OfferId).Success)
                {
                    yield return new System.ComponentModel.DataAnnotations.ValidationResult("Invalid value for OfferId, must match a pattern of " + regexOfferId, new [] { "OfferId" });
                }
            }

            yield break;
        }
    }

}
