...
void
GetOfferCardsContentStatusResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new OfferCardsContentStatusDTO();
}

void
GetOfferCardsContentStatusResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
GetOfferCardsContentStatusResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("OfferCardsContentStatusDTO")) {
			jsonToValue(&result, node, "OfferCardsContentStatusDTO", "OfferCardsContentStatusDTO");
		} else {
			
			OfferCardsContentStatusDTO* obj = static_cast<OfferCardsContentStatusDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
GetOfferCardsContentStatusResponse::GetOfferCardsContentStatusResponse(char* json)
{
	this->fromJson(json);
}
...
GetOfferCardsContentStatusResponse::toJson()
{
	...
	if (isprimitive("OfferCardsContentStatusDTO")) {
		OfferCardsContentStatusDTO obj = getResult();
		node = converttoJson(&obj, "OfferCardsContentStatusDTO", "");
	}
	else {
		
		OfferCardsContentStatusDTO obj = static_cast<OfferCardsContentStatusDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
