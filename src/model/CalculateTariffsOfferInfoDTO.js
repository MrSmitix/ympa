...
class CalculateTariffsOfferInfoDTO {
    ...
    /**
     * Validates the JSON data with respect to <code>CalculateTariffsOfferInfoDTO</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>CalculateTariffsOfferInfoDTO</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of CalculateTariffsOfferInfoDTO.RequiredProperties) {
        ...
        if (data['offer']) { // data not null
          CalculateTariffsOfferDTO.validateJSON(data['offer']);
        }
        if (data['tariffs']) { // data not null
            ...
            for (const item of data['tariffs']) {
                CalculatedTariffDTO.validateJSON(item);
            };
        }
...
CalculateTariffsOfferInfoDTO.RequiredProperties = ["offer", "tariffs"];

/**
 * @member {module:model/CalculateTariffsOfferDTO} offer
 */
CalculateTariffsOfferInfoDTO.prototype['offer'] = undefined;

/**
 * Список услуг и их стоимость.  По некоторым услугам могут возвращаться несколько разных стоимостей. Например, в модели FBS стоимость услуги `SORTING` (обработка заказа) зависит от способа отгрузки и количества заказов в отгрузке. Подробнее о тарифах на услуги читайте [в Справке Маркета для продавцов](https://yandex.ru/support2/marketplace/ru/introduction/rates/models/). 
 * @member {Array.<module:model/CalculatedTariffDTO>} tariffs
 */
CalculateTariffsOfferInfoDTO.prototype['tariffs'] = undefined;
