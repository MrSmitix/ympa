...
void
GetOrderLabelsDataResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new OrderLabelDTO();
}

void
GetOrderLabelsDataResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
GetOrderLabelsDataResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("OrderLabelDTO")) {
			jsonToValue(&result, node, "OrderLabelDTO", "OrderLabelDTO");
		} else {
			
			OrderLabelDTO* obj = static_cast<OrderLabelDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
GetOrderLabelsDataResponse::GetOrderLabelsDataResponse(char* json)
{
	this->fromJson(json);
}
...
GetOrderLabelsDataResponse::toJson()
{
	...
	if (isprimitive("OrderLabelDTO")) {
		OrderLabelDTO obj = getResult();
		node = converttoJson(&obj, "OrderLabelDTO", "");
	}
	else {
		
		OrderLabelDTO obj = static_cast<OrderLabelDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
