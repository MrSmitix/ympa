...
void
GetWarehousesResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new WarehousesDTO();
}

void
GetWarehousesResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
GetWarehousesResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("WarehousesDTO")) {
			jsonToValue(&result, node, "WarehousesDTO", "WarehousesDTO");
		} else {
			
			WarehousesDTO* obj = static_cast<WarehousesDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
GetWarehousesResponse::GetWarehousesResponse(char* json)
{
	this->fromJson(json);
}
...
GetWarehousesResponse::toJson()
{
	...
	if (isprimitive("WarehousesDTO")) {
		WarehousesDTO obj = getResult();
		node = converttoJson(&obj, "WarehousesDTO", "");
	}
	else {
		
		WarehousesDTO obj = static_cast<WarehousesDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
