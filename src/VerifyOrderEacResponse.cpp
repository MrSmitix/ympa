...
void
VerifyOrderEacResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new EacVerificationResultDTO();
}

void
VerifyOrderEacResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
VerifyOrderEacResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("EacVerificationResultDTO")) {
			jsonToValue(&result, node, "EacVerificationResultDTO", "EacVerificationResultDTO");
		} else {
			
			EacVerificationResultDTO* obj = static_cast<EacVerificationResultDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
VerifyOrderEacResponse::VerifyOrderEacResponse(char* json)
{
	this->fromJson(json);
}
...
VerifyOrderEacResponse::toJson()
{
	...
	if (isprimitive("EacVerificationResultDTO")) {
		EacVerificationResultDTO obj = getResult();
		node = converttoJson(&obj, "EacVerificationResultDTO", "");
	}
	else {
		
		EacVerificationResultDTO obj = static_cast<EacVerificationResultDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
