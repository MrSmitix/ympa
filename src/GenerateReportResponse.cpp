...
void
GenerateReportResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new GenerateReportDTO();
}

void
GenerateReportResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
GenerateReportResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("GenerateReportDTO")) {
			jsonToValue(&result, node, "GenerateReportDTO", "GenerateReportDTO");
		} else {
			
			GenerateReportDTO* obj = static_cast<GenerateReportDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
GenerateReportResponse::GenerateReportResponse(char* json)
{
	this->fromJson(json);
}
...
GenerateReportResponse::toJson()
{
	...
	if (isprimitive("GenerateReportDTO")) {
		GenerateReportDTO obj = getResult();
		node = converttoJson(&obj, "GenerateReportDTO", "");
	}
	else {
		
		GenerateReportDTO obj = static_cast<GenerateReportDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
