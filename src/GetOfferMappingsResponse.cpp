...
void
GetOfferMappingsResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new GetOfferMappingsResultDTO();
}

void
GetOfferMappingsResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
GetOfferMappingsResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("GetOfferMappingsResultDTO")) {
			jsonToValue(&result, node, "GetOfferMappingsResultDTO", "GetOfferMappingsResultDTO");
		} else {
			
			GetOfferMappingsResultDTO* obj = static_cast<GetOfferMappingsResultDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
GetOfferMappingsResponse::GetOfferMappingsResponse(char* json)
{
	this->fromJson(json);
}
...
GetOfferMappingsResponse::toJson()
{
	...
	if (isprimitive("GetOfferMappingsResultDTO")) {
		GetOfferMappingsResultDTO obj = getResult();
		node = converttoJson(&obj, "GetOfferMappingsResultDTO", "");
	}
	else {
		
		GetOfferMappingsResultDTO obj = static_cast<GetOfferMappingsResultDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
