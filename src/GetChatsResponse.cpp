...
void
GetChatsResponse::__init()
{
	//status = new ApiResponseStatusType();
	//result = new GetChatsInfoDTO();
}

void
GetChatsResponse::__cleanup()
{
	//if(status != NULL) {
	//
	//delete status;
...
void
GetChatsResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("GetChatsInfoDTO")) {
			jsonToValue(&result, node, "GetChatsInfoDTO", "GetChatsInfoDTO");
		} else {
			
			GetChatsInfoDTO* obj = static_cast<GetChatsInfoDTO*> (&result);
			obj->fromJson(json_to_string(node, false));
...
GetChatsResponse::GetChatsResponse(char* json)
{
	this->fromJson(json);
}
...
GetChatsResponse::toJson()
{
	...
	if (isprimitive("GetChatsInfoDTO")) {
		GetChatsInfoDTO obj = getResult();
		node = converttoJson(&obj, "GetChatsInfoDTO", "");
	}
	else {
		
		GetChatsInfoDTO obj = static_cast<GetChatsInfoDTO> (getResult());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
