/*
 * Партнерский API Маркета
 *
 * API Яндекс Маркета помогает продавцам автоматизировать и упростить работу с маркетплейсом.  В числе возможностей интеграции:  * управление каталогом товаров и витриной,  * обработка заказов,  * изменение настроек магазина,  * получение отчетов. 
 *
 * The version of the OpenAPI document: LATEST
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// GenerateUnitedNettingReportRequest : Данные, необходимые для генерации отчета: идентификатор магазина, период, за который нужен отчет, а также фильтры. 
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct GenerateUnitedNettingReportRequest {
    /// Идентификатор бизнеса.
    #[serde(rename = "businessId")]
    pub business_id: i64,
    /// {% note warning \"\" %}  Этот параметр устарел. Не используйте его.  {% endnote %}  Начало периода, включительно. 
    #[serde(rename = "dateTimeFrom", skip_serializing_if = "Option::is_none")]
    pub date_time_from: Option<String>,
    /// {% note warning \"\" %}  Этот параметр устарел. Не используйте его.  {% endnote %}  Конец периода, включительно. Максимальный период — 1 год. 
    #[serde(rename = "dateTimeTo", skip_serializing_if = "Option::is_none")]
    pub date_time_to: Option<String>,
    /// Начало периода, включительно.
    #[serde(rename = "dateFrom", skip_serializing_if = "Option::is_none")]
    pub date_from: Option<String>,
    /// Конец периода, включительно. Максимальный период — 1 год.
    #[serde(rename = "dateTo", skip_serializing_if = "Option::is_none")]
    pub date_to: Option<String>,
    /// Номер платежного поручения.
    #[serde(rename = "bankOrderId", skip_serializing_if = "Option::is_none")]
    pub bank_order_id: Option<i64>,
    /// Дата платежного поручения.
    #[serde(rename = "bankOrderDateTime", skip_serializing_if = "Option::is_none")]
    pub bank_order_date_time: Option<String>,
    /// Список моделей, которые нужны в отчете. 
    #[serde(rename = "placementPrograms", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub placement_programs: Option<Option<Vec<models::PlacementType>>>,
    /// Список ИНН, которые нужны в отчете.
    #[serde(rename = "inns", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub inns: Option<Option<Vec<String>>>,
    /// Список магазинов, которые нужны в отчете.
    #[serde(rename = "campaignIds", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub campaign_ids: Option<Option<Vec<i64>>>,
}

impl GenerateUnitedNettingReportRequest {
    /// Данные, необходимые для генерации отчета: идентификатор магазина, период, за который нужен отчет, а также фильтры. 
    pub fn new(business_id: i64) -> GenerateUnitedNettingReportRequest {
        GenerateUnitedNettingReportRequest {
            business_id,
            date_time_from: None,
            date_time_to: None,
            date_from: None,
            date_to: None,
            bank_order_id: None,
            bank_order_date_time: None,
            placement_programs: None,
            inns: None,
            campaign_ids: None,
        }
    }
}

