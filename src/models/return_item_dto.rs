/*
 * Партнерский API Маркета
 *
 * API Яндекс Маркета помогает продавцам автоматизировать и упростить работу с маркетплейсом.  В числе возможностей интеграции:  * управление каталогом товаров и витриной,  * обработка заказов,  * изменение настроек магазина,  * получение отчетов. 
 *
 * The version of the OpenAPI document: LATEST
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// ReturnItemDto : Список товаров в возврате.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct ReturnItemDto {
    /// SKU на Маркете.
    #[serde(rename = "marketSku", skip_serializing_if = "Option::is_none")]
    pub market_sku: Option<i64>,
    /// Ваш SKU — идентификатор товара в вашей системе.  Разрешена любая последовательность длиной до 255 знаков.  Правила использования SKU:  * У каждого товара SKU должен быть свой.  * SKU товара нельзя менять — можно только удалить товар и добавить заново с новым SKU.  * Уже заданный SKU нельзя освободить и использовать заново для другого товара. Каждый товар должен получать новый идентификатор, до того никогда не использовавшийся в вашем каталоге.  [Что такое SKU и как его назначать](https://yandex.ru/support/marketplace/assortment/add/index.html#fields) 
    #[serde(rename = "shopSku")]
    pub shop_sku: String,
    /// Количество единиц товара.
    #[serde(rename = "count")]
    pub count: i64,
    /// Список решений по возврату.
    #[serde(rename = "decisions", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub decisions: Option<Option<Vec<models::ReturnDecisionDto>>>,
    /// Список логистических позиций возврата.
    #[serde(rename = "instances", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub instances: Option<Option<Vec<models::ReturnInstanceDto>>>,
    /// Список трек-кодов для почтовых отправлений.
    #[serde(rename = "tracks", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub tracks: Option<Option<Vec<models::TrackDto>>>,
}

impl ReturnItemDto {
    /// Список товаров в возврате.
    pub fn new(shop_sku: String, count: i64) -> ReturnItemDto {
        ReturnItemDto {
            market_sku: None,
            shop_sku,
            count,
            decisions: None,
            instances: None,
            tracks: None,
        }
    }
}

