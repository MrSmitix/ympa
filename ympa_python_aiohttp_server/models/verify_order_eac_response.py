# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from ympa_python_aiohttp_server.models.base_model import Model
from ympa_python_aiohttp_server.models.api_response_status_type import ApiResponseStatusType
from ympa_python_aiohttp_server.models.eac_verification_result_dto import EacVerificationResultDTO
from ympa_python_aiohttp_server import util


class VerifyOrderEacResponse(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, status: ApiResponseStatusType=None, result: EacVerificationResultDTO=None):
        """VerifyOrderEacResponse - a model defined in OpenAPI

        :param status: The status of this VerifyOrderEacResponse.
        :param result: The result of this VerifyOrderEacResponse.
        """
        self.openapi_types = {
            'status': ApiResponseStatusType,
            'result': EacVerificationResultDTO
        }

        self.attribute_map = {
            'status': 'status',
            'result': 'result'
        }

        self._status = status
        self._result = result

    @classmethod
    def from_dict(cls, dikt: dict) -> 'VerifyOrderEacResponse':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The VerifyOrderEacResponse of this VerifyOrderEacResponse.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def status(self):
        """Gets the status of this VerifyOrderEacResponse.


        :return: The status of this VerifyOrderEacResponse.
        :rtype: ApiResponseStatusType
        """
        return self._status

    @status.setter
    def status(self, status):
        """Sets the status of this VerifyOrderEacResponse.


        :param status: The status of this VerifyOrderEacResponse.
        :type status: ApiResponseStatusType
        """

        self._status = status

    @property
    def result(self):
        """Gets the result of this VerifyOrderEacResponse.


        :return: The result of this VerifyOrderEacResponse.
        :rtype: EacVerificationResultDTO
        """
        return self._result

    @result.setter
    def result(self, result):
        """Sets the result of this VerifyOrderEacResponse.


        :param result: The result of this VerifyOrderEacResponse.
        :type result: EacVerificationResultDTO
        """

        self._result = result
