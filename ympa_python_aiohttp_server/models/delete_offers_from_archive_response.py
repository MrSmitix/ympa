# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from ympa_python_aiohttp_server.models.base_model import Model
from ympa_python_aiohttp_server.models.api_response_status_type import ApiResponseStatusType
from ympa_python_aiohttp_server.models.delete_offers_from_archive_dto import DeleteOffersFromArchiveDTO
from ympa_python_aiohttp_server import util


class DeleteOffersFromArchiveResponse(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, status: ApiResponseStatusType=None, result: DeleteOffersFromArchiveDTO=None):
        """DeleteOffersFromArchiveResponse - a model defined in OpenAPI

        :param status: The status of this DeleteOffersFromArchiveResponse.
        :param result: The result of this DeleteOffersFromArchiveResponse.
        """
        self.openapi_types = {
            'status': ApiResponseStatusType,
            'result': DeleteOffersFromArchiveDTO
        }

        self.attribute_map = {
            'status': 'status',
            'result': 'result'
        }

        self._status = status
        self._result = result

    @classmethod
    def from_dict(cls, dikt: dict) -> 'DeleteOffersFromArchiveResponse':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The DeleteOffersFromArchiveResponse of this DeleteOffersFromArchiveResponse.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def status(self):
        """Gets the status of this DeleteOffersFromArchiveResponse.


        :return: The status of this DeleteOffersFromArchiveResponse.
        :rtype: ApiResponseStatusType
        """
        return self._status

    @status.setter
    def status(self, status):
        """Sets the status of this DeleteOffersFromArchiveResponse.


        :param status: The status of this DeleteOffersFromArchiveResponse.
        :type status: ApiResponseStatusType
        """

        self._status = status

    @property
    def result(self):
        """Gets the result of this DeleteOffersFromArchiveResponse.


        :return: The result of this DeleteOffersFromArchiveResponse.
        :rtype: DeleteOffersFromArchiveDTO
        """
        return self._result

    @result.setter
    def result(self, result):
        """Sets the result of this DeleteOffersFromArchiveResponse.


        :param result: The result of this DeleteOffersFromArchiveResponse.
        :type result: DeleteOffersFromArchiveDTO
        """

        self._result = result
