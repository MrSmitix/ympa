...
class CampaignSettingsDTO(BaseModel):
    ...
    show_in_premium: Optional[StrictBool] = Field(default=None, description="Признак показа предложений магазина в рекламном блоке над результатами поиска (cпецразмещение). Возможные значения: * `false` — предложения не показываются в блоке cпецразмещения. * `true` — предложения показываются в блоке cпецразмещения. ", alias="showInPremium")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "localRegion": CampaignSettingsLocalRegionDTO.from_dict(obj["localRegion"]) if obj.get("localRegion") is not None else None
        })
        return _obj
