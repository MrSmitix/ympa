...
class GetPricesByOfferIdsRequest(BaseModel):
    ...
    Запрос списка цен.
    """ # noqa: E501
    offer_ids: Optional[Annotated[List[Annotated[str, Field(min_length=1, strict=True, max_length=255)]], Field(min_length=1, max_length=2000)]] = Field(default=None, description="Список SKU.  {% note warning \"Такой список возвращается только целиком\" %}  Если вы запрашиваете информацию по конкретным SKU, не заполняйте:  * `page_token` * `limit`  {% endnote %}    ", alias="offerIds")
    __properties: ClassVar[List[str]] = ["offerIds"]
